...
Update()
{
  ...
  if (state->lgar_bmi_params.allow_flux_caching){
    ...
    if ( (subtimestep_h == state->lgar_bmi_params.forcing_resolution_h) ){
      if ( (state->lgar_bmi_input_params->precipitation_mm_per_h < 1.E-6) && ( (state->lgar_mass_balance.previous_AET / (state->lgar_mass_balance.previous_PET + 1.E-8 )) < 0.75) && (state->lgar_bmi_params.cache_count!=num_timesteps_before_reset_cache) && (volon_timestep_cm==0) && (state->lgar_mass_balance.previous_recharge<1.E-4) ){
