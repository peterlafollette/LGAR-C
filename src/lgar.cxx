...
/*
  ...
  refinement is in capillary-head space, while MIN_DEPTH_SPAN_CM is the density
  limiter in physical space.  Lowering MIN_DEPTH_SPAN_CM allows more closely
  packed inserted fronts and a smoother psi profile, but increases per-step
  cost and can create very dense chains in low-K dry tails.  Raising it leaves
  larger psi jumps across already-short intervals, but those intervals have
  little storage support and should not need more wetting-front resolution.
*/
#define TO_PSI_GAP_REFINEMENT_MIN_GAP_CM 5.0
#define TO_PSI_GAP_REFINEMENT_RELATIVE_GAP_FACTOR 0.25
#define TO_PSI_GAP_REFINEMENT_MIN_DEPTH_SPAN_CM 1.0
#define TO_PSI_GAP_REFINEMENT_LAYER_FRONT_PADDING 2
...
#define TO_PSI_GAP_REFINEMENT_MOBILE_BOUNDARY_MIN_DEPTH_SPAN_CM TO_PSI_GAP_REFINEMENT_MIN_DEPTH_SPAN_CM
...
static double lgarto_surface_fronts_cross_layer_boundary_upward(
  ...
  double *frozen_factor, struct soil_properties_ *soil_properties,
  struct wetting_front **head, const char *context);
static struct wetting_front *lgarto_insert_missing_to_bottom_scaffold(
  int layer_num, double *cum_layer_thickness_cm, struct wetting_front **head,
  struct wetting_front *reference_front);
...
static bool lgar_is_mobile_groundwater_storage_pair_member(
  const struct wetting_front *front, struct wetting_front *head,
...
static double lgar_boundary_roundtrip_psi_tolerance_cm(double psi_a_cm, double psi_b_cm)
{
  ...
  return fmax(1.E-1, 1.E-5 * psi_scale_cm);
}

static bool lgarto_same_layer_TO_GW_zero_storage_duplicate(
  const struct wetting_front *current,
  const struct wetting_front *next,
  bool preserve_saturated_support_pair)
{
  ...
  // Equal representable theta in one soil has no storage jump even when
  // retained and reinverted psi differ.
  return (!preserve_saturated_support_pair ||
...
static bool lgarto_same_layer_TO_GW_near_contact_merge_ready(
  const struct wetting_front *current,
  const struct wetting_front *next)
{
  if (current == NULL || next == NULL) {
  ...
  const double gap_cm = next->depth_cm - current->depth_cm;
  return current->is_WF_GW && next->is_WF_GW &&
         current->layer_num == next->layer_num &&
         !current->to_bottom && !next->to_bottom &&
         (current->theta < next->theta ||
          lgarto_same_layer_TO_GW_zero_storage_duplicate(current, next, true)) &&
         gap_cm > TRUNCATION_DEPTH &&
         gap_cm <= LGARTO_TO_PACKET_EVENT_SPLIT_MERGE_READY_SPACING_CM;
}

static bool lgarto_reversed_same_layer_TO_GW_duplicate_merge_ready(
  const struct wetting_front *current,
  const struct wetting_front *next,
  struct wetting_front *head,
  double *cum_layer_thickness_cm,
  bool mobile_groundwater_level,
  double expected_pair_depth_cm)
{
  if (!lgarto_same_layer_TO_GW_zero_storage_duplicate(current, next, false) ||
  ...
  const bool both_saturated =
    current->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM &&
    next->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM;
  const bool mobile_support_pair =
    mobile_groundwater_level && both_saturated &&
    (lgar_is_mobile_groundwater_storage_pair_member(
       current, head, cum_layer_thickness_cm, expected_pair_depth_cm) ||
     lgar_is_mobile_groundwater_storage_pair_member(
       next, head, cum_layer_thickness_cm, expected_pair_depth_cm));
  // Saturated fixed-GW duplicates are storage-neutral; only retain the
  // colocated psi=0 pair that explicitly represents mobile-GW CR storage.
  return !mobile_support_pair;
}
...
static bool lgar_boundary_psi_snap_candidate(const struct wetting_front *upper,
                                             const struct wetting_front *lower,
                                             int *soil_type,
                                             struct soil_properties_ *soil_properties)
{
                                                ...
                                                double *mass_change_out_cm)
{
  ...
  upper->theta = calc_theta_from_h(upper->psi_cm,
                                                     ...
                                                     struct wetting_front *lower)
{
  ...
  std::cerr << "Error: boundary psi canonicalization would change total storage too much.\n"
            ...
            << " depth_cm=" << upper->depth_cm
            << " old_psi_cm=" << old_psi_cm
            << " snapped_psi_cm=" << lower->psi_cm << "\n"
            << "  lower front_num=" << lower->front_num
            << " layer=" << lower->layer_num
            << " depth_cm=" << lower->depth_cm
            << " psi_cm=" << lower->psi_cm << "\n"
            << "  |storage_change_cm|=" << std::fabs(mass_change_cm)
            << " > " << BOUNDARY_PSI_SNAP_MASS_TOLERANCE_CM << "\n"
...
static std::vector<struct wetting_front *> lgar_collect_boundary_upper_fronts(
  struct wetting_front *head)
{
  std::vector<struct wetting_front *> boundary_upper_fronts;
  for (struct wetting_front *current = head;
       current != NULL && current->next != NULL;
       current = current->next) {
    if (current->to_bottom && current->layer_num != current->next->layer_num) {
      boundary_upper_fronts.push_back(current);
    }
  }

  return boundary_upper_fronts;
}

struct lgar_preserved_boundary_psi_state
{
...
static bool lgar_TO_psi_matches_theta(const struct wetting_front *front,
                                      int *soil_type,
                                      struct soil_properties_ *soil_properties)
{
  if (front == NULL || !front->is_WF_GW || !std::isfinite(front->psi_cm) ||
      front->psi_cm < 0.0) {
    return false;
  }

  const struct soil_properties_ &soil = soil_properties[soil_type[front->layer_num]];
  // Preserve a valid TO psi when theta cannot uniquely recover it near saturation or residual.
  return front->theta == calc_theta_from_h(front->psi_cm, soil.vg_alpha_per_cm,
                                            soil.vg_m, soil.vg_n,
                                            soil.theta_e, soil.theta_r);
}
...
lgar_collect_lossless_boundary_psi_states(struct wetting_front *head,
                                          ...
                                          struct soil_properties_ *soil_properties)
{
  std::vector<lgar_preserved_boundary_psi_state> states;
  for (struct wetting_front *upper = head;
       upper != NULL && upper->next != NULL; upper = upper->next) {
    struct wetting_front *lower = upper->next;
    if (!upper->to_bottom || upper->layer_num == lower->layer_num ||
        !std::isfinite(upper->psi_cm) || upper->psi_cm != lower->psi_cm) {
      continue;
    }
    ...
    if (lossless) {
      ...
      if (upper->is_WF_GW && lower->is_WF_GW) {
        chain_start = head;
      ...
      // Preserve the exact same-layer TO chain feeding a lossless boundary pair;
      // theta cannot uniquely recover psi near residual or saturation.
      for (struct wetting_front *member = chain_start; member != lower->next;
           member = member->next) {
        states.push_back({member, upper->psi_cm});
      }
    }
  }
  return states;
...
static bool lgar_restore_lossless_boundary_psi(
  struct wetting_front *front,
  const std::vector<lgar_preserved_boundary_psi_state> &states)
{
  for (const lgar_preserved_boundary_psi_state &state : states) {
    if (state.front == front) {
      front->psi_cm = state.psi_cm;
      return true;
    }
  }
  return false;
...
static bool lgarto_has_out_of_layer_movable_TO_front(int num_layers,
                                                     double *cum_layer_thickness_cm,
                                                     struct wetting_front *head)
{
  const double layer_tol_cm = fmax(1.0e-8, 10.0 * TRUNCATION_DEPTH);
  for (struct wetting_front *front = head; front != NULL; front = front->next) {
    if (!front->is_WF_GW || front->to_bottom ||
        front->layer_num < 1 || front->layer_num > num_layers) {
      continue;
    }
    const double layer_top_cm = cum_layer_thickness_cm[front->layer_num - 1];
    const double layer_bottom_cm = cum_layer_thickness_cm[front->layer_num];
    if (front->depth_cm < layer_top_cm - layer_tol_cm ||
        front->depth_cm > layer_bottom_cm + layer_tol_cm) {
      return true;
    }
  }
  return false;
                                                         ...
                                                         double subtimestep_h)
{
  ...
  if (current_offset_cm * projected_offset_cm < 0.0) {
    // Damp any one-step crossing of the hydrostatic target.  The target can be
    // shallower than a fixed fraction of D, so key this to the actual crossing.
                                                                      ...
                                                                      double vadose_lower_boundary_depth_cm)
{
  ...
  double lower_boundary_depth_cm = cum_layer_thickness_cm[num_layers];
  if (std::isfinite(vadose_lower_boundary_depth_cm) &&
      vadose_lower_boundary_depth_cm > lower_boundary_depth_cm) {
    lower_boundary_depth_cm = vadose_lower_boundary_depth_cm;
  }
...
struct TOPsiGapRefinementCandidate {
  ...
  double psi_gap_cm;
  double refinement_lower_depth_cm;
  double refinement_lower_psi_cm;
  bool uses_mobile_boundary_support;
};
...
static double lgarto_restore_to_psi_gap_mass_via_depth(struct wetting_front *upper_front,
                                                       ...
                                                       struct wetting_front *head,
                                                       double refinement_lower_depth_cm,
                                                       bool allow_inserted_depth_restore)
{
  ...
  double original_lower_depth_cm = lower_front->depth_cm;
  if (std::isfinite(refinement_lower_depth_cm) &&
      refinement_lower_depth_cm > original_upper_depth_cm + 2.0 * CREATION_COLOCATED_TOLERANCE_CM &&
      refinement_lower_depth_cm <= lower_front->depth_cm + CREATION_COLOCATED_TOLERANCE_CM) {
    original_lower_depth_cm = refinement_lower_depth_cm;
  }
  ...
  if (allow_inserted_depth_restore &&
      mass_at_original_geometry + TO_PSI_GAP_REFINEMENT_MASS_TOLERANCE_CM < target_mass) {
    const double maximum_inserted_depth_cm = inserted->depth_cm;
    const double minimum_inserted_depth_cm =
      original_upper_depth_cm + DEPTH_AVOIDS_SAME_WF_DEPTH;

    auto apply_inserted_depth_geometry = [&](double inserted_depth_cm) {
      upper_front->depth_cm = original_upper_depth_cm;
      inserted->depth_cm = inserted_depth_cm;
    };

    apply_inserted_depth_geometry(minimum_inserted_depth_cm);
    const double mass_at_minimum_inserted_depth =
      lgar_calc_mass_bal(cum_layer_thickness_cm, head);

    if (mass_at_minimum_inserted_depth + TO_PSI_GAP_REFINEMENT_MASS_TOLERANCE_CM >= target_mass) {
      double bracket_shallow_depth_cm = minimum_inserted_depth_cm;
      double bracket_deep_depth_cm = maximum_inserted_depth_cm;
      double best_inserted_depth_cm = minimum_inserted_depth_cm;
      double best_mass_error_cm = fabs(target_mass - mass_at_minimum_inserted_depth);

      for (int iter = 0; iter < 80; iter++) {
        const double probe_inserted_depth_cm =
          0.5 * (bracket_shallow_depth_cm + bracket_deep_depth_cm);
        apply_inserted_depth_geometry(probe_inserted_depth_cm);
        ...
        if (fabs(current_mass_error_cm) < best_mass_error_cm) {
          ...
          best_inserted_depth_cm = probe_inserted_depth_cm;
        }
        ...
        if (current_mass_error_cm > 0.0) {
          bracket_deep_depth_cm = probe_inserted_depth_cm;
        }
        else {
          bracket_shallow_depth_cm = probe_inserted_depth_cm;
        }
      }

      apply_inserted_depth_geometry(best_inserted_depth_cm);
      return target_mass - lgar_calc_mass_bal(cum_layer_thickness_cm, head);
    }
...
static bool lgarto_mobile_boundary_support_refinement_target(int num_layers,
                                                             double *cum_layer_thickness_cm,
                                                             const struct wetting_front *current,
                                                             const struct wetting_front *next,
                                                             bool mobile_groundwater_level,
                                                             ...
                                                             double *refinement_lower_psi_cm)
{
  if (!mobile_groundwater_level || current == NULL || next == NULL ||
      refinement_lower_depth_cm == NULL || refinement_lower_psi_cm == NULL ||
  ...
  const double layer_top_depth_cm = cum_layer_thickness_cm[current->layer_num - 1];
  const double layer_bottom_depth_cm = cum_layer_thickness_cm[current->layer_num];
  if (groundwater_depth_cm <= layer_top_depth_cm + CREATION_COLOCATED_TOLERANCE_CM ||
      groundwater_depth_cm > layer_bottom_depth_cm + CREATION_COLOCATED_TOLERANCE_CM) {
    return false;
  }

  if (groundwater_depth_cm <= current->depth_cm + 2.0 * DEPTH_AVOIDS_SAME_WF_DEPTH ||
      groundwater_depth_cm > next->depth_cm + CREATION_COLOCATED_TOLERANCE_CM) {
    return false;
  }

  *refinement_lower_depth_cm = fmin(groundwater_depth_cm, next->depth_cm);
  if (*refinement_lower_depth_cm - current->depth_cm <
      TO_PSI_GAP_REFINEMENT_MOBILE_BOUNDARY_MIN_DEPTH_SPAN_CM) {
    return false;
  }

  *refinement_lower_psi_cm = 0.0;
  return true;
}
                                                   ...
                                                   TOPsiGapRefinementCandidate *candidate)
{
  ...
  std::vector<int> to_front_count_by_layer(num_layers + 1, 0);
  for (struct wetting_front *front = head; front != NULL; front = front->next) {
    if (front->is_WF_GW == TRUE &&
        front->layer_num >= 1 && front->layer_num <= num_layers) {
      to_front_count_by_layer[front->layer_num]++;
    }
  }
  bool found = false;
  ...
  for (struct wetting_front *current = head; current != NULL && current->next != NULL;
    ...
    const double layer_depth_span_cm =
      cum_layer_thickness_cm[current->layer_num] -
      cum_layer_thickness_cm[current->layer_num - 1];
    const int layer_front_cap =
      (int) ceil(layer_depth_span_cm / TO_PSI_GAP_REFINEMENT_MIN_DEPTH_SPAN_CM) +
      TO_PSI_GAP_REFINEMENT_LAYER_FRONT_PADDING;
    // This cap is the layer-scale counterpart to MIN_DEPTH_SPAN_CM.  It does
    // not delete existing fronts, but it stops this optional psi-smoothing pass
    // from adding more TO/GW fronts once a layer is already near the requested
    // spatial density.  Larger padding permits a few support/canonical fronts
    // beyond the ideal spacing; smaller padding makes refinement stop sooner.
    if (to_front_count_by_layer[current->layer_num] >= layer_front_cap) {
      continue;
    }

    double refinement_lower_depth_cm = next->depth_cm;
    double refinement_lower_psi_cm = next->psi_cm;
    bool uses_mobile_boundary_support = false;

    // Leave fixed boundary-support intervals alone. With mobile groundwater, the
    // actual water table can lie above the to_bottom scaffold; in that case only
    // refine the active capillary-fringe interval down to the mobile boundary.
    if (next->to_bottom == TRUE) {
      if (!lgarto_mobile_boundary_support_refinement_target(
            num_layers, cum_layer_thickness_cm, current, next,
            mobile_groundwater_level, groundwater_depth_cm,
            &refinement_lower_depth_cm, &refinement_lower_psi_cm)) {
        continue;
      }
      uses_mobile_boundary_support = true;
    }

    const double refinement_depth_span_cm = refinement_lower_depth_cm - current->depth_cm;
    if (refinement_depth_span_cm <= DEPTH_AVOIDS_SAME_WF_DEPTH) {
      continue;
    }

    // Once a TO interval is already this short, another midpoint insert mostly
    // adds bookkeeping cost.  Keep the stored profile coarser there instead of
    // creating dense front packets that are expensive for interflow and CR sync.
    if (refinement_depth_span_cm < TO_PSI_GAP_REFINEMENT_MIN_DEPTH_SPAN_CM) {
      continue;
    }

    const double psi_gap_cm = current->psi_cm - refinement_lower_psi_cm;
    const double local_psi_midpoint_cm =
      0.5 * (fmax(0.0, current->psi_cm) + fmax(0.0, refinement_lower_psi_cm));
    ...
    candidate->refinement_lower_depth_cm = refinement_lower_depth_cm;
    candidate->refinement_lower_psi_cm = refinement_lower_psi_cm;
    candidate->uses_mobile_boundary_support = uses_mobile_boundary_support;
  }
                                               ...
                                               double groundwater_depth_cm)
{
  ...
  if (!lgarto_find_large_to_psi_gap_candidate(num_layers, cum_layer_thickness_cm, *head,
                                              ...
                                              groundwater_depth_cm, &candidate)) {
    return false;
  }
  ...
  const double residual_mass_error_cm =
    fabs(lgarto_restore_to_psi_gap_mass_via_depth(candidate.upper_front, inserted, candidate.lower_front,
                                                  ...
                                                  candidate.refinement_lower_depth_cm,
                                                  candidate.uses_mobile_boundary_support));
  if (residual_mass_error_cm > TO_PSI_GAP_REFINEMENT_MASS_TOLERANCE_CM ||
      inserted->depth_cm <= candidate.upper_front->depth_cm + DEPTH_AVOIDS_SAME_WF_DEPTH) {
    if (*head != NULL) {
      listDelete(*head);
    }
    *head = snapshot;
    if (verbosity.compare("high") == 0) {
      printf("Rolling back TO psi-gap refinement due to residual mass error %.17lf cm "
             "or insufficient inserted-front depth spacing.\n",
  ...
  if (verbosity.compare("high") == 0) {
    printf("Inserted corrective TO front at depth %.6f cm with psi %.6f cm to refine a psi gap of %.6f cm%s.\n",
           inserted->depth_cm, inserted->psi_cm, candidate.psi_gap_cm,
           candidate.uses_mobile_boundary_support ? " above the mobile groundwater boundary" : "");
    listPrint(*head);
  }
...
extern void InitFromConfigFile(string config_file, struct model_state *state)
{
  ...
  state->lgar_bmi_params.mobile_groundwater_reference_depth_cm = 0.0;
  state->lgar_bmi_params.mobile_groundwater_reference_CR_storage_cm = 0.0;
  ...
  state->lgar_bmi_params.initial_extra_moisture_factor_cm = 0.0;
  state->lgar_bmi_params.initial_wetting_fronts_per_layer = 16;
  ...
  while (fp) {
	...
	state->lgar_bmi_params.cum_layer_thickness_cm[layer] = state->lgar_bmi_params.cum_layer_thickness_cm[layer-1] + vec[layer-1];
	      ...
	      state->lgar_bmi_params.mobile_groundwater_reference_depth_cm = state->lgar_bmi_params.soil_depth_cm;
    ...
    else if (param_key == "initial_extra_moisture_factor_cm") {
      state->lgar_bmi_params.initial_extra_moisture_factor_cm = stod(param_value);
      continue;
    }
    else if (param_key == "initial_wetting_fronts_per_layer") {
      state->lgar_bmi_params.initial_wetting_fronts_per_layer = stoi(param_value);
      if (state->lgar_bmi_params.initial_wetting_fronts_per_layer < 2) {
        std::cerr<<"Invalid option: initial_wetting_fronts_per_layer must be >= 2. \n";
        abort();
      }
      continue;
    }
    ...
    else if (param_key == "lateral_flow_psi_threshold" ||
             param_key == "lateral_flow_psi_threshold_cm" ||
             param_key == "lateral_flow_threshold_psi") {
      state->lgar_bmi_params.lateral_flow_psi_threshold_cm = stod(param_value);
      is_lateral_flow_psi_threshold_set = true;

      if (verbosity.compare("high") == 0) {
	std::cerr<<"lateral_flow_psi_threshold [cm] : "
  ...
  if (state->lgar_bmi_params.log_mode){
    ...
    if (is_lateral_flow_psi_threshold_set && is_lateral_flow_factor_set){
      state->lgar_bmi_params.lateral_flow_psi_threshold_cm =
        pow(10.0, state->lgar_bmi_params.lateral_flow_psi_threshold_cm);
      state->lgar_bmi_params.lateral_flow_factor =
        pow(10.0, state->lgar_bmi_params.lateral_flow_factor);
    }
  }
  ...
  if (is_lateral_flow_factor_set) {
    ...
    if (state->lgar_bmi_params.lateral_flow_factor < 1.E-4 ||
        state->lgar_bmi_params.lateral_flow_factor > 1.E4) {
      stringstream errMsg;
  ...
  const bool non_vadose_restart_loaded =
    is_state_path_set && !state->lgar_bmi_params.init_non_vadose_state_path.empty();
  if (!non_vadose_restart_loaded) {
    state->lgar_mass_balance.CR_fast_storage_cm =
      state->lgar_bmi_params.initial_CR_fast_storage_cm;
    state->lgar_mass_balance.CR_slow_storage_cm =
  ...
  if (state->lgar_bmi_params.mobile_groundwater_level &&
    ...
    if (trial_head != NULL) {
      (void) lgarto_maintain_mobile_groundwater_support(
        state->lgar_bmi_params.groundwater_depth_cm,
        state->lgar_bmi_params.num_layers,
        state->lgar_bmi_params.cum_layer_thickness_cm,
        state->lgar_bmi_params.layer_soil_type,
        ...
        &trial_head,
        state->soil_properties);
      double explicit_mass_change_cm = 0.0;
      (void) lgarto_sync_mobile_groundwater_support_to_CR_storage(
        ...
        &trial_head,
        state->soil_properties,
        &solved_groundwater_depth_cm,
        &explicit_mass_change_cm);
      const double chain_storage_cm =
        lgarto_mobile_groundwater_CR_storage_cm(
          state->lgar_bmi_params.num_layers,
          state->lgar_bmi_params.cum_layer_thickness_cm,
          state->lgar_bmi_params.layer_soil_type,
          trial_head,
      ...
      if (std::isfinite(chain_storage_cm) &&
        ...
        state->lgar_bmi_params.groundwater_depth_cm =
          fmax(0.0, solved_groundwater_depth_cm);
        trial_head = NULL;
      }
				    ...
				    struct wetting_front** head, struct soil_properties_ *soil_properties)
{
  if (TO_enabled) {
    ...
    const int number_of_WFs_per_layer = initial_wetting_fronts_per_layer;
    ...
    for (int front = 1; front <= (num_layers * number_of_WFs_per_layer); front++) {
      ...
      if ((front % number_of_WFs_per_layer) == 0) {
        wf_in_layer = 1;
        initial_psi_cm = (layer_thickness_cm[layer] / number_of_WFs_per_layer) - initial_extra_moisture_factor_cm;
        if (layer < num_layers) {
          initial_psi_cm =
            (total_depth -
             (cum_layer_thickness_cm[layer - 1] +
              (number_of_WFs_per_layer - 1) * layer_thickness_cm[layer] / number_of_WFs_per_layer)) -
            initial_extra_moisture_factor_cm;
        }
      ...
      if (mobile_groundwater_level && front == num_layers * number_of_WFs_per_layer) {
        initial_psi_cm = 0.0;
      }
...
extern void lgar_global_mass_balance(struct model_state *state, double *giuh_runoff_queue_cm)
{
  ...
  double global_error_cm = volstart + volprecip - volrunoff - vollateral_flow - volAET - volon - volrech - volend + volchange_calib_cm - volrunoff_CR - volCRend + volCRstart;
...
extern int wetting_front_free_drainage_mobile_groundwater(struct wetting_front* head) {
  const int legacy_front_num = wetting_front_free_drainage(head);
  struct wetting_front *legacy_front = listFindFront(legacy_front_num, head, NULL);
  if (legacy_front == NULL) {
    return legacy_front_num;
  }

  if (legacy_front->next != NULL && legacy_front->is_WF_GW == FALSE) {
    return legacy_front_num;
  }

  int candidate_front_num = -1;
  for (struct wetting_front *current = head; current != NULL; current = current->next) {
    if (current->is_WF_GW == FALSE && current->next != NULL) {
      candidate_front_num = current->front_num;
    }
  }

  if (candidate_front_num > 0) {
    if (verbosity.compare("high") == 0 &&
        candidate_front_num != legacy_front_num) {
      printf("Mobile groundwater adjusted free-drainage/infiltration front "
             "from terminal/non-surface front %d to surface front %d.\n",
             legacy_front_num,
             candidate_front_num);
    }
    return candidate_front_num;
  }

  if (legacy_front->next == NULL && legacy_front_num > 1) {
    return legacy_front_num - 1;
  }
                                                             ...
                                                             bool lgarto_active)
{
  ...
  bool limit_at_TO_handoff = false;
  if (lgarto_active && timestep_h > 0.0 && !current->is_WF_GW && !current->to_bottom &&
      original_dzdt_cm_per_h > 0.0 && vadose_lower_boundary_depth_cm > 0.0) {
    const double max_surface_depth_cm = 1.10 * vadose_lower_boundary_depth_cm;
    if (limited_depth_cm > max_surface_depth_cm) {
      limited_depth_cm = max_surface_depth_cm;
      limit_at_domain_overshoot = true;
    }

    if (next != NULL && next->is_WF_GW && next->layer_num == current->layer_num &&
        next->depth_cm > original_depth_cm + DEPTH_AVOIDS_SAME_WF_DEPTH) {
      /*
        Let the existing surface/TO correction machinery handle the contact,
        but only after a tiny controlled overtake.  A full-step leap through the
        TO chain can otherwise collapse several fronts and dump the swept mass
        into CR in one numerical event.
      */
      const double handoff_depth_cm =
        next->depth_cm + DEPTH_AVOIDS_SAME_WF_DEPTH;
      if (projected_depth_cm > next->depth_cm &&
          limited_depth_cm > handoff_depth_cm) {
        limited_depth_cm = handoff_depth_cm;
        limit_at_TO_handoff = true;
        limit_at_domain_overshoot = false;
      }
    }
  }

  if (!limit_at_domain_overshoot && !limit_at_TO_handoff) {
  ...
  if (limit_at_domain_overshoot && verbosity.compare("high") == 0) {
    printf("Domain-limited LGARTO surface wetting front %d from projected depth %.17lf cm "
           "to %.17lf cm (110%% of active lower boundary %.17lf cm; old_dzdt=%.17lf cm/h, "
           "new_dzdt=%.17lf cm/h).\n",
           current->front_num,
           projected_depth_cm,
           current->depth_cm,
           vadose_lower_boundary_depth_cm,
           original_dzdt_cm_per_h,
           current->dzdt_cm_per_h);
  }
  if (limit_at_TO_handoff && verbosity.compare("high") == 0) {
    printf("Event-limited LGARTO surface wetting front %d from projected depth %.17lf cm "
           "to %.17lf cm for local surface/TO handoff at front %d depth %.17lf cm "
           "(old_dzdt=%.17lf cm/h, new_dzdt=%.17lf cm/h).\n",
           current->front_num,
           projected_depth_cm,
           current->depth_cm,
           next->front_num,
...
static bool lgarto_to_bottom_surface_front_connected_to_GW_below(
  const struct wetting_front *front)
{
  if (front == NULL || front->next == NULL || front->to_bottom != TRUE ||
      front->is_WF_GW == TRUE || front->next->is_WF_GW != TRUE) {
    return false;
  }

  const double psi_tol_cm =
    lgar_boundary_roundtrip_psi_tolerance_cm(front->psi_cm,
                                             front->next->psi_cm);
  return !std::isfinite(front->psi_cm) ||
  ...
  for (int pass = 0; pass < max_passes; pass++) {
    ...
    while (temp_WF != NULL && temp_WF->next != NULL) {
      if (temp_WF->is_WF_GW == FALSE && temp_WF->next->is_WF_GW == TRUE &&
        ...
        temp_WF->dzdt_cm_per_h = 0.0;
        converted_any = true;
        converted_this_pass = true;
      }
	      else if (temp_WF->to_bottom == TRUE && temp_WF->is_WF_GW == FALSE &&
	               temp_WF->next->is_WF_GW == TRUE) {
	        const bool hydraulically_connected_or_wetter =
	          lgarto_to_bottom_surface_front_connected_to_GW_below(temp_WF);
	        if (!hydraulically_connected_or_wetter) {
	          if (verbosity.compare("high") == 0) {
	            printf("%s leaving front %d as surface because its psi %.15f "
	                   "is drier than GW psi %.15f below it.\n",
	                   correction_context, temp_WF->front_num,
	                   temp_WF->psi_cm, temp_WF->next->psi_cm);
	          }
	        }
                                                               ...
                                                               struct soil_properties_ *soil_properties)
{
  ...
  // The pair conversion can expose a hydraulically connected upstream layer-boundary
  // surface bridge; classify it with the TO/GW chain before AET can extract from it.
  lgarto_convert_surface_fronts_drier_than_TO_below(head, "mixed surface/surface/TO correction");
...
static bool lgarto_find_mobile_groundwater_storage_pair(struct wetting_front *head,
                                                        double depth_tol_cm,
                                                        struct wetting_front **vadose_side_front,
												struct wetting_front **saturated_support_front,
												double expected_pair_depth_cm = NAN);

static bool lgar_lateral_is_saturated_gw_front(struct wetting_front *front,
                                               int num_layers,
                                               int *soil_type,
                                               struct soil_properties_ *soil_properties)
{
  if (front == NULL || !front->is_WF_GW || soil_type == NULL ||
      soil_properties == NULL ||
      front->layer_num < 1 || front->layer_num > num_layers) {
    return false;
  }

  const int soil = soil_type[front->layer_num];
  const double theta_e = soil_properties[soil].theta_e;
  return front->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM ||
         front->theta >= theta_e - 1.0e-10;
}
...
extern bool lgar_lateral_front_is_mobile_CR_chain(struct wetting_front *front,
                                                  ...
                                                  struct soil_properties_ *soil_properties)
{
  if (front == NULL || head == NULL || !front->is_WF_GW ||
      num_layers <= 0 || cum_layer_thickness_cm == NULL) {
    return false;
  }

  const double fixed_column_depth_cm = cum_layer_thickness_cm[num_layers];
  const double support_tol_cm =
    fmax(MOBILE_GROUNDWATER_SUBMERGENCE_TOL_CM,
         1.0e-10 * fmax(1.0, fixed_column_depth_cm));
  struct wetting_front *vadose_side_front = NULL;
  struct wetting_front *saturated_support_front = NULL;
  if (!lgarto_find_mobile_groundwater_storage_pair(head, support_tol_cm,
                                                   &vadose_side_front,
                                                   &saturated_support_front)) {
    return false;
  }

  if (front == vadose_side_front || front == saturated_support_front) {
    return true;
  }

  for (struct wetting_front *current = saturated_support_front;
       current != NULL; current = current->next) {
    if (!lgar_lateral_is_saturated_gw_front(current, num_layers, soil_type,
                                            soil_properties)) {
      break;
    }
    if (current == front) {
...
static double lgar_lateral_flow_support_depth_cm(double layer_top_cm,
                                                 double layer_bottom_cm,
                                                 struct wetting_front *previous,
                                                 struct wetting_front *current)
{
  if (current == NULL || !std::isfinite(current->depth_cm) ||
      !std::isfinite(layer_top_cm) || !std::isfinite(layer_bottom_cm) ||
      layer_bottom_cm <= layer_top_cm) {
    return 0.0;
  }
  ...
  double support_bottom_cm =
    current->to_bottom ? layer_bottom_cm : current->depth_cm;
  support_bottom_cm = fmax(layer_top_cm, fmin(layer_bottom_cm, support_bottom_cm));
  support_top_cm = fmax(layer_top_cm, fmin(layer_bottom_cm, support_top_cm));

  return fmax(0.0, support_bottom_cm - support_top_cm);
}

extern double lgarto_lateral_flux_candidate_cm(double timestep_h,
                                               int num_layers,
                                               ...
                                               int *soil_type,
                                               struct soil_properties_ *soil_properties,
                                               const bool *cached_is_mobile_CR_chain)
{
  if (timestep_h <= 0.0 || lateral_flow_factor <= 0.0 ||
      current == NULL || cum_layer_thickness_cm == NULL ||
      current->layer_num < 1 || current->layer_num > num_layers ||
  ...
  // Event projections cache this unchanged membership instead of rescanning
  // the entire wetting-front list during every bisection trial.
  if (mobile_groundwater_level &&
      (cached_is_mobile_CR_chain != NULL
         ? *cached_is_mobile_CR_chain
         : lgar_lateral_front_is_mobile_CR_chain(
             current, head, num_layers, cum_layer_thickness_cm,
  ...
  const double layer_top_cm = cum_layer_thickness_cm[current->layer_num - 1];
  const double layer_bottom_cm = cum_layer_thickness_cm[current->layer_num];
  const double support_depth_cm =
    lgar_lateral_flow_support_depth_cm(layer_top_cm, layer_bottom_cm,
                                       previous, current);
  if (support_depth_cm <= 0.0) {
    return 0.0;
  }

  const double vadose_fraction = fmin(1.0, support_depth_cm / column_depth_cm);
  const double lateral_flux_cm_per_h =
    fmax(0.0, current->K_cm_per_h) * lateral_flow_factor * vadose_fraction;
...
static void lgar_calc_surface_lateral_fluxes_by_front(double timestep_h,
                                                      ...
                                                      int *soil_type,
                                                      struct soil_properties_ *soil_properties,
                                                      std::vector<double>& lateral_flux_cm_by_front)
{
  ...
  for (struct wetting_front *current = head; current != NULL;
    ...
    if (current->to_bottom) {
      ...
      if (target == NULL || target->is_WF_GW) {
        continue;
      }
    }

    const double lateral_flux_cm =
      lgarto_lateral_flux_candidate_cm(timestep_h, num_layers,
                                       lateral_flow_psi_threshold_cm,
                                       lateral_flow_factor,
                                       cum_layer_thickness_cm, head, previous,
                                       current, mobile_groundwater_level,
                                       soil_type, soil_properties);
    if (lateral_flux_cm <= 0.0 || target->front_num < 0 ||
        target->front_num >= (int) lateral_flux_cm_by_front.size()) {
      continue;
    }

    lateral_flux_cm_by_front[target->front_num] += lateral_flux_cm;
  }
}
...
{
  if (applied_lateral_flux_cm != NULL) {
    *applied_lateral_flux_cm = 0.0;
  }
  if (requested_lateral_flux_cm <= 0.0 ||
      !std::isfinite(original_depth_cm) ||
      !std::isfinite(layer_bottom_cm) ||
      !std::isfinite(next_depth_cm)) {
    return original_depth_cm;
  }
  double max_depth_cm =
    fmin(layer_bottom_cm, next_depth_cm) - DEPTH_AVOIDS_SAME_WF_DEPTH;
  max_depth_cm = fmax(original_depth_cm, max_depth_cm);
  if (max_depth_cm <= original_depth_cm + DEPTH_AVOIDS_SAME_WF_DEPTH) {
    return original_depth_cm;
  }

  /*
    Keep the mover and event predictor on one spacing/tolerance/mass-to-depth
    calculation so a later numerical change cannot make their paths diverge.
  */
  const double removal_cm_per_depth_cm = next_theta - theta;
  if (!std::isfinite(removal_cm_per_depth_cm) ||
      removal_cm_per_depth_cm <= 0.0) {
    return original_depth_cm;
  }

  const double max_removed_cm =
    (max_depth_cm - original_depth_cm) * removal_cm_per_depth_cm;
  const double target_removed_cm =
    fmin(requested_lateral_flux_cm, max_removed_cm);
  double projected_depth_cm = original_depth_cm;
  if (target_removed_cm >= max_removed_cm - MBAL_ITERATIVE_TOLERANCE) {
    projected_depth_cm = max_depth_cm;
  }
  else {
    projected_depth_cm =
      fmin(max_depth_cm,
           original_depth_cm + target_removed_cm / removal_cm_per_depth_cm);
  }

  const double applied_cm =
    fmax(0.0, (projected_depth_cm - original_depth_cm) *
              removal_cm_per_depth_cm);
  if (applied_lateral_flux_cm != NULL) {
    *applied_lateral_flux_cm = applied_cm;
  }
  return applied_cm > 0.0 ? projected_depth_cm : original_depth_cm;
}
...
static double lgar_apply_lateral_flux_to_TO_front_by_depth(double requested_lateral_flux_cm,
                                                           ...
                                                           double *lateral_flow_subtimestep_cm)
{
  if (requested_lateral_flux_cm <= 0.0 || front == NULL || head == NULL ||
      *head == NULL || !front->is_WF_GW || front->to_bottom ||
      front->layer_num < 1 || front->layer_num > num_layers ||
      cum_layer_thickness_cm == NULL || front->next == NULL ||
      front->next->layer_num != front->layer_num) {
    return 0.0;
  }

  double applied_lateral_flux_cm = 0.0;
  front->depth_cm =
    lgarto_project_TO_interflow_depth_cm(
      requested_lateral_flux_cm, front->depth_cm,
      cum_layer_thickness_cm[front->layer_num], front->next->depth_cm,
      front->theta, front->next->theta, &applied_lateral_flux_cm);
  if (lateral_flow_subtimestep_cm != NULL) {
    *lateral_flow_subtimestep_cm += applied_lateral_flux_cm;
  }
						     ...
						     bool mobile_groundwater_level)
{
  ...
  struct wetting_front *next_old;
	  ...
	  double TO_lower_boundary_depth_cm = column_depth;
	  if (std::isfinite(groundwater_depth_cm) &&
	      groundwater_depth_cm > TO_lower_boundary_depth_cm) {
	    TO_lower_boundary_depth_cm = groundwater_depth_cm;
	  }
  ...
  lgar_calc_surface_lateral_fluxes_by_front(timestep_h, num_layers,
                                            lateral_flow_psi_threshold_cm,
                                            lateral_flow_factor,
                                            cum_layer_thickness_cm,
                                            state_previous,
  ...
  const bool mobile_groundwater_boundary_active =
    lgarto_active &&
    std::isfinite(groundwater_depth_cm) &&
    groundwater_depth_cm > 0.0 &&
    std::fabs(groundwater_depth_cm - column_depth) > LOWER_BOUNDARY_FINAL_TOL_CM;
  ...
  double precip_mass_to_add = (*volin_cm); // water to be added to the soil
	  ...
	  bool routed_surface_to_gw_boundary_snap_flux = false;
  ...
  auto add_AET_with_pet_budget = [&](double requested_increment_cm,
    ...
    return accepted_increment_cm;
	  ...
	  auto apply_dry_over_wet_mass_residual = [&](double mass_change_cm,
	                                              const char *context) {
    if (fabs(mass_change_cm) <= MBAL_ITERATIVE_TOLERANCE) {
      return;
    }

    if (mass_change_cm > 0.0) {
      const double lower_boundary_flux_correction_cm = -mass_change_cm;
      lgarto_assert_gw_flux_mass_balance_correction_within_debug_threshold(
    ...
    (void) add_AET_with_pet_budget(-mass_change_cm, context, true);
  };
  ...
  bool lower_surface_front_was_deleted = false;
  for (int wf = deepest_surface_front_index; wf != 0; wf--) {

    // A deletion changes the moisture pedestal beneath only the next shallower front.
    const bool use_current_surface_prior_mass = lower_surface_front_was_deleted;
    lower_surface_front_was_deleted = false;
    ...
    double shared_upper_boundary_psi_cm =
      ...
      lgar_theta_is_compatible_with_shared_psi(previous, current->psi_cm,
                                               soil_type, soil_properties) &&
      lgar_theta_is_compatible_with_shared_psi(current, current->psi_cm,
                                               soil_type, soil_properties)
        ? current->psi_cm : NAN;
    ...
    */
	...
	printf("case (deepest wetting front within layer) : layer_num (%d) != layer_num_below (%d) \n", layer_num, layer_num_below);
      ...
      const bool surface_to_gw_boundary_snap =
        current->to_bottom == TRUE && current->is_WF_GW == FALSE &&
        next != NULL && next->is_WF_GW == TRUE;
      const double mass_before_boundary_snap_cm =
        surface_to_gw_boundary_snap ? lgar_calc_mass_bal(cum_layer_thickness_cm, *head) : 0.0;

      current->theta = calc_theta_from_h(next->psi_cm, vg_a,vg_m, vg_n, theta_e, theta_r);
      current->psi_cm = next->psi_cm;

      if (surface_to_gw_boundary_snap) {
        const double boundary_snap_flux_cm =
          mass_before_boundary_snap_cm - lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
        if (std::fabs(boundary_snap_flux_cm) > MBAL_ITERATIVE_TOLERANCE) {
          lgarto_assert_gw_flux_mass_balance_correction_within_debug_threshold(
            boundary_snap_flux_cm,
            "lgar_move_wetting_fronts",
            "surface to_bottom boundary snap to TO/GW chain",
            *head);
          bottom_boundary_flux_cm += boundary_snap_flux_cm;
          routed_surface_to_gw_boundary_snap_flux = true;
          if (verbosity.compare("high") == 0) {
            printf("Surface to_bottom boundary snap routed %.17lf cm to bottom-boundary flux.\n",
                   boundary_snap_flux_cm);
          }
        }
      }
    ...
    */
	...
	printf("case (number_of_wetting_fronts equal to num_layers) : l (%d) == num_layers (%d) == num_wetting_fronts(%d) \n", wf, num_layers,number_of_wetting_fronts);
      ...
      lgar_apply_surface_depth_update_with_event_limit(current, next, timestep_h, surface_lower_boundary_depth_cm, num_layers,
  ...
  //does this really need a dynamic array in this case???
	...
	delta_thickness[k] = layer_thickness;
      ...
      // Retain a shared cross-layer psi when residual theta cannot uniquely recover it.
      current->psi_cm =
        std::isfinite(shared_upper_boundary_psi_cm) &&
        lgar_theta_is_compatible_with_shared_psi(
          current, shared_upper_boundary_psi_cm, soil_type, soil_properties)
          ? shared_upper_boundary_psi_cm
          : calc_h_from_Se(Se, vg_a, vg_m, vg_n);
	...
	double prior_mass = use_current_surface_prior_mass
	  ? current->depth_cm * (current->theta - next->theta)
	...
	if (wf < (int) lateral_flux_cm_by_front.size()) {
	  ...
	  if (current->depth_cm > surface_lower_boundary_depth_cm && listLength_surface(*head)==listLength(*head)) {
	    if (surface_lower_boundary_depth_cm >= column_depth - LOWER_BOUNDARY_FINAL_TOL_CM) {
		    current->depth_cm = surface_lower_boundary_depth_cm + TRUNCATION_DEPTH; //we want WFs to exceed the lower boundary in the event that they must be partially truncated and then WFs above this one will correctly have their moisture corrected, but also want WFs to not exceed the lower boundary much
	    }
		  }
	...
	if (current->dzdt_cm_per_h == 0.0 && current->to_bottom == FALSE) // a new front was just created, so don't update it.
      ...
      if ((prior_mass/current->depth_cm + next->theta)<theta_r){
        ...
        *AET_demand_cm -= aet_reduction_cm;
        if (use_TO_surface_AET) {
          accepted_positive_AET_bookkeeping_cm =
            fmax(0.0, accepted_positive_AET_bookkeeping_cm - aet_reduction_cm);
        ...
        lower_surface_front_was_deleted = true;
      }
  ...
  free(delta_thickness);
    ...
    if (wf == top_most_surface_front_index) { 
      ...
      wf_free_drainage_demand =
        mobile_groundwater_boundary_active
          ? wetting_front_free_drainage_mobile_groundwater(*head)
          : wetting_front_free_drainage(*head);
    ...
    // if ((wf == wf_free_drainage_demand) && (current->theta>=theta_e) ) {
      ...
      const double total_ET_demand_cm = use_TO_surface_AET ? *AET_demand_cm : actual_ET_demand;
	      double mass_timestep = (old_mass + precip_mass_to_add) -
	        (total_ET_demand_cm + free_drainage_demand +
	         cached_lower_boundary_flux_correction_cm +
	         lateral_flow_for_mass_balance_cm());
        ...
        lgar_wetting_front_cross_domain_boundary(surface_lower_boundary_depth_cm,
          ...
          if (isnan(bottom_boundary_flux_cm)){
		    ...
		    if (correction_type_surf==4){
		      ...
		      apply_dry_over_wet_mass_residual(
            mass_change,
  ...
  if (has_TO_fronts) {
	    if (use_TO_surface_AET) {
	      ...
	      double explicit_to_aet_lower_boundary_depth_cm = root_zone_depth_cm;
	      if (mobile_groundwater_level && std::isfinite(groundwater_depth_cm) &&
	          groundwater_depth_cm >= 0.0) {
	        explicit_to_aet_lower_boundary_depth_cm =
	          fmax(0.0, fmin(root_zone_depth_cm, groundwater_depth_cm));
	      }
    ...
    if (verbosity.compare("high") == 0) {
      ...
      // Dense TO chains make repeated listFindFront() calls expensive; build a
      // stable front-number lookup for this depth-update pass instead.
      std::vector<struct wetting_front *> front_by_num(updated_wetting_front_count + 1, NULL);
      for (struct wetting_front *front = *head; front != NULL; front = front->next) {
        if (front->front_num >= 1 && front->front_num <= updated_wetting_front_count) {
      ...
      if (requested_TO_lateral_flux_cm > 0.0) {
        (void) lgar_apply_lateral_flux_to_TO_front_by_depth(
          ...
          cum_layer_thickness_cm, head, lateral_flow_subtimestep_cm);
      }
      ...
      double dzdt_cap_boundary_cm = TO_lower_boundary_depth_cm;
      double allowed_boundary_overshoot_cm = 0.001 * TO_lower_boundary_depth_cm;
      if (current->layer_num >= 1 && current->layer_num < num_layers) {
        dzdt_cap_boundary_cm = cum_layer_thickness_cm[current->layer_num];
      }
      else if (current->layer_num == num_layers) {
        dzdt_cap_boundary_cm = TO_lower_boundary_depth_cm;
      }
      if (current->layer_num >= 1 && current->layer_num < num_layers) {
      ...
      const double to_motion_flux_cm = delta_depth * delta_theta;
	      if (!TO_WFs_above_surface_WFs_flag) {
	        current->depth_cm += delta_depth;
	        bottom_boundary_flux_cm += to_motion_flux_cm;
	      }
	      else if (routed_surface_to_gw_boundary_snap_flux) {
	        if (verbosity.compare("high") == 0 &&
	            std::fabs(to_motion_flux_cm) > MBAL_ITERATIVE_TOLERANCE) {
	          printf("Suppressing %.17lf cm of TO-above-surface virtual flux because "
	                 "a surface to_bottom boundary snap already routed the realized "
	                 "storage change this substep.\n",
	                 to_motion_flux_cm);
	        }
	      }
	      else {
	        bottom_boundary_flux_above_surface_WFs_cm += to_motion_flux_cm;
	        bottom_boundary_flux_cm += to_motion_flux_cm;
	      }
	    }

    if (TO_lower_boundary_depth_cm > column_depth + LOWER_BOUNDARY_FINAL_TOL_CM) {
      bool final_layer_TO_crossed_fixed_bottom = false;
      for (struct wetting_front *front = *head; front != NULL; front = front->next) {
        if (front->is_WF_GW && !front->to_bottom &&
            front->layer_num == num_layers &&
            std::isfinite(front->depth_cm) &&
            front->depth_cm > column_depth + LOWER_BOUNDARY_FINAL_TOL_CM) {
          final_layer_TO_crossed_fixed_bottom = true;
          break;
        }
      }
      if (final_layer_TO_crossed_fixed_bottom) {
        // listSortFrontsByDepth(*head); //please keep this in to indicate it was once needed, commented out because listSortFrontsByDepth is really not the preferred solution to a problem unless it is literally the only option
      }
    }
	    ...
	    if (listLength_surface(*head) > 0) {
	      const double requested_surface_TO_flux_cm = bottom_boundary_flux_above_surface_WFs_cm;
	      const double flux_before_surface_TO_extraction_cm = bottom_boundary_flux_cm;
	      const double AET_before_surface_TO_extraction_cm = *AET_demand_cm;
	      const double mass_before_surface_TO_extraction_cm =
	        lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
	      ...
	      lgar_global_theta_update(bottom_boundary_flux_above_surface_WFs_cm,
	                               ...
	                               mobile_groundwater_level);
	      const double realized_surface_TO_flux_cm = mass_before_surface_TO_extraction_cm -
	        lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
	      if (requested_surface_TO_flux_cm > MBAL_ITERATIVE_TOLERANCE &&
	          realized_surface_TO_flux_cm >= -MBAL_ITERATIVE_TOLERANCE &&
	          requested_surface_TO_flux_cm - realized_surface_TO_flux_cm >
	            MBAL_ITERATIVE_TOLERANCE) {
	        // A residual-state profile cannot supply virtual TO drainage beyond its
	        // realized storage loss; leave that potential drainage unbooked.
	        bottom_boundary_flux_cm = flux_before_surface_TO_extraction_cm -
	          requested_surface_TO_flux_cm + fmax(0.0, realized_surface_TO_flux_cm);
	        bottom_boundary_flux_above_surface_WFs_cm = 0.0;
	        *AET_demand_cm = AET_before_surface_TO_extraction_cm;
	      }
    ...
    int correction_type =
      lgarto_correction_type(num_layers, cum_layer_thickness_cm, head,
                             surface_lower_boundary_depth_cm, mobile_groundwater_level);
      ...
      if (correction_type == 1) {
        ...
        const double target_mass_after_fluxes =
          (old_mass + precip_mass_to_add) -
           ...
           bottom_boundary_flux_cm + lateral_flow_for_mass_balance_cm());
	        const double mass_balance_flux_correction_cm =
	          lgarto_TO_WFs_merge_via_depth(target_mass_after_fluxes, column_depth, cum_layer_thickness_cm, head,
	                                        soil_type, soil_properties, mobile_groundwater_level,
	                                        surface_lower_boundary_depth_cm);
        lgarto_assert_gw_flux_mass_balance_correction_within_debug_threshold(
          mass_balance_flux_correction_cm,
  ...
  // Refresh layer-specific conductivity without discarding a valid non-unique TO psi.
  const std::vector<lgar_preserved_boundary_psi_state>
    preserved_final_boundary_psi_states =
      lgar_collect_lossless_boundary_psi_states(
        *head, soil_type, soil_properties);
  for (current = *head; current != NULL; current = current->next) {
    lgar_refresh_front_state_from_theta(current, soil_type, frozen_factor, soil_properties);
    (void) lgar_restore_lossless_boundary_psi(current, preserved_final_boundary_psi_states);
  }

  const double mass_before_final_boundary_snaps_cm =
    lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
  const std::vector<struct wetting_front *> final_boundary_upper_fronts =
    lgar_collect_boundary_upper_fronts(*head);
  for (auto boundary_it = final_boundary_upper_fronts.rbegin();
       boundary_it != final_boundary_upper_fronts.rend();
       ++boundary_it) {
    current = *boundary_it;
    struct wetting_front *next = current->next;
    if (!lgar_boundary_psi_snap_candidate(current, next, soil_type, soil_properties)) {
      continue;
    }

    lgar_snap_boundary_psi_to_lower_or_abort(mass_before_final_boundary_snaps_cm,
                                             cum_layer_thickness_cm,
                                             soil_type, soil_properties,
                                             *head, current, next);
  ...
  lgarto_cap_zero_depth_TO_psi(num_layers, soil_type, frozen_factor, soil_properties, *head);
		  ...
		  bottom_boundary_flux_cm +=
		    lgarto_clip_final_layer_GW_overshoot_to_vadose_boundary(cum_layer_thickness_cm,
		                                                            num_layers,
		                                                            head,
		                                                            TO_lower_boundary_depth_cm);

	  // Final storage-neutral cleanup for layer-boundary surface bridges exposed
	  // after the main correction loop has already converged.
	  lgarto_convert_surface_fronts_drier_than_TO_below(head, "final TO cleanup");
				      ...
				      struct soil_properties_ *soil_properties)
{
  ...
  for (int wf=1; wf != listLength(*head); wf++) {
    ...
    /**********************************************************/
	    ...
	    // Mixed surface/TO contacts must return to the correction loop for the
	    // mass-aware surface/TO handoff instead of using this surface merge.
	    if ( (current->depth_cm > next->depth_cm) && (current->layer_num == next->layer_num) &&
	         !current->is_WF_GW && !next->is_WF_GW && !next->to_bottom) {
        if (lgarto_surface_front_overtook_surface_front_above_TO_chain(current)) {
	      ...
	      const double merge_denominator = current->theta - next_to_next->theta;
	      double current_mass_this_layer = current->depth_cm * (current->theta - next->theta) + next->depth_cm*(next->theta - next_to_next->theta);
	      current->depth_cm = current_mass_this_layer / merge_denominator;
      ...
      struct lgarto_merge_saved_to_bottom_state
      {
        struct wetting_front *front;
        bool is_WF_GW;
        double theta;
      ...
      std::vector<lgarto_merge_saved_to_bottom_state> to_bottom_states;
      for (struct wetting_front *snap = *head; snap != NULL; snap = snap->next) {
        if (snap->to_bottom) {
          to_bottom_states.push_back({
            snap,
            snap->is_WF_GW,
            snap->theta,
            snap->psi_cm,
      ...
      for (size_t i = 0; i < to_bottom_states.size(); i++) {
        to_bottom_states[i].front->is_WF_GW = to_bottom_states[i].is_WF_GW;
        to_bottom_states[i].front->theta = to_bottom_states[i].theta;
        to_bottom_states[i].front->psi_cm = to_bottom_states[i].psi_cm;
        to_bottom_states[i].front->K_cm_per_h = to_bottom_states[i].K_cm_per_h;
      }
						     ...
						     struct soil_properties_ *soil_properties)
{
  ...
  for (int wf=1; wf != listLength(*head); wf++) {
	...
	&& (layer_num!=num_layers) ) {
      ...
      const double remap_denominator = theta_new - next_to_next->theta;
      double mbal_Z_correction = mbal_correction / remap_denominator; // this is the new wetting front depth
      if (std::isinf(mbal_Z_correction)) {
        // Keep a zero-contrast remap finite so the iterative cleanup can merge or truncate it.
        mbal_Z_correction = cum_layer_thickness_cm[layer_num + 1] / 1.E3;
      }
								 ...
								 bool mobile_groundwater_level)
{
  if (*bottom_boundary_flux_above_surface_WFs_cm == 0.0) {
    return bottom_boundary_flux_cm;
	  ...
	  const int wf_free_drainage_demand =
	    mobile_groundwater_level
	      ? wetting_front_free_drainage_mobile_groundwater(*head)
	      : wetting_front_free_drainage(*head);
  ...
  const int extraction_soil_num = soil_type[current->layer_num];
  const soil_properties_ &extraction_soil = soil_properties[extraction_soil_num];
  // Preserve an already drier state, but do not let this flux extraction create psi above 1e7 cm.
  const double theta_floor = fmin(original_theta,
    calc_theta_from_h(PSI_UPPER_LIM, extraction_soil.vg_alpha_per_cm, extraction_soil.vg_m,
                      extraction_soil.vg_n, extraction_soil.theta_e, extraction_soil.theta_r));
  struct wetting_front *next = current->next;
  ...
  if (listLength_surface(*head) > 0) {
    if (current->layer_num == 1) {
      if (current->depth_cm > 0.0) {
        ...
        current->theta = fmax(theta_floor,
        ...
        current->psi_cm = calc_h_from_Se(Se, vg_a_k, vg_m_k, vg_n_k);
          ...
          lgarto_assert_gw_flux_mass_balance_correction_within_debug_threshold(
            ...
            "top-layer extraction psi-cap residual adjustment",
            *head);
          bottom_boundary_flux_cm -= requested_flux_cm;
	        ...
	        // Keep this residual as unresolved TO demand; zero means the surface
	        // storage actually supplied the requested flux.
	        *bottom_boundary_flux_above_surface_WFs_cm -=
	          (mass_at_start_of_extraction - lgar_calc_mass_bal(cum_layer_thickness_cm, *head));
	      }
	    }
      ...
      if (*bottom_boundary_flux_above_surface_WFs_cm != 0.0) {
        theta_new = lgar_theta_mass_balance(layer_num, soil_num, current->psi_cm, new_mass, prior_mass, 0.0,
                                            ...
                                            soil_properties, true, fmax(PSI_UPPER_LIM, current->psi_cm),
                                            &solved_psi_cm);
      }
      ...
      if (current->theta == theta_floor) {
        ...
        lgarto_assert_gw_flux_mass_balance_correction_within_debug_threshold(
          actual_extracted_flux_cm - requested_flux_cm,
          "lgarto_extract_TO_GW_flux_from_surface_WFs",
          "deeper-layer extraction psi-cap residual adjustment",
          *head);
        bottom_boundary_flux_cm -= requested_flux_cm;
        bottom_boundary_flux_cm += actual_extracted_flux_cm;
        *bottom_boundary_flux_above_surface_WFs_cm = actual_extracted_flux_cm;
      ...
      const double Se = calc_Se_from_theta(current->theta, theta_e_k, theta_r_k);
      if (*bottom_boundary_flux_above_surface_WFs_cm != 0.0) {
        // Retain the mass solver's shared psi when saturated theta has a non-unique inverse.
        current->psi_cm = std::isfinite(solved_psi_cm) ? fmax(psi_cm, solved_psi_cm) :
                          calc_h_from_Se(Se, vg_a_k, vg_m_k, vg_n_k);
        current->theta = calc_theta_from_h(current->psi_cm, vg_a_k, vg_m_k, vg_n_k,
                                            theta_e_k, theta_r_k);
        *bottom_boundary_flux_above_surface_WFs_cm -=
          (mass_at_start_of_extraction - lgar_calc_mass_bal(cum_layer_thickness_cm, *head));
      }
  ...
  // Do not reorder here: boundary correction needs upward-crossed TO fronts
  // beside their to_bottom bridge so it can translate the compound crossing.
					   ...
					   struct wetting_front **head)
{
  ...
  for (int wf = 1; wf != listLength(*head); wf++) {
    ...
    if (!lgar_restore_lossless_boundary_psi(
      ...
      if (!lgar_TO_psi_matches_theta(current, soil_type, soil_properties)) {
  ...
  const double mass_before_boundary_snaps_cm =
    (cum_layer_thickness_cm != NULL) ? lgar_calc_mass_bal(cum_layer_thickness_cm, *head) : NAN;

  const std::vector<struct wetting_front *> boundary_upper_fronts =
    lgar_collect_boundary_upper_fronts(*head);
  for (auto boundary_it = boundary_upper_fronts.rbegin();
       boundary_it != boundary_upper_fronts.rend();
       ++boundary_it) {
    current = *boundary_it;
    struct wetting_front *next = current->next;
    if (!lgar_boundary_psi_snap_candidate(current, next, soil_type, soil_properties)) {
...
{
  const double depth_tol_cm = fmax(1.0e-8, 10.0 * TRUNCATION_DEPTH);

  for (struct wetting_front *current = head; current != NULL && current->next != NULL;
       current = current->next) {
    struct wetting_front *next = current->next;
    if (next->depth_cm + depth_tol_cm >= current->depth_cm) {
      continue;
    }

    fprintf(stderr,
            "Error: wetting-front list ended a substep out of depth order.\n"
            "  upper/list-earlier front_num=%d layer=%d depth_cm=%.16g written_depth_mm=%.16g theta=%.16g psi_cm=%.16g dzdt_cm_per_h=%.16g is_WF_GW=%d to_bottom=%d\n"
            "  lower/list-later   front_num=%d layer=%d depth_cm=%.16g written_depth_mm=%.16g theta=%.16g psi_cm=%.16g dzdt_cm_per_h=%.16g is_WF_GW=%d to_bottom=%d\n"
            "  depth_reversal_cm=%.16g > %.16g\n"
            "  Wetting front list follows:\n",
            current->front_num,
            current->layer_num,
            current->depth_cm,
...
static bool lgarto_coarsen_negligible_same_layer_TO_reversal(
  ...
  double *frozen_factor)
{
  ...
  for (struct wetting_front *current = *head;
       ...
       previous = current, current = current->next) {
    struct wetting_front *next = current->next;
    if (previous == NULL || !previous->is_WF_GW || previous->to_bottom ||
        ...
        current->theta + 1.0e-12 < next->theta ||
        lgar_is_mobile_groundwater_storage_pair_member(
          current, *head, cum_layer_thickness_cm) ||
        lgar_is_mobile_groundwater_storage_pair_member(
          next, *head, cum_layer_thickness_cm)) {
      continue;
    }

    const double inverted_storage_cm =
      (current->depth_cm - previous->depth_cm) *
      fmax(0.0, current->theta - next->theta);
    if (inverted_storage_cm > FINITE_SAME_LAYER_TO_DELETE_MASS_TOL_CM) {
      continue;
    }

    struct wetting_front *trial_head = listCopy(*head, NULL);
    struct wetting_front *trial_previous =
      listFindFront(previous->front_num, trial_head, NULL);
    struct wetting_front *trial_current =
      listFindFront(current->front_num, trial_head, NULL);
    struct wetting_front *trial_next =
      listFindFront(next->front_num, trial_head, NULL);
    if (trial_previous == NULL || trial_current == NULL || trial_next == NULL) {
      listDelete(trial_head);
      return false;
    }

    const int deleted_front_num = trial_current->front_num;
    (void) listDeleteFront(
      deleted_front_num, &trial_head, soil_type, soil_properties);
    const double merged_thickness_cm =
      trial_next->depth_cm - trial_previous->depth_cm;
    const double residual_cm =
      target_mass_cm - lgar_calc_mass_bal(cum_layer_thickness_cm, trial_head);
    if (merged_thickness_cm <= 0.0 || !std::isfinite(residual_cm) ||
        fabs(residual_cm) > FINITE_SAME_LAYER_TO_DELETE_MASS_TOL_CM) {
      listDelete(trial_head);
      continue;
    }

    const int soil_num = soil_type[trial_next->layer_num];
    const struct soil_properties_ &soil = soil_properties[soil_num];
    const double repaired_theta =
      trial_next->theta + residual_cm / merged_thickness_cm;
    if (!std::isfinite(repaired_theta) ||
        repaired_theta < soil.theta_r - 1.0e-12 ||
        repaired_theta > soil.theta_e + 1.0e-12) {
      listDelete(trial_head);
      continue;
    }
    trial_next->theta = fmax(soil.theta_r, fmin(soil.theta_e, repaired_theta));
    const double Se = calc_Se_from_theta(
      trial_next->theta, soil.theta_e, soil.theta_r);
    trial_next->psi_cm = calc_h_from_Se(
      Se, soil.vg_alpha_per_cm, soil.vg_m, soil.vg_n);
    trial_next->K_cm_per_h = calc_K_from_Se(
      Se, soil.Ksat_cm_per_h *
            (frozen_factor != NULL ? frozen_factor[trial_next->layer_num] : 1.0),
      soil.vg_m);
    trial_next->dzdt_cm_per_h = 0.0;

    struct wetting_front *trial_lower = trial_next->next;
    const bool upper_ordered =
      trial_next->psi_cm <= trial_previous->psi_cm +
        lgar_psi_assertion_tolerance_cm(
          trial_previous->psi_cm, trial_next->psi_cm);
    const bool lower_ordered =
      trial_lower == NULL || !trial_lower->is_WF_GW ||
      trial_lower->layer_num != trial_next->layer_num ||
      trial_lower->psi_cm <= trial_next->psi_cm +
        lgar_psi_assertion_tolerance_cm(
          trial_next->psi_cm, trial_lower->psi_cm);
    const double final_residual_cm =
      target_mass_cm - lgar_calc_mass_bal(cum_layer_thickness_cm, trial_head);
    const double target_tolerance_cm =
      1.0e-12 * fmax(1.0, fabs(target_mass_cm));
    if (!upper_ordered || !lower_ordered ||
        !std::isfinite(final_residual_cm) ||
        fabs(final_residual_cm) > target_tolerance_cm) {
      listDelete(trial_head);
      continue;
    }

    if (verbosity.compare("high") == 0) {
      printf("Coarsened negligible same-layer TO/GW reversal at front %d "
             "(inverted storage %.17g cm, final residual %.17g cm).\n",
             deleted_front_num, inverted_storage_cm, final_residual_cm);
    }
    listDelete(*head);
    *head = trial_head;
    return true;
  ...
  double *frozen_factor)
{
  ...
  // Near saturation, a visible psi reversal can carry negligible storage.
  // Coarsen only that numerical-resolution case before boundary-chain solves.
  while (lgarto_coarsen_negligible_same_layer_TO_reversal(
           target_mass_cm, cum_layer_thickness_cm, soil_type, head,
           soil_properties, frozen_factor)) {}

  bool reconciled = false;
  const std::vector<struct wetting_front *> boundaries =
    lgar_collect_boundary_upper_fronts(*head);
  std::vector<std::vector<struct wetting_front *> > boundary_chains;
  for (auto boundary = boundaries.rbegin(); boundary != boundaries.rend(); ++boundary) {
    ...
    if (lgarto_find_mobile_groundwater_storage_pair(
      ...
      // Include the vadose pair member so a boundary snap cannot leave a
      // wetter TO front above a drier groundwater contact.
      chain.push_back(gw_vadose);
    }
    boundary_chains.push_back(chain);
  ...
  if (std::fabs(lgar_calc_mass_bal(cum_layer_thickness_cm, *head) -
                target_mass_cm) > MBAL_ITERATIVE_TOLERANCE) {
    return false;
  }

  // Neighboring layer-boundary solves can each conserve the complete profile
  // yet select incompatible psi values in their shared intervening layer.
  // Pool adjacent chains and solve one common psi before rejecting the trial.
  for (size_t solve = 0; solve <= boundaries.size(); solve++) {
    struct wetting_front *shallower = NULL;
    struct wetting_front *deeper = NULL;
    for (struct wetting_front *current = *head;
    ...
    const auto contains = [](const std::vector<struct wetting_front *> &fronts,
                             const struct wetting_front *front) {
      return std::find(fronts.begin(), fronts.end(), front) != fronts.end();
    };
    bool shallower_in_chain = false;
    bool deeper_in_chain = false;
    for (const auto &chain : boundary_chains) {
      shallower_in_chain = shallower_in_chain || contains(chain, shallower);
      deeper_in_chain = deeper_in_chain || contains(chain, deeper);
    }
    if (!shallower_in_chain || !deeper_in_chain) {
      return false;
    }

    std::vector<struct wetting_front *> pooled_members = {shallower, deeper};
    // Following newly added members makes this a transitive merge if an
    // earlier pooled solve already joined more than two boundary chains.
    for (size_t member_index = 0; member_index < pooled_members.size(); member_index++) {
      for (const auto &chain : boundary_chains) {
        if (!contains(chain, pooled_members[member_index])) continue;
        for (struct wetting_front *front : chain) {
          if (!contains(pooled_members, front)) {
            pooled_members.push_back(front);
          }
        }
      }
    }

    std::vector<struct wetting_front *> pooled_chain;
    for (struct wetting_front *front = *head; front != NULL; front = front->next) {
      if (contains(pooled_members, front)) {
        pooled_chain.push_back(front);
      }
    }
    if (!lgarto_mass_aware_to_bottom_snap(
          target_mass_cm, cum_layer_thickness_cm, soil_type,
          soil_properties, head, pooled_chain)) {
      return false;
    }
    boundary_chains.push_back(pooled_chain);
  }
  return false;
}
                                                                     ...
                                                                     struct wetting_front *head)
{
  ...
  for (struct wetting_front *current = head; current != NULL; current = current->next) {
    ...
    surface_count++;
    if (current->to_bottom) {
      surface_to_bottom_count++;
    }
  }

  if (surface_count == 0 || surface_count == num_layers ||
      surface_count != surface_to_bottom_count) {
    return;
  }

  fprintf(stderr,
          "Error: TO substep ended with only a partial to_bottom surface scaffold.\n"
          "  surface_fronts=%d surface_to_bottom_fronts=%d num_layers=%d\n"
          "  Rule: if any active surface front remains in TO mode, at least one surface front should be non-to_bottom unless every layer is represented.\n"
          "  Wetting front list follows:\n",
          surface_count,
          surface_to_bottom_count,
          num_layers);
...
extern void lgar_assert_boundary_psi_continuity(struct wetting_front *head)
{
  for (struct wetting_front *current = head; current != NULL && current->next != NULL; current = current->next) {
    ...
    // Keep the near-saturated upper-boundary roundoff exemption, but catch
    // a materially dry upper boundary sitting above a psi=0 lower front.
    if (psi_mismatch_cm > psi_tol_cm && current->psi_cm > 0.1) {
      std::cerr << "Error: psi mismatch across soil layer boundary exceeds tolerance.\n"
  ...
  for (int wf = listLength(*head); wf != 0; wf--) {
    ...
    if (next != NULL && current->depth_cm > cum_layer_thickness_cm[layer_num] && next->to_bottom &&
      ...
      if (remap_hits_lower_layer_bottom) {
        ...
        depth_new = bounded_lower_depth_cm;
      }
...
extern double lgarto_TO_WFs_merge_via_depth(double target_mass, double column_depth,
						    ...
						    int *soil_type, struct soil_properties_ *soil_properties,
						    bool mobile_groundwater_level,
						    double expected_mobile_groundwater_depth_cm)
{
  bool merged = false;
  struct wetting_front *current = *head;
  ...
  while (current != NULL && next != NULL) {
      if (((current->depth_cm > next->depth_cm) &&
         current->is_WF_GW && next->is_WF_GW &&
         (next->to_bottom == FALSE) && (current->to_bottom == FALSE) &&
         (current->theta < next->theta)) ||
	        lgarto_same_layer_TO_GW_near_contact_merge_ready(current, next) ||
	        lgarto_reversed_same_layer_TO_GW_duplicate_merge_ready(
	          current, next, *head, cum_layer_thickness_cm, mobile_groundwater_level,
	          expected_mobile_groundwater_depth_cm)) {
      if (verbosity.compare("high") == 0) {
...
static void lgarto_renumber_fronts_by_list_order(struct wetting_front *head)
{
  int front_num = 1;
  for (struct wetting_front *front = head; front != NULL; front = front->next) {
    front->front_num = front_num++;
  }
}

static bool lgarto_reinsert_existing_front_by_depth(struct wetting_front **head,
                                                    struct wetting_front *front_to_move)
{
  if (head == NULL || *head == NULL || front_to_move == NULL) {
    return false;
  }

  struct wetting_front *previous = NULL;
  struct wetting_front *current = *head;
  while (current != NULL && current != front_to_move) {
    previous = current;
    current = current->next;
  }
  ...
  struct wetting_front *insert_after = NULL;
  struct wetting_front *scan = *head;
  while (scan != NULL &&
         scan->depth_cm <= front_to_move->depth_cm + DEPTH_AVOIDS_SAME_WF_DEPTH) {
    insert_after = scan;
    scan = scan->next;
  }

  if (insert_after == NULL) {
    front_to_move->next = *head;
    *head = front_to_move;
  }
  else {
    front_to_move->next = insert_after->next;
    insert_after->next = front_to_move;
  }

  lgarto_renumber_fronts_by_list_order(*head);
  return true;
}
  ...
  struct wetting_front **head, const char *context)
{
  ...
  for (struct wetting_front *current = *head; current != NULL; current = current->next) {
    ...
    lgarto_update_front_from_shared_psi(current, old_psi_cm, num_layers, soil_type,
                                        frozen_factor, soil_properties);
    const int front_num_before_reinsert = current->front_num;
    const bool reinserted =
      lgarto_reinsert_existing_front_by_depth(head, current);
    for (int scaffold_layer = current->layer_num; scaffold_layer < old_layer; scaffold_layer++) {
      (void) lgarto_insert_missing_to_bottom_scaffold(scaffold_layer, cum_layer_thickness_cm,
                                                      head, current);
    }
    repaired_any = true;
  ...
  while (current != NULL && next != NULL) {
    if ((current->is_WF_GW == 0) && (next->is_WF_GW == 0)) {
      ...
      if (dry_over_wet && current->layer_num == next->layer_num) {
        ...
        if (current->to_bottom && !next->to_bottom &&
            std::fabs(current->depth_cm - next->depth_cm) <= colocated_boundary_tol_cm) {
          const int preserved_front_num = current->front_num;
          const bool preserved_is_WF_GW = current->is_WF_GW;
          const double preserved_theta = current->theta;
          const double preserved_psi_cm = current->psi_cm;
          const double preserved_K_cm_per_h = current->K_cm_per_h;

          (void) listDeleteFront(next->front_num, head, soil_type, soil_properties);
          current = listFindFront(preserved_front_num, *head, NULL);
          if (current == NULL) {
            break;
          }

          current->to_bottom = TRUE;
          current->is_WF_GW = preserved_is_WF_GW;
          current->theta = preserved_theta;
          current->psi_cm = preserved_psi_cm;
          current->K_cm_per_h = preserved_K_cm_per_h;
          current->dzdt_cm_per_h = 0.0;
...
static struct wetting_front *lgarto_insert_front_after(struct wetting_front *previous,
						       double depth_cm,
						       double theta,
						       int layer_num,
						       bool to_bottom,
						       struct wetting_front **head)
{
  ...
  if (previous == NULL) {
    listInsertFirst(depth_cm, theta, 1, layer_num, to_bottom, head);
    return *head;
  }

  struct wetting_front *inserted =
    (struct wetting_front*) malloc(sizeof(struct wetting_front));
  if (inserted == NULL) {
    return NULL;
  }

  inserted->depth_cm = depth_cm;
  inserted->theta = theta;
  inserted->front_num = previous->front_num + 1;
  inserted->layer_num = layer_num;
  inserted->to_bottom = to_bottom;
  inserted->dzdt_cm_per_h = 0.0;
  inserted->is_WF_GW = false;
  inserted->psi_cm = previous->psi_cm;
  inserted->K_cm_per_h = previous->K_cm_per_h;
  inserted->next = previous->next;
  previous->next = inserted;
  for (struct wetting_front *renumber = inserted->next;
       renumber != NULL; renumber = renumber->next) {
    renumber->front_num++;
  }
...
static bool lgarto_front_is_near_saturated(struct wetting_front *front,
					   int num_layers,
					   int *soil_type,
					   struct soil_properties_ *soil_properties)
{
...
static void lgar_restore_upstream_to_bottom_chain_psi(struct wetting_front *surviving_front,
						      ...
						      struct soil_properties_ *soil_properties,
						      struct wetting_front *head);
static void lgar_delete_redundant_colocated_psi0_support_stacks(
  struct wetting_front **head,
  int *soil_type,
  struct soil_properties_ *soil_properties,
  double *cum_layer_thickness_cm);

static double lgarto_canonical_mobile_groundwater_depth_cm(double groundwater_depth_cm,
							   int num_layers,
							   double *cum_layer_thickness_cm)
{
  if (num_layers <= 0 || cum_layer_thickness_cm == NULL ||
      !std::isfinite(groundwater_depth_cm)) {
    return groundwater_depth_cm;
  }
  ...
  groundwater_depth_cm = fmin(groundwater_depth_cm, fixed_column_depth_cm);

  /*
   * Please do not delete this commented-out block.  We are intentionally
   * avoiding mobile-GW boundary snapping unless broad stability evidence shows
   * we need it again.  If it is reintroduced, keep the conservative
   * support-tolerance-scaled form below rather than the old 1e-3 cm floor, so
   * small but resolvable saturated storage near a layer boundary is preserved.
   *
   * const double support_tol_cm =
   *   fmax(MOBILE_GROUNDWATER_SUBMERGENCE_TOL_CM,
   *        1.0e-10 * fmax(1.0, fixed_column_depth_cm));
   * const double boundary_snap_tol_cm = 10.0 * support_tol_cm;
   * for (int layer = 1; layer <= num_layers; layer++) {
   *   if (fabs(groundwater_depth_cm - cum_layer_thickness_cm[layer]) <=
   *       boundary_snap_tol_cm) {
   *     if (layer < num_layers) {
   *       groundwater_depth_cm =
   *         fmax(cum_layer_thickness_cm[layer - 1] + support_tol_cm,
   *              cum_layer_thickness_cm[layer] - support_tol_cm);
   *     }
   *     else if (fabs(groundwater_depth_cm - cum_layer_thickness_cm[layer]) <=
   *              support_tol_cm) {
   *       groundwater_depth_cm = cum_layer_thickness_cm[layer];
   *     }
   *     break;
   *   }
   * }
...
extern double lgarto_maintain_mobile_groundwater_support(double groundwater_depth_cm,
							 int num_layers,
							 double *cum_layer_thickness_cm,
							 int *soil_type,
							 double *frozen_factor,
							 ...
							 struct soil_properties_ *soil_properties)
{
  ...
  const double fixed_column_depth_cm = cum_layer_thickness_cm[num_layers];
  if (!std::isfinite(fixed_column_depth_cm) || fixed_column_depth_cm <= 0.0 ||
      groundwater_depth_cm > fixed_column_depth_cm + MOBILE_GROUNDWATER_SUBMERGENCE_TOL_CM) {
    return 0.0;
  }
  groundwater_depth_cm =
    lgarto_canonical_mobile_groundwater_depth_cm(groundwater_depth_cm,
                                                 num_layers,
                                                 cum_layer_thickness_cm);
  ...
  struct wetting_front *support = NULL;
  struct wetting_front *closest_existing_support = NULL;
  double closest_existing_distance_cm = HUGE_VAL;
  for (struct wetting_front *current = *head; current != NULL; current = current->next) {
    const bool is_upper_member_of_existing_pair =
      current->next != NULL &&
      current->next->is_WF_GW &&
      !current->next->to_bottom &&
      current->layer_num == current->next->layer_num &&
      fabs(current->depth_cm - current->next->depth_cm) <= support_tol_cm &&
      current->next->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM;
    if (!current->is_WF_GW || current->to_bottom ||
        is_upper_member_of_existing_pair ||
        current->depth_cm <= ZERO_DEPTH_TO_DELETE_DEPTH_TOL_CM ||
        current->psi_cm > BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM) {
      continue;
    }

    const double distance_cm = fabs(current->depth_cm - groundwater_depth_cm);
    if (current->layer_num == groundwater_layer &&
        distance_cm <= support_tol_cm) {
      support = current;
      break;
    }

    if (distance_cm < closest_existing_distance_cm) {
      closest_existing_support = current;
      closest_existing_distance_cm = distance_cm;
    }
  }

  struct wetting_front *vadose_side_front = NULL;
  bool has_saved_vadose_side_state = false;
  bool saved_vadose_side_is_WF_GW = true;
  double saved_vadose_side_theta = NAN;
  double saved_vadose_side_psi_cm = NAN;
  double saved_vadose_side_K_cm_per_h = NAN;
  double saved_vadose_side_dzdt_cm_per_h = 0.0;
  if (support == NULL && closest_existing_support != NULL) {
    struct wetting_front *previous_to_existing_support = NULL;
    for (struct wetting_front *scan = *head;
         scan != NULL && scan != closest_existing_support;
         scan = scan->next) {
    ...
    if (previous_to_existing_support != NULL &&
        ...
        fabs(previous_to_existing_support->depth_cm -
             closest_existing_support->depth_cm) <= support_tol_cm &&
        closest_existing_distance_cm <= support_tol_cm) {
      vadose_side_front = previous_to_existing_support;
      support = closest_existing_support;
    }
  }

  if (support == NULL && closest_existing_support != NULL) {
    const int old_support_layer = closest_existing_support->layer_num;
    struct lgarto_support_saved_to_bottom_state {
      struct wetting_front *front;
      bool is_WF_GW;
      double theta;
      double psi_cm;
    ...
    std::vector<lgarto_support_saved_to_bottom_state> to_bottom_states;
    for (struct wetting_front *scan = *head; scan != NULL; scan = scan->next) {
      if (scan->to_bottom && scan != closest_existing_support) {
        to_bottom_states.push_back({
          scan,
          scan->is_WF_GW,
          scan->theta,
    ...
    struct wetting_front *paired_vadose_side_front = NULL;
    for (struct wetting_front *scan = *head;
         scan != NULL && scan != closest_existing_support;
         scan = scan->next) {
      paired_vadose_side_front = scan;
    }
    if (paired_vadose_side_front != NULL &&
        (!paired_vadose_side_front->is_WF_GW ||
         paired_vadose_side_front->to_bottom ||
         paired_vadose_side_front->layer_num != closest_existing_support->layer_num ||
         fabs(paired_vadose_side_front->depth_cm -
              closest_existing_support->depth_cm) > support_tol_cm)) {
      paired_vadose_side_front = NULL;
    }
    if (paired_vadose_side_front != NULL &&
        lgarto_front_is_near_saturated(paired_vadose_side_front, num_layers,
                                       soil_type, soil_properties)) {
      struct wetting_front *boundary_vadose_side_source = NULL;
      for (struct wetting_front *scan = *head;
           scan != NULL && scan != paired_vadose_side_front;
           scan = scan->next) {
        if (scan->is_WF_GW &&
            scan->to_bottom &&
            scan->layer_num + 1 == paired_vadose_side_front->layer_num &&
            std::isfinite(scan->psi_cm)) {
          boundary_vadose_side_source = scan;
        }
      }
      if (boundary_vadose_side_source != NULL &&
          !lgarto_front_is_near_saturated(boundary_vadose_side_source,
                                          num_layers, soil_type,
                                          soil_properties)) {
        paired_vadose_side_front = NULL;
      }
    }
    if (paired_vadose_side_front != NULL) {
      has_saved_vadose_side_state = true;
      saved_vadose_side_is_WF_GW = paired_vadose_side_front->is_WF_GW;
      saved_vadose_side_theta = paired_vadose_side_front->theta;
      saved_vadose_side_psi_cm = paired_vadose_side_front->psi_cm;
      saved_vadose_side_K_cm_per_h = paired_vadose_side_front->K_cm_per_h;
      saved_vadose_side_dzdt_cm_per_h = paired_vadose_side_front->dzdt_cm_per_h;
    }

    (void) listDeleteFront(closest_existing_support->front_num, head,
                           soil_type, soil_properties);
    if (paired_vadose_side_front != NULL) {
      (void) listDeleteFront(paired_vadose_side_front->front_num, head,
                             soil_type, soil_properties);
    }
    for (size_t i = 0; i < to_bottom_states.size(); i++) {
      to_bottom_states[i].front->is_WF_GW = to_bottom_states[i].is_WF_GW;
      ...
      to_bottom_states[i].front->psi_cm = to_bottom_states[i].psi_cm;
      to_bottom_states[i].front->K_cm_per_h = to_bottom_states[i].K_cm_per_h;
      to_bottom_states[i].front->dzdt_cm_per_h = to_bottom_states[i].dzdt_cm_per_h;
    }
    if (old_support_layer >= 1 && old_support_layer <= num_layers &&
        groundwater_depth_cm > cum_layer_thickness_cm[old_support_layer] + support_tol_cm &&
        has_saved_vadose_side_state &&
        saved_vadose_side_psi_cm > BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM) {
      for (struct wetting_front *scan = *head; scan != NULL; scan = scan->next) {
        if (scan->to_bottom && scan->layer_num == old_support_layer) {
          // Receding below this layer drains the old saturated support; keep the
          // deleted vadose-side psi on the layer boundary instead of leaving psi=0.
          scan->is_WF_GW = TRUE;
          scan->dzdt_cm_per_h = 0.0;
          lgarto_set_front_hydraulic_state_from_psi(
            scan, saved_vadose_side_psi_cm, soil_type, frozen_factor,
            soil_properties);
          break;
        }
      }
  ...
  if (support == NULL) {
    struct wetting_front *previous = NULL;
    for (struct wetting_front *current = *head; current != NULL; current = current->next) {
      if (current->depth_cm > groundwater_depth_cm + support_tol_cm) {
        break;
      }
      if (current->to_bottom &&
          current->depth_cm >= groundwater_depth_cm &&
          fabs(current->depth_cm - groundwater_depth_cm) <= support_tol_cm) {
        // A shallower boundary must precede a support placed just below it.
        break;
      }
      previous = current;
    }

    const int soil_num = soil_type[groundwater_layer];
    const double theta_e = soil_properties[soil_num].theta_e;
    if (previous == NULL) {
      listInsertFirst(groundwater_depth_cm, theta_e, 1, groundwater_layer,
                      FALSE, head);
      support = *head;
    }
    else {
      support = lgarto_insert_front_after(previous, groundwater_depth_cm,
                                          theta_e, groundwater_layer,
                                          FALSE, head);
    }
  }
  ...
  struct wetting_front *previous_to_support = NULL;
  for (struct wetting_front *scan = *head; scan != NULL && scan != support; scan = scan->next) {
    previous_to_support = scan;
  }
  if (vadose_side_front == NULL &&
    ...
    vadose_side_front = previous_to_support;
  }

  if (vadose_side_front == NULL) {
    struct wetting_front *boundary_vadose_side_source = NULL;
    for (struct wetting_front *scan = *head; scan != NULL && scan != support;
         scan = scan->next) {
      if (scan->is_WF_GW &&
          scan->to_bottom &&
          scan->layer_num + 1 == groundwater_layer &&
          std::isfinite(scan->psi_cm)) {
        boundary_vadose_side_source = scan;
      }
    }

    struct wetting_front *local_state_source = NULL;
    bool local_state_source_is_saturated = false;
    if (previous_to_support != NULL &&
        previous_to_support->layer_num == groundwater_layer &&
        !previous_to_support->to_bottom &&
        previous_to_support->depth_cm <= groundwater_depth_cm + support_tol_cm) {
      const int local_soil_num = soil_type[groundwater_layer];
      const double local_theta_e = soil_properties[local_soil_num].theta_e;
      local_state_source_is_saturated =
        previous_to_support->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM ||
        previous_to_support->theta >= local_theta_e - 1.0e-10;
      if (!local_state_source_is_saturated) {
        local_state_source = previous_to_support;
      }
    }
    const bool use_saved_vadose_side_state =
      has_saved_vadose_side_state && local_state_source == NULL;
    const bool use_boundary_vadose_side_psi =
      !use_saved_vadose_side_state &&
      local_state_source == NULL &&
      boundary_vadose_side_source != NULL;
    struct wetting_front *state_source = local_state_source;
    if (state_source == NULL && !use_saved_vadose_side_state &&
        !use_boundary_vadose_side_psi) {
      state_source = support->next;
      if (state_source == NULL) {
        state_source = previous_to_support;
      }
    }
    const int groundwater_soil_num = soil_type[groundwater_layer];
    const double vadose_theta =
      (use_saved_vadose_side_state && std::isfinite(saved_vadose_side_theta))
        ? saved_vadose_side_theta
        : use_boundary_vadose_side_psi
        ? calc_theta_from_h(boundary_vadose_side_source->psi_cm,
                            soil_properties[groundwater_soil_num].vg_alpha_per_cm,
                            soil_properties[groundwater_soil_num].vg_m,
                            soil_properties[groundwater_soil_num].vg_n,
                            soil_properties[groundwater_soil_num].theta_e,
                            soil_properties[groundwater_soil_num].theta_r)
        : (state_source != NULL && std::isfinite(state_source->theta))
        ? state_source->theta
        : support->theta;
    vadose_side_front =
      lgarto_insert_front_after(previous_to_support,
                                groundwater_depth_cm,
                                vadose_theta,
                                groundwater_layer,
                                FALSE,
                                head);
    if (vadose_side_front != NULL) {
      if (use_saved_vadose_side_state) {
        vadose_side_front->is_WF_GW = saved_vadose_side_is_WF_GW;
        vadose_side_front->psi_cm = saved_vadose_side_psi_cm;
        vadose_side_front->K_cm_per_h = saved_vadose_side_K_cm_per_h;
        vadose_side_front->dzdt_cm_per_h = saved_vadose_side_dzdt_cm_per_h;
      }
      else if (use_boundary_vadose_side_psi) {
        vadose_side_front->is_WF_GW = TRUE;
        vadose_side_front->dzdt_cm_per_h = 0.0;
        lgarto_set_front_hydraulic_state_from_psi(
          vadose_side_front, boundary_vadose_side_source->psi_cm, soil_type,
          frozen_factor, soil_properties);
        if (local_state_source_is_saturated &&
            previous_to_support != NULL &&
            ...
            previous_to_support->layer_num == groundwater_layer &&
            previous_to_support->depth_cm < groundwater_depth_cm - support_tol_cm) {
          previous_to_support->dzdt_cm_per_h = 0.0;
          lgarto_set_front_hydraulic_state_from_psi(
            previous_to_support, boundary_vadose_side_source->psi_cm,
            soil_type, frozen_factor, soil_properties);
        }
      }
      else {
        vadose_side_front->is_WF_GW =
          state_source != NULL ? state_source->is_WF_GW : TRUE;
        vadose_side_front->psi_cm =
          state_source != NULL ? state_source->psi_cm : support->psi_cm;
        vadose_side_front->K_cm_per_h =
          state_source != NULL ? state_source->K_cm_per_h : support->K_cm_per_h;
      }
    }
  }
  ...
  struct wetting_front *boundary_vadose_side_source = NULL;
  bool has_same_layer_front_above_support = false;
  for (struct wetting_front *scan = *head;
       scan != NULL && scan != vadose_side_front;
       scan = scan->next) {
    if (scan->is_WF_GW &&
        !scan->to_bottom &&
        scan->layer_num == groundwater_layer) {
  ...
  if (!has_same_layer_front_above_support &&
      boundary_vadose_side_source != NULL) {
    const bool boundary_source_is_near_saturated =
      lgarto_front_is_near_saturated(boundary_vadose_side_source, num_layers,
                                     soil_type, soil_properties);
    const bool vadose_side_is_near_saturated =
      lgarto_front_is_near_saturated(vadose_side_front, num_layers,
                                     soil_type, soil_properties);
    if (boundary_source_is_near_saturated &&
        !vadose_side_is_near_saturated &&
        std::isfinite(vadose_side_front->psi_cm)) {
      boundary_vadose_side_source->dzdt_cm_per_h = 0.0;
      lgarto_set_front_hydraulic_state_from_psi(
        boundary_vadose_side_source, vadose_side_front->psi_cm, soil_type,
        frozen_factor, soil_properties);
    }
    else {
      lgarto_set_front_hydraulic_state_from_psi(
  ...
  vadose_side_front->depth_cm = groundwater_depth_cm;
  vadose_side_front->layer_num = groundwater_layer;
  vadose_side_front->to_bottom = FALSE;
  vadose_side_front->is_WF_GW = TRUE;
  vadose_side_front->dzdt_cm_per_h = 0.0;
  lgarto_set_front_hydraulic_state_from_psi(vadose_side_front,
                                            vadose_side_front->psi_cm,
                                            soil_type, frozen_factor,
                                            soil_properties);

  support->depth_cm = groundwater_depth_cm;
  support->layer_num = groundwater_layer;
  support->to_bottom = FALSE;
  support->is_WF_GW = TRUE;
  support->dzdt_cm_per_h = 0.0;
  lgarto_set_front_hydraulic_state_from_psi(support, 0.0, soil_type,
                                            frozen_factor, soil_properties);

  bool below_support = false;
  for (struct wetting_front *current = *head; current != NULL; current = current->next) {
    if (current == support) {
      below_support = true;
      continue;
    ...
    if (current->is_WF_GW) {
      lgarto_set_front_hydraulic_state_from_psi(current, 0.0, soil_type,
                                                frozen_factor, soil_properties);
      if (!current->to_bottom &&
          current->layer_num > 1 &&
          current->layer_num <= num_layers &&
          current->depth_cm <
            cum_layer_thickness_cm[current->layer_num - 1] - support_tol_cm) {
        current->depth_cm = cum_layer_thickness_cm[current->layer_num - 1];
      }
    }
  }

  if (std::isfinite(vadose_side_front->psi_cm) &&
      vadose_side_front->psi_cm > BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM) {
    for (struct wetting_front *current = *head; current != NULL;
         current = current->next) {
      if (current == vadose_side_front || current == support ||
          !current->is_WF_GW || current->to_bottom ||
          current->layer_num != groundwater_layer ||
          current->depth_cm >= support->depth_cm - support_tol_cm ||
          current->psi_cm > BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM) {
        continue;
      }
      ...
      current->dzdt_cm_per_h = 0.0;
      lgarto_set_front_hydraulic_state_from_psi(
        current, vadose_side_front->psi_cm, soil_type, frozen_factor,
        soil_properties);
    }
  }

  struct wetting_front *previous_same_layer_TO = NULL;
  for (struct wetting_front *current = *head;
       current != NULL && current != support;
       current = current->next) {
    if (!current->is_WF_GW ||
        current->to_bottom ||
        current->layer_num != groundwater_layer ||
    ...
    if (previous_same_layer_TO != NULL &&
        std::isfinite(previous_same_layer_TO->psi_cm) &&
        std::isfinite(current->psi_cm) &&
        // Use the invariant's tolerance so CR sync cannot leave a state it rejects.
        current->psi_cm >
          previous_same_layer_TO->psi_cm +
            lgar_psi_assertion_tolerance_cm(
              previous_same_layer_TO->psi_cm, current->psi_cm)) {
      current->dzdt_cm_per_h = 0.0;
      lgarto_set_front_hydraulic_state_from_psi(
        current, previous_same_layer_TO->psi_cm, soil_type, frozen_factor,
        soil_properties);
    }

    previous_same_layer_TO = current;
  }

  struct wetting_front *active_vadose_side_front = NULL;
  struct wetting_front *active_saturated_support_front = NULL;
  if (lgarto_find_mobile_groundwater_storage_pair(
        *head, support_tol_cm, &active_vadose_side_front,
        &active_saturated_support_front, groundwater_depth_cm)) {
    struct wetting_front *active_boundary_source = NULL;
    bool has_unsaturated_same_layer_source = false;
    for (struct wetting_front *scan = *head;
         scan != NULL && scan != active_vadose_side_front;
         scan = scan->next) {
      if (scan->is_WF_GW &&
          !scan->to_bottom &&
      ...
      if (scan->is_WF_GW &&
          scan->to_bottom &&
          scan->layer_num + 1 == active_vadose_side_front->layer_num &&
          std::isfinite(scan->psi_cm)) {
        active_boundary_source = scan;
      }
    }

    if (!has_unsaturated_same_layer_source &&
        active_boundary_source != NULL) {
      const bool active_boundary_is_near_saturated =
        lgarto_front_is_near_saturated(active_boundary_source, num_layers,
                                       soil_type, soil_properties);
      const bool active_vadose_is_near_saturated =
        lgarto_front_is_near_saturated(active_vadose_side_front, num_layers,
                                       soil_type, soil_properties);
      if (!active_boundary_is_near_saturated &&
          active_vadose_is_near_saturated) {
        active_vadose_side_front->dzdt_cm_per_h = 0.0;
        lgarto_set_front_hydraulic_state_from_psi(
          active_vadose_side_front, active_boundary_source->psi_cm, soil_type,
          frozen_factor, soil_properties);
      }
      else if (active_boundary_is_near_saturated &&
               !active_vadose_is_near_saturated &&
               std::isfinite(active_vadose_side_front->psi_cm)) {
        active_boundary_source->dzdt_cm_per_h = 0.0;
        lgarto_set_front_hydraulic_state_from_psi(
          active_boundary_source, active_vadose_side_front->psi_cm, soil_type,
          frozen_factor, soil_properties);
      }
      else if (active_boundary_is_near_saturated) {
        // The saturated boundary source now anchors the CR chain; keep any
        // immediately upstream to_bottom bridges psi-continuous with it.
        lgar_restore_upstream_to_bottom_chain_psi(
          active_boundary_source, active_boundary_source->psi_cm, soil_type,
          soil_properties, *head);
      }
    }

    if (std::isfinite(active_vadose_side_front->psi_cm) &&
        active_vadose_side_front->psi_cm >
          BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM) {
      for (struct wetting_front *current = *head;
           current != NULL && current != active_saturated_support_front;
           current = current->next) {
        if (current == active_vadose_side_front ||
            !current->is_WF_GW || current->to_bottom ||
            current->layer_num != active_saturated_support_front->layer_num ||
            fabs(current->depth_cm -
                 active_saturated_support_front->depth_cm) > support_tol_cm ||
            !lgarto_front_is_near_saturated(current, num_layers, soil_type,
                                            soil_properties)) {
          continue;
        }
        ...
        current->dzdt_cm_per_h = 0.0;
        lgarto_set_front_hydraulic_state_from_psi(
          current, active_vadose_side_front->psi_cm, soil_type, frozen_factor,
          soil_properties);
      }
    }

    bool deleted_duplicate_saturated_support = true;
    while (deleted_duplicate_saturated_support) {
      deleted_duplicate_saturated_support = false;
      for (struct wetting_front *current = *head; current != NULL;
           current = current->next) {
        if (current == active_vadose_side_front ||
            current == active_saturated_support_front ||
            !current->is_WF_GW ||
            current->to_bottom ||
            current->layer_num != active_saturated_support_front->layer_num ||
            fabs(current->depth_cm -
        ...
        (void) listDeleteFront(current->front_num, head, soil_type,
                               soil_properties);
        deleted_duplicate_saturated_support = true;
        break;
      }
	    }
	  }

	  lgar_delete_redundant_colocated_psi0_support_stacks(
	    head, soil_type, soil_properties, cum_layer_thickness_cm);

	  const double mass_after_cm =
	    lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
  const double lower_boundary_flux_correction_cm =
    mass_before_cm - mass_after_cm;
  if (!std::isfinite(lower_boundary_flux_correction_cm)) {
    fprintf(stderr,
            "Error: mobile groundwater support maintenance produced non-finite flux correction.\n"
...
static int lgarto_layer_for_depth_cm(double depth_cm,
				     int num_layers,
				     double *cum_layer_thickness_cm)
{
  if (num_layers <= 0 || cum_layer_thickness_cm == NULL ||
...
{
  if (vadose_side_front != NULL) {
    *vadose_side_front = NULL;
  }
  if (saturated_support_front != NULL) {
    *saturated_support_front = NULL;
  }
  ...
  for (struct wetting_front *current = head; current != NULL && current->next != NULL;
    ...
    if (current->is_WF_GW && next->is_WF_GW &&
        ...
        // When the scalar GW depth is known, it is the pair identity; do not
        // let an unrelated dry/zero TO contact elsewhere steal that identity.
        (!std::isfinite(expected_pair_depth_cm) ||
         std::fabs(current->depth_cm - expected_pair_depth_cm) <= depth_tol_cm) &&
        next->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM &&
...
static bool lgarto_is_saturated_groundwater_front(struct wetting_front *front,
						  int num_layers,
						  int *soil_type,
						  struct soil_properties_ *soil_properties)
{
...
extern double lgarto_mobile_groundwater_CR_storage_cm(int num_layers,
							      double *cum_layer_thickness_cm,
							      int *soil_type,
							      struct wetting_front *head,
							      struct soil_properties_ *soil_properties,
							      double expected_pair_depth_cm)
{
  if (head == NULL || num_layers <= 0 || cum_layer_thickness_cm == NULL ||
      soil_type == NULL || soil_properties == NULL) {
    return 0.0;
  }
  ...
  if (!lgarto_find_mobile_groundwater_storage_pair(head, support_tol_cm,
                                                   &vadose_side_front,
                                                   &saturated_support_front,
                                                   expected_pair_depth_cm)) {
    return 0.0;
  }

  double storage_cm = 0.0;
  struct wetting_front *previous = vadose_side_front;
  for (struct wetting_front *current = saturated_support_front;
       current != NULL; current = current->next) {
    if (!lgarto_is_saturated_groundwater_front(current, num_layers,
                                               soil_type, soil_properties)) {
      break;
    }

    const int layer = current->layer_num;
    const int soil = soil_type[layer];
    const double theta_e = soil_properties[soil].theta_e;
    const double theta_r = soil_properties[soil].theta_r;
    if (theta_e <= theta_r) {
      previous = current;
      continue;
    }

    double interval_top_cm = cum_layer_thickness_cm[layer - 1];
    if (previous != NULL && previous->layer_num == layer) {
      interval_top_cm = fmax(interval_top_cm, previous->depth_cm);
    }
    interval_top_cm = fmax(interval_top_cm, cum_layer_thickness_cm[layer - 1]);
    interval_top_cm = fmin(interval_top_cm, cum_layer_thickness_cm[layer]);

    double interval_bottom_cm = fmin(current->depth_cm, cum_layer_thickness_cm[layer]);
    interval_bottom_cm = fmin(interval_bottom_cm, fixed_column_depth_cm);
    if (interval_bottom_cm > interval_top_cm) {
      storage_cm += (theta_e - theta_r) * (interval_bottom_cm - interval_top_cm);
      previous = current;
    }
  }

  return fmax(0.0, storage_cm);
}

static int lgarto_count_mobile_groundwater_storage_pairs(struct wetting_front *head,
							 double depth_tol_cm,
							 struct wetting_front **vadose_side_front,
							 struct wetting_front **saturated_support_front)
{
...
{
  if (num_layers <= 0 || cum_layer_thickness_cm == NULL ||
      soil_type == NULL || soil_properties == NULL || head == NULL ||
      !std::isfinite(target_CR_storage_cm) ||
      !std::isfinite(groundwater_depth_cm)) {
    return;
  }
  ...
  // The scalar groundwater proxy may recede below the fixed soil column, while
  // its explicit zero-storage support pair remains clipped to the column bottom.
  const double expected_pair_depth_cm =
    fmin(groundwater_depth_cm, fixed_column_depth_cm);

  const double support_tol_cm =
    fmax(MOBILE_GROUNDWATER_SUBMERGENCE_TOL_CM,
         1.0e-10 * fmax(1.0, fixed_column_depth_cm));
  const double depth_tol_cm = fmax(1.0e-5, 10.0 * support_tol_cm);
  const double storage_tol_cm =
    fmax(1.0e-4, 1.0e-4 * fmax(1.0, fabs(target_CR_storage_cm)));
  const double psi_zero_tol_cm = 1.0e-8;
  const double theta_tol = 1.0e-8;

  struct wetting_front *vadose_side_front = NULL;
  struct wetting_front *saturated_support_front = NULL;
  const bool has_storage_pair =
    lgarto_find_mobile_groundwater_storage_pair(head, support_tol_cm,
                                                &vadose_side_front,
                                                &saturated_support_front,
                                                expected_pair_depth_cm);
  if (!has_storage_pair && fmax(0.0, target_CR_storage_cm) <= storage_tol_cm) {
    return;
  }
  if (!has_storage_pair || vadose_side_front == NULL ||
      saturated_support_front == NULL) {
    fprintf(stderr,
            "Error: mobile groundwater CR chain has no canonical storage pair.\n"
            "  target_CR_storage_cm=%.17g groundwater_depth_cm=%.17g\n",
            target_CR_storage_cm, groundwater_depth_cm);
    fflush(stderr);
    listPrint(head);
    fflush(stdout);
    abort();
  ...
  if (fabs(vadose_side_front->depth_cm - expected_pair_depth_cm) > depth_tol_cm ||
      fabs(saturated_support_front->depth_cm - expected_pair_depth_cm) > depth_tol_cm) {
    fprintf(stderr,
            "Error: mobile groundwater CR storage pair depth does not match groundwater depth.\n"
            "  groundwater_depth_cm=%.17g vadose_pair_depth_cm=%.17g "
            "saturated_pair_depth_cm=%.17g depth_tol_cm=%.17g\n",
            groundwater_depth_cm,
            vadose_side_front->depth_cm,
            saturated_support_front->depth_cm,
            depth_tol_cm);
    fflush(stderr);
    listPrint(head);
    fflush(stdout);
  ...
  if (fabs(saturated_support_front->psi_cm) > psi_zero_tol_cm) {
    fprintf(stderr,
            "Error: mobile groundwater saturated support psi is not zero.\n"
            "  support_psi_cm=%.17g psi_zero_tol_cm=%.17g\n",
            saturated_support_front->psi_cm,
            psi_zero_tol_cm);
    fflush(stderr);
    listPrint(head);
    fflush(stdout);
  ...
  const double chain_storage_cm =
    lgarto_mobile_groundwater_CR_storage_cm(num_layers, cum_layer_thickness_cm,
                                            ...
                                            expected_pair_depth_cm);
  if (fabs(chain_storage_cm - fmax(0.0, target_CR_storage_cm)) >
      storage_tol_cm) {
    fprintf(stderr,
            "Error: mobile groundwater CR chain storage does not match conceptual reservoir storage.\n"
            "  target_CR_storage_cm=%.17g chain_storage_cm=%.17g "
            "storage_tol_cm=%.17g groundwater_depth_cm=%.17g\n",
            target_CR_storage_cm,
            chain_storage_cm,
            storage_tol_cm,
            groundwater_depth_cm);
  ...
  for (struct wetting_front *current = vadose_side_front;
       current != NULL; current = current->next) {
    const bool in_pair = current == vadose_side_front ||
                         current == saturated_support_front;
    const bool in_saturated_chain =
      current == saturated_support_front ||
      (current != vadose_side_front && current->is_WF_GW &&
       current->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM);
    if (!in_pair && !in_saturated_chain) {
      break;
    }

    if (current->layer_num < 1 || current->layer_num > num_layers) {
      fprintf(stderr,
              "Error: mobile groundwater CR chain front has invalid layer.\n"
              "  layer_num=%d depth_cm=%.17g\n",
              current->layer_num, current->depth_cm);
      fflush(stderr);
      listPrint(head);
      fflush(stdout);
      abort();
    ...
    const double layer_top_cm = cum_layer_thickness_cm[current->layer_num - 1];
    const double layer_bottom_cm = cum_layer_thickness_cm[current->layer_num];
    if (current->depth_cm < layer_top_cm - depth_tol_cm ||
        current->depth_cm > layer_bottom_cm + depth_tol_cm) {
      fprintf(stderr,
              "Error: mobile groundwater CR chain front is outside its assigned layer.\n"
              "  layer_num=%d depth_cm=%.17g layer_top_cm=%.17g "
              "layer_bottom_cm=%.17g\n",
              current->layer_num,
              current->depth_cm,
              layer_top_cm,
    ...
    const int soil = soil_type[current->layer_num];
    const double expected_theta =
      calc_theta_from_h(current->psi_cm,
    ...
    if (fabs(current->theta - expected_theta) > theta_tol) {
      fprintf(stderr,
              "Error: mobile groundwater CR chain front hydraulic state is not canonical for its layer.\n"
              "  layer_num=%d depth_cm=%.17g psi_cm=%.17g theta=%.17g "
              "expected_theta=%.17g theta_tol=%.17g\n",
              current->layer_num,
              current->depth_cm,
              current->psi_cm,
              current->theta,
...
static bool lgarto_depth_from_bottom_saturated_CR_storage_cm(double target_storage_cm,
							     int num_layers,
							     double *cum_layer_thickness_cm,
							     int *soil_type,
							     struct soil_properties_ *soil_properties,
							     double *groundwater_depth_cm)
{
  if (groundwater_depth_cm != NULL) {
    *groundwater_depth_cm = NAN;
  }
  if (groundwater_depth_cm == NULL || num_layers <= 0 ||
      cum_layer_thickness_cm == NULL || soil_type == NULL ||
  ...
  double remaining_storage_cm = fmax(0.0, target_storage_cm);
  if (remaining_storage_cm <= MBAL_ITERATIVE_TOLERANCE) {
    *groundwater_depth_cm = fixed_column_depth_cm;
    return true;
  }
  ...
  for (int layer = num_layers; layer >= 1; layer--) {
    ...
    const double theta_e = soil_properties[soil].theta_e;
    const double theta_r = soil_properties[soil].theta_r;
    const double storage_coefficient = theta_e - theta_r;
    const double layer_top_cm = cum_layer_thickness_cm[layer - 1];
    const double layer_bottom_cm = cum_layer_thickness_cm[layer];
    const double layer_thickness_cm = layer_bottom_cm - layer_top_cm;
    if (storage_coefficient <= 0.0 || layer_thickness_cm <= 0.0 ||
        !std::isfinite(storage_coefficient) ||
        !std::isfinite(layer_thickness_cm)) {
      continue;
    }

    const double layer_capacity_cm = storage_coefficient * layer_thickness_cm;
    if (remaining_storage_cm <= layer_capacity_cm + MBAL_ITERATIVE_TOLERANCE) {
      const double saturated_height_cm = remaining_storage_cm / storage_coefficient;
      *groundwater_depth_cm =
        fmin(layer_bottom_cm, fmax(layer_top_cm,
                                   layer_bottom_cm - saturated_height_cm));
      return true;
    }

    remaining_storage_cm -= layer_capacity_cm;
  }
...
static bool lgarto_rebuild_mobile_groundwater_storage_pair_at_depth(double groundwater_depth_cm,
								    int num_layers,
								    double *cum_layer_thickness_cm,
								    int *soil_type,
								    double *frozen_factor,
								    ...
								    struct soil_properties_ *soil_properties)
{
  if (head == NULL || *head == NULL || num_layers <= 0 ||
      cum_layer_thickness_cm == NULL || soil_type == NULL ||
      soil_properties == NULL || !std::isfinite(groundwater_depth_cm)) {
    return false;
  }
  ...
  const double old_support_depth_cm = saturated_support_front->depth_cm;
  const int old_support_layer = saturated_support_front->layer_num;
  const bool saved_vadose_is_WF_GW = vadose_side_front->is_WF_GW;
  const double saved_vadose_psi_cm = vadose_side_front->psi_cm;
  const double saved_vadose_dzdt_cm_per_h = vadose_side_front->dzdt_cm_per_h;
  ...
  std::vector<lgarto_saved_to_bottom_state> to_bottom_states;
  for (struct wetting_front *scan = *head; scan != NULL; scan = scan->next) {
    if (scan->to_bottom &&
        scan != vadose_side_front &&
        scan != saturated_support_front) {
      to_bottom_states.push_back({
        scan,
        scan->is_WF_GW,
        scan->theta,
  ...
  (void) listDeleteFront(saturated_support_front->front_num, head,
                         soil_type, soil_properties);
  (void) listDeleteFront(vadose_side_front->front_num, head,
                         soil_type, soil_properties);
  for (size_t i = 0; i < to_bottom_states.size(); i++) {
    to_bottom_states[i].front->is_WF_GW = to_bottom_states[i].is_WF_GW;
    to_bottom_states[i].front->theta = to_bottom_states[i].theta;
  ...
  if (old_support_layer >= 1 && old_support_layer <= num_layers &&
      groundwater_depth_cm > cum_layer_thickness_cm[old_support_layer] + support_tol_cm &&
      saved_vadose_is_WF_GW &&
      saved_vadose_psi_cm > BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM) {
    for (struct wetting_front *scan = *head; scan != NULL; scan = scan->next) {
      if (scan->to_bottom && scan->layer_num == old_support_layer) {
        // Receding below this layer drains the old saturated support; keep the
        // deleted vadose-side psi on the layer boundary instead of leaving psi=0.
        ...
        scan->dzdt_cm_per_h = 0.0;
        lgarto_set_front_hydraulic_state_from_psi(
          scan, saved_vadose_psi_cm, soil_type, frozen_factor,
          soil_properties);
        break;
      }
    }
  ...
  (void)
    lgarto_maintain_mobile_groundwater_support(
      groundwater_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
      frozen_factor, head, soil_properties);

  struct wetting_front *new_vadose_side_front = NULL;
  struct wetting_front *new_saturated_support_front = NULL;
  if (!lgarto_find_mobile_groundwater_storage_pair(
        *head, support_tol_cm, &new_vadose_side_front,
        &new_saturated_support_front)) {
    return false;
  }

  if (saved_vadose_is_WF_GW && std::isfinite(saved_vadose_psi_cm)) {
    double applied_vadose_psi_cm = saved_vadose_psi_cm;
    struct wetting_front *previous_to_new_vadose =
      lgar_find_previous_front(*head, new_vadose_side_front);
    if (previous_to_new_vadose != NULL &&
        previous_to_new_vadose->is_WF_GW &&
        previous_to_new_vadose->layer_num == new_vadose_side_front->layer_num &&
        std::isfinite(previous_to_new_vadose->psi_cm) &&
        previous_to_new_vadose->psi_cm < applied_vadose_psi_cm) {
      applied_vadose_psi_cm = previous_to_new_vadose->psi_cm;
    }
    struct wetting_front *boundary_vadose_side_source = NULL;
    bool has_same_layer_front_above_support = false;
    for (struct wetting_front *scan = *head;
      ...
      if (scan->is_WF_GW &&
          scan->to_bottom &&
          scan->layer_num + 1 == new_vadose_side_front->layer_num &&
          std::isfinite(scan->psi_cm)) {
        boundary_vadose_side_source = scan;
      }
    }
    bool sync_boundary_source_to_new_vadose = false;
    if (!has_same_layer_front_above_support &&
        boundary_vadose_side_source != NULL) {
      const bool boundary_source_is_near_saturated =
        lgarto_front_is_near_saturated(boundary_vadose_side_source, num_layers,
                                       soil_type, soil_properties);
      if (boundary_source_is_near_saturated &&
          applied_vadose_psi_cm >
            BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM) {
        sync_boundary_source_to_new_vadose = true;
      }
      else {
        applied_vadose_psi_cm = boundary_vadose_side_source->psi_cm;
      }
    }

    new_vadose_side_front->is_WF_GW = TRUE;
    new_vadose_side_front->dzdt_cm_per_h = saved_vadose_dzdt_cm_per_h;
    lgarto_set_front_hydraulic_state_from_psi(
      new_vadose_side_front, applied_vadose_psi_cm, soil_type,
      frozen_factor, soil_properties);
    if (sync_boundary_source_to_new_vadose &&
        boundary_vadose_side_source != NULL) {
      boundary_vadose_side_source->dzdt_cm_per_h = 0.0;
      lgarto_set_front_hydraulic_state_from_psi(
        boundary_vadose_side_source, applied_vadose_psi_cm, soil_type,
        frozen_factor, soil_properties);
    }

    if (groundwater_depth_cm > old_support_depth_cm + support_tol_cm) {
      for (struct wetting_front *current = *head;
           current != NULL && current != new_saturated_support_front;
           current = current->next) {
        if (current->is_WF_GW &&
            current->depth_cm > old_support_depth_cm - support_tol_cm &&
            current->depth_cm < groundwater_depth_cm - support_tol_cm &&
            current->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM) {
          lgarto_set_front_hydraulic_state_from_psi(
            current, applied_vadose_psi_cm, soil_type, frozen_factor,
            soil_properties);
        }
      }
    }
...
{
  if (!lgarto_rebuild_mobile_groundwater_storage_pair_at_depth(
        groundwater_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
        frozen_factor, head, soil_properties)) {
    return false;
  }

  // A rising GW table can overtake packets or boundary scaffolds; canonicalize
  // them before chain-storage measurement can select a stale support.
  (void) lgarto_submerge_wetting_fronts_below_groundwater(
    groundwater_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
    frozen_factor, head, soil_properties);
  struct wetting_front *vadose_side_front = NULL;
  struct wetting_front *saturated_support_front = NULL;
  ...
  const double support_tol_cm =
    fmax(MOBILE_GROUNDWATER_SUBMERGENCE_TOL_CM,
         1.0e-10 * fmax(1.0, fixed_column_depth_cm));
  if (lgarto_find_mobile_groundwater_storage_pair(
        *head, support_tol_cm, &vadose_side_front,
        &saturated_support_front)) {
    if (std::isfinite(preserved_vadose_psi_cm)) {
      // Keep the profile immediately above a packet-crossing GW rise; generic
      // submergence cleanup can otherwise inherit a much shallower wet state.
      vadose_side_front->dzdt_cm_per_h = 0.0;
      lgarto_set_front_hydraulic_state_from_psi(
        vadose_side_front, preserved_vadose_psi_cm, soil_type, frozen_factor,
        soil_properties);
    }
    struct wetting_front *boundary_source = NULL;
    ...
    for (struct wetting_front *scan = *head;
      ...
      if (scan->to_bottom &&
          scan->layer_num + 1 == vadose_side_front->layer_num &&
          std::isfinite(scan->psi_cm)) {
        boundary_source = scan;
      }
    }
    if (!std::isfinite(preserved_vadose_psi_cm) &&
        !has_same_layer_source && boundary_source != NULL &&
        !lgarto_front_is_near_saturated(boundary_source, num_layers,
                                        soil_type, soil_properties)) {
      vadose_side_front->dzdt_cm_per_h = 0.0;
      lgarto_set_front_hydraulic_state_from_psi(
        vadose_side_front, boundary_source->psi_cm, soil_type, frozen_factor,
        soil_properties);
    }
  }
  return true;
...
static bool lgarto_move_mobile_groundwater_storage_pair_to_depth(double groundwater_depth_cm,
								 int num_layers,
								 double *cum_layer_thickness_cm,
								 struct wetting_front *head,
								 double source_pair_depth_cm)
{
  const double fixed_column_depth_cm =
    (num_layers > 0 && cum_layer_thickness_cm != NULL)
      ? cum_layer_thickness_cm[num_layers]
      : 0.0;
  ...
  struct wetting_front *vadose_side_front = NULL;
  struct wetting_front *saturated_support_front = NULL;
  // Trial-list pointers differ, so retain pair identity by its original depth.
  if (!lgarto_find_mobile_groundwater_storage_pair(head, depth_tol_cm,
                                                   &vadose_side_front,
                                                   &saturated_support_front,
                                                   source_pair_depth_cm)) {
    return false;
  }
  ...
  if (vadose_side_front->layer_num != layer ||
      saturated_support_front->layer_num != layer) {
    return false;
  }

  vadose_side_front->depth_cm = groundwater_depth_cm;
  vadose_side_front->layer_num = layer;
  saturated_support_front->depth_cm = groundwater_depth_cm;
  saturated_support_front->layer_num = layer;
  return true;
}

static bool lgarto_promote_surface_front_intercepted_by_rising_mobile_groundwater(
  double groundwater_depth_cm,
  ...
  struct soil_properties_ *soil_properties)
{
  if (head == NULL || *head == NULL || num_layers <= 0 ||
      ...
      soil_properties == NULL ||
      !std::isfinite(groundwater_depth_cm) ||
      !std::isfinite(current_groundwater_depth_cm)) {
    return false;
  }
  ...
  struct wetting_front *surface_front = NULL;
  for (struct wetting_front *current = *head; current != NULL; current = current->next) {
    if (current->is_WF_GW || current->to_bottom ||
        current->layer_num != groundwater_layer ||
    ...
    if (surface_front == NULL || current->depth_cm < surface_front->depth_cm) {
      surface_front = current;
    }
  }
  ...
  struct wetting_front *old_vadose_side_front = NULL;
  struct wetting_front *old_saturated_support_front = NULL;
  if (!lgarto_find_mobile_groundwater_storage_pair(
        *head, support_tol_cm, &old_vadose_side_front,
        &old_saturated_support_front)) {
    return false;
  }

  const int old_front_num = surface_front->front_num;
  const double old_depth_cm = surface_front->depth_cm;
  ...
  for (struct wetting_front *scan = *head;
       ...
       scan = scan->next) {
    if (scan->to_bottom != TRUE || scan->is_WF_GW == TRUE ||
        scan->layer_num >= groundwater_layer ||
        (!std::isfinite(scan->psi_cm) ||
         !std::isfinite(scan->next->psi_cm))) {
      continue;
    }
    if (scan->next != surface_front &&
        !(scan->next->to_bottom == TRUE &&
          scan->next->is_WF_GW == FALSE &&
          scan->next->layer_num > scan->layer_num &&
          scan->next->layer_num <= groundwater_layer)) {
      continue;
    }
    const double psi_tol_cm =
      lgar_boundary_roundtrip_psi_tolerance_cm(scan->psi_cm,
                                               scan->next->psi_cm);
    if (scan->psi_cm <= scan->next->psi_cm + psi_tol_cm &&
        scan->psi_cm > zero_depth_scaffold_psi_cm) {
      // This upstream bridge will become TO; keep the zero-depth scaffold no wetter.
      zero_depth_scaffold_psi_cm = scan->psi_cm;
    }
  }
  const double operation_mass_before_cm =
    lgar_calc_mass_bal(cum_layer_thickness_cm, *head);

  if (old_saturated_support_front != surface_front) {
    (void) listDeleteFront(old_saturated_support_front->front_num, head,
                           soil_type, soil_properties);
  }
  if (old_vadose_side_front != surface_front) {
    (void) listDeleteFront(old_vadose_side_front->front_num, head,
                           soil_type, soil_properties);
  }

  // Match the surface/TO convention without applying its event-limit remap:
  // the old TO state becomes a zero-depth scaffold, while the intercepted
  // surface state becomes the latched vadose-side member of the mobile-GW pair.
  surface_front->depth_cm = 0.0;
  surface_front->layer_num = groundwater_layer;
  surface_front->to_bottom = FALSE;
  surface_front->is_WF_GW = TRUE;
  surface_front->dzdt_cm_per_h = 0.0;
  lgarto_set_front_hydraulic_state_from_psi(
    surface_front, zero_depth_scaffold_psi_cm, soil_type, frozen_factor,
    soil_properties);
  lgarto_rehome_zero_depth_front_to_top_layer_from_psi(
    surface_front, soil_type, frozen_factor, soil_properties);
  listSendToTop(*head);

  struct wetting_front *previous_to_pair = NULL;
  for (struct wetting_front *current = *head; current != NULL;
       current = current->next) {
    if (current->depth_cm > groundwater_depth_cm + support_tol_cm) {
      break;
    }
    previous_to_pair = current;
  }

  const int groundwater_soil_num = soil_type[groundwater_layer];
  struct wetting_front *vadose_side =
    lgarto_insert_front_after(previous_to_pair, groundwater_depth_cm,
                              saved_vadose_theta, groundwater_layer,
                              FALSE, head);
  if (vadose_side == NULL) {
    return false;
  }
  vadose_side->is_WF_GW = TRUE;
  vadose_side->dzdt_cm_per_h = 0.0;
  lgarto_set_front_hydraulic_state_from_psi(
    vadose_side, saved_surface_psi_cm, soil_type, frozen_factor,
    soil_properties);
  struct wetting_front *support =
    lgarto_insert_front_after(vadose_side, groundwater_depth_cm,
                              soil_properties[groundwater_soil_num].theta_e,
                              groundwater_layer, FALSE, head);
  if (support == NULL) {
    return false;
  }
  support->is_WF_GW = TRUE;
  ...
  lgarto_set_front_hydraulic_state_from_psi(support, 0.0, soil_type,
                                            frozen_factor, soil_properties);
  for (struct wetting_front *current = support->next;
       current != NULL; current = current->next) {
    if (!current->is_WF_GW &&
        current->layer_num == groundwater_layer &&
        current->depth_cm > groundwater_depth_cm + support_tol_cm &&
        current->depth_cm <= current_groundwater_depth_cm + support_tol_cm) {
      // This cluster member is fully submerged by the same rising table event.
      current->is_WF_GW = TRUE;
      current->dzdt_cm_per_h = 0.0;
    }
    if (current->is_WF_GW) {
  ...
  if (verbosity.compare("high") == 0) {
    const double operation_mass_after_cm =
      lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
    printf("Mobile groundwater promoted surface front %d from %.17lf cm to "
           "TO/GW at rising GW depth %.17lf cm; routed %.17lf cm through "
           "mobile groundwater exchange.\n",
           old_front_num,
           old_depth_cm,
           groundwater_depth_cm,
           operation_mass_before_cm - operation_mass_after_cm);
  }

  return true;
}

static double lgarto_trial_mass_after_mobile_groundwater_storage_pair_depth(double groundwater_depth_cm,
									   int num_layers,
									   double *cum_layer_thickness_cm,
									   struct wetting_front *head,
									   double source_pair_depth_cm)
{
  ...
  if (!lgarto_move_mobile_groundwater_storage_pair_to_depth(
          groundwater_depth_cm, num_layers, cum_layer_thickness_cm,
          trial_head, source_pair_depth_cm)) {
    listDelete(trial_head);
    return NAN;
  }

  const double trial_mass_cm =
    lgar_calc_mass_bal(cum_layer_thickness_cm, trial_head);
  listDelete(trial_head);
  return trial_mass_cm;
}

static double lgarto_trial_CR_storage_after_mobile_groundwater_storage_pair_depth(double groundwater_depth_cm,
										 int num_layers,
										 double *cum_layer_thickness_cm,
										 int *soil_type,
										 double *frozen_factor,
										 ...
										 struct soil_properties_ *soil_properties)
{
  ...
  (void) lgarto_maintain_mobile_groundwater_support(
      groundwater_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
      frozen_factor, &trial_head, soil_properties);

  const double trial_storage_cm =
    lgarto_mobile_groundwater_CR_storage_cm(num_layers, cum_layer_thickness_cm,
                                            soil_type, trial_head,
                                            soil_properties);
  listDelete(trial_head);
  return trial_storage_cm;
}
...
{
  if (updated_groundwater_depth_cm != NULL) {
  ...
  if (head == NULL || *head == NULL || num_layers <= 0 ||
      cum_layer_thickness_cm == NULL || soil_type == NULL ||
      soil_properties == NULL ||
      !std::isfinite(requested_storage_change_cm) ||
      !std::isfinite(current_groundwater_depth_cm)) {
    return 0.0;
  }
  ...
  const double support_tol_cm =
    fmax(MOBILE_GROUNDWATER_SUBMERGENCE_TOL_CM,
         1.0e-10 * fmax(1.0, fixed_column_depth_cm));
  const double shallow_depth_cm = support_tol_cm;
  const double deep_depth_cm = fmax(shallow_depth_cm,
                                    fixed_column_depth_cm - 2.0 * support_tol_cm);
  const double start_depth_cm =
    fmax(shallow_depth_cm, fmin(current_groundwater_depth_cm, deep_depth_cm));
  struct wetting_front *vadose_side_front = NULL;
  struct wetting_front *saturated_support_front = NULL;
  const bool has_storage_pair =
    lgarto_find_mobile_groundwater_storage_pair(*head, support_tol_cm,
  ...
  const int pair_layer = saturated_support_front->layer_num;
  if (pair_layer < 1 || pair_layer > num_layers) {
    return 0.0;
  }
  const double pair_shallow_depth_cm =
    fmax(cum_layer_thickness_cm[pair_layer - 1] + support_tol_cm,
         shallow_depth_cm);
  const double pair_deep_depth_cm =
    fmin(cum_layer_thickness_cm[pair_layer] - 2.0 * support_tol_cm,
         deep_depth_cm);
  if (pair_deep_depth_cm <= pair_shallow_depth_cm) {
    return 0.0;
  }

  if (std::fabs(requested_storage_change_cm) <= MBAL_ITERATIVE_TOLERANCE) {
    if (updated_groundwater_depth_cm != NULL) {
      *updated_groundwater_depth_cm = start_depth_cm;
    }
    return 0.0;
  }
  ...
  const double mass_before_cm =
    lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
  const double target_mass_cm = mass_before_cm + requested_storage_change_cm;

  const double mass_at_shallow_cm =
    lgarto_trial_mass_after_mobile_groundwater_storage_pair_depth(
      pair_shallow_depth_cm, num_layers, cum_layer_thickness_cm, *head,
      start_depth_cm);
  const double mass_at_deep_cm =
    lgarto_trial_mass_after_mobile_groundwater_storage_pair_depth(
      pair_deep_depth_cm, num_layers, cum_layer_thickness_cm, *head,
      start_depth_cm);
  if (!std::isfinite(mass_at_shallow_cm) ||
      !std::isfinite(mass_at_deep_cm)) {
    return 0.0;
  }

  const bool mass_decreases_with_depth =
    mass_at_shallow_cm >= mass_at_deep_cm;
  if (!mass_decreases_with_depth) {
    return 0.0;
  }

  double selected_depth_cm = start_depth_cm;
  if (target_mass_cm >= mass_at_shallow_cm) {
    selected_depth_cm = pair_shallow_depth_cm;
  }
  else if (target_mass_cm <= mass_at_deep_cm) {
    selected_depth_cm = pair_deep_depth_cm;
  }
  else {
    double lo_depth_cm = pair_shallow_depth_cm;
    double hi_depth_cm = pair_deep_depth_cm;
    double best_depth_cm = start_depth_cm;
    double best_error_cm = HUGE_VAL;

    for (int iter = 0; iter < 80; iter++) {
      const double mid_depth_cm = 0.5 * (lo_depth_cm + hi_depth_cm);
      const double mid_mass_cm =
        lgarto_trial_mass_after_mobile_groundwater_storage_pair_depth(
          mid_depth_cm, num_layers, cum_layer_thickness_cm, *head,
          start_depth_cm);
      if (!std::isfinite(mid_mass_cm)) {
        break;
      }

      const double mid_error_cm = target_mass_cm - mid_mass_cm;
      if (std::fabs(mid_error_cm) < best_error_cm) {
        best_error_cm = std::fabs(mid_error_cm);
        best_depth_cm = mid_depth_cm;
      }
      if (std::fabs(mid_error_cm) <= MBAL_ITERATIVE_TOLERANCE) {
        best_depth_cm = mid_depth_cm;
        break;
      }

      if (mid_mass_cm < target_mass_cm) {
        hi_depth_cm = mid_depth_cm;
      }
      else {
        lo_depth_cm = mid_depth_cm;
      }
    }

    selected_depth_cm = best_depth_cm;
  }

  if (!lgarto_move_mobile_groundwater_storage_pair_to_depth(
          selected_depth_cm, num_layers, cum_layer_thickness_cm, *head,
          start_depth_cm)) {
    return 0.0;
  }
  if (updated_groundwater_depth_cm != NULL) {
    *updated_groundwater_depth_cm = selected_depth_cm;
  }

  const double mass_after_cm =
    lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
  const double actual_storage_change_cm = mass_after_cm - mass_before_cm;
  if (!std::isfinite(actual_storage_change_cm)) {
    fprintf(stderr,
            "Error: mobile groundwater support storage-change solve produced non-finite mass change.\n"
            "  requested_storage_change_cm=%.17g selected_depth_cm=%.17g\n"
            "  mass_before_cm=%.17g mass_after_cm=%.17g\n",
            requested_storage_change_cm,
            selected_depth_cm,
            mass_before_cm,
            mass_after_cm);
    fflush(stderr);
  ...
  return actual_storage_change_cm;
}

extern double lgarto_move_mobile_groundwater_support_for_CR_storage_change(double requested_CR_storage_change_cm,
									  double current_groundwater_depth_cm,
									  int num_layers,
									  double *cum_layer_thickness_cm,
									  int *soil_type,
									  ...
									  double *updated_groundwater_depth_cm)
{
  ...
  if (head == NULL || *head == NULL || num_layers <= 0 ||
      cum_layer_thickness_cm == NULL || soil_type == NULL ||
      soil_properties == NULL ||
      !std::isfinite(requested_CR_storage_change_cm) ||
      !std::isfinite(current_groundwater_depth_cm)) {
    return 0.0;
  }
  ...
  const double support_tol_cm =
    fmax(MOBILE_GROUNDWATER_SUBMERGENCE_TOL_CM,
         1.0e-10 * fmax(1.0, fixed_column_depth_cm));
  const double shallow_depth_cm = support_tol_cm;
  const double deep_depth_cm = fixed_column_depth_cm;
  const double start_depth_cm =
    lgarto_canonical_mobile_groundwater_depth_cm(
      fmax(shallow_depth_cm, fmin(current_groundwater_depth_cm, deep_depth_cm)),
  ...
  const bool has_storage_pair =
    lgarto_find_mobile_groundwater_storage_pair(*head, support_tol_cm,
                                                ...
                                                start_depth_cm);
  if (!has_storage_pair) {
    const double target_storage_cm = fmax(0.0, requested_CR_storage_change_cm);
    double bootstrap_depth_cm = NAN;
    if (target_storage_cm > MBAL_ITERATIVE_TOLERANCE &&
        lgarto_depth_from_bottom_saturated_CR_storage_cm(
          target_storage_cm, num_layers, cum_layer_thickness_cm, soil_type,
          soil_properties, &bootstrap_depth_cm) &&
        std::isfinite(bootstrap_depth_cm)) {
      bootstrap_depth_cm =
        lgarto_canonical_mobile_groundwater_depth_cm(
          bootstrap_depth_cm, num_layers, cum_layer_thickness_cm);
      struct wetting_front *trial_head = listCopy(*head, NULL);
      if (trial_head != NULL) {
        // Rewetting from an empty mobile-GW chain needs to recreate the
        // support pair before CR-chain storage can represent the new sliver.
        (void) lgarto_maintain_mobile_groundwater_support(
          bootstrap_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
          frozen_factor, &trial_head, soil_properties);
        const double trial_storage_cm =
          lgarto_mobile_groundwater_CR_storage_cm(num_layers,
                                                  cum_layer_thickness_cm,
                                                  soil_type, trial_head,
                                                  soil_properties);
        listDelete(trial_head);
        const double bootstrap_storage_tol_cm =
          fmax(1.0e-8, 1.0e-8 * fmax(1.0, fabs(target_storage_cm)));
        if (std::isfinite(trial_storage_cm) &&
            fabs(trial_storage_cm - target_storage_cm) <=
              bootstrap_storage_tol_cm) {
          const double storage_before_cm =
            lgarto_mobile_groundwater_CR_storage_cm(
              num_layers, cum_layer_thickness_cm, soil_type, *head,
              soil_properties);
          (void) lgarto_maintain_mobile_groundwater_support(
            bootstrap_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
            frozen_factor, head, soil_properties);
          if (updated_groundwater_depth_cm != NULL) {
            *updated_groundwater_depth_cm = bootstrap_depth_cm;
          }
          const double storage_after_cm =
            lgarto_mobile_groundwater_CR_storage_cm(
              num_layers, cum_layer_thickness_cm, soil_type, *head,
              soil_properties);
          return storage_after_cm - storage_before_cm;
        }
      }
    }
    return 0.0;
  ...
  const double pair_shallow_depth_cm = shallow_depth_cm;
  const double pair_deep_depth_cm = deep_depth_cm;
  if (pair_deep_depth_cm <= pair_shallow_depth_cm) {
    return 0.0;
  }

  if (std::fabs(requested_CR_storage_change_cm) <= MBAL_ITERATIVE_TOLERANCE) {
    const double storage_before_zero_change_cm =
      lgarto_mobile_groundwater_CR_storage_cm(num_layers, cum_layer_thickness_cm,
                                              soil_type, *head, soil_properties,
                                              start_depth_cm);
    (void)
      lgarto_maintain_mobile_groundwater_support(
        start_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
        frozen_factor, head, soil_properties);
    if (updated_groundwater_depth_cm != NULL) {
      *updated_groundwater_depth_cm = start_depth_cm;
    }
    const double storage_after_zero_change_cm =
      lgarto_mobile_groundwater_CR_storage_cm(num_layers, cum_layer_thickness_cm,
                                              soil_type, *head, soil_properties,
                                              start_depth_cm);
    return storage_after_zero_change_cm - storage_before_zero_change_cm;
  }
  ...
  const double storage_before_cm =
    lgarto_mobile_groundwater_CR_storage_cm(num_layers, cum_layer_thickness_cm,
                                            soil_type, *head, soil_properties,
                                            start_depth_cm);
  const double target_storage_cm = storage_before_cm + requested_CR_storage_change_cm;
  double rebuild_depth_cm = NAN;
  if (target_storage_cm <= MBAL_ITERATIVE_TOLERANCE) {
    // Empty CR storage is represented by a bottom-anchored support pair; do not
    // let the generic search preserve a stale saturated chain above it.
    if (lgarto_rebuild_mobile_groundwater_storage_pair_at_depth(
          fixed_column_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
          frozen_factor, head, soil_properties)) {
      if (updated_groundwater_depth_cm != NULL) {
        *updated_groundwater_depth_cm = fixed_column_depth_cm;
      }
      const double storage_after_zero_cm =
        lgarto_mobile_groundwater_CR_storage_cm(num_layers,
                                                cum_layer_thickness_cm,
                                                soil_type, *head,
                                                soil_properties);
      return storage_after_zero_cm - storage_before_cm;
    }
  }
  // A CR-driven rise can re-anchor the mobile-GW pair in a shallower layer,
  // especially when existing psi=0 scaffold fronts already saturate that layer.
  if (lgarto_depth_from_bottom_saturated_CR_storage_cm(
        target_storage_cm, num_layers, cum_layer_thickness_cm, soil_type,
        soil_properties, &rebuild_depth_cm) &&
      std::isfinite(rebuild_depth_cm)) {
    rebuild_depth_cm =
      lgarto_canonical_mobile_groundwater_depth_cm(rebuild_depth_cm,
                                                   num_layers,
                                                   cum_layer_thickness_cm);
    const int rebuild_layer =
      lgarto_layer_for_depth_cm(rebuild_depth_cm, num_layers,
                                cum_layer_thickness_cm);
    const bool cross_layer_rise =
      rebuild_depth_cm < saturated_support_front->depth_cm - support_tol_cm &&
      rebuild_layer != pair_layer;
    if (rebuild_layer >= 1 && rebuild_layer <= num_layers &&
        cross_layer_rise) {
      struct wetting_front *trial_head = listCopy(*head, NULL);
      if (trial_head != NULL) {
        const bool rebuilt_trial =
          lgarto_rebuild_mobile_groundwater_storage_pair_and_submerge_at_depth(
            rebuild_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
            frozen_factor, &trial_head, soil_properties, NAN);
        const double rebuild_trial_storage_cm =
          rebuilt_trial
            ? lgarto_mobile_groundwater_CR_storage_cm(num_layers,
                                                      cum_layer_thickness_cm,
                                                      soil_type, trial_head,
            ...
            : NAN;
        listDelete(trial_head);
        const double rebuild_storage_tol_cm =
          fmax(1.0e-8, 1.0e-8 * fmax(1.0, fabs(target_storage_cm)));
        if (rebuilt_trial &&
            std::isfinite(rebuild_trial_storage_cm) &&
            fabs(rebuild_trial_storage_cm - target_storage_cm) <=
              rebuild_storage_tol_cm &&
            lgarto_rebuild_mobile_groundwater_storage_pair_and_submerge_at_depth(
              rebuild_depth_cm, num_layers, cum_layer_thickness_cm,
              soil_type, frozen_factor, head, soil_properties, NAN)) {
          if (updated_groundwater_depth_cm != NULL) {
            *updated_groundwater_depth_cm = rebuild_depth_cm;
          }
          const double storage_after_rebuild_cm =
            lgarto_mobile_groundwater_CR_storage_cm(num_layers,
                                                    cum_layer_thickness_cm,
                                                    soil_type, *head,
                                                    soil_properties);
          return storage_after_rebuild_cm - storage_before_cm;
        }
      }
    }
  }
  const int pair_soil_num = soil_type[pair_layer];
  const double pair_storage_coefficient_cm =
    soil_properties[pair_soil_num].theta_e - soil_properties[pair_soil_num].theta_r;
  if (pair_storage_coefficient_cm > 0.0 &&
      std::isfinite(pair_storage_coefficient_cm)) {
    double same_layer_shallow_depth_cm =
      fmax(cum_layer_thickness_cm[pair_layer - 1] + support_tol_cm,
           shallow_depth_cm);
    struct wetting_front *previous_to_pair =
      lgar_find_previous_front(*head, vadose_side_front);
    if (previous_to_pair != NULL &&
        previous_to_pair->layer_num == pair_layer) {
      same_layer_shallow_depth_cm =
        fmax(same_layer_shallow_depth_cm,
             previous_to_pair->depth_cm + support_tol_cm);
    }

    double same_layer_deep_depth_cm = cum_layer_thickness_cm[pair_layer];
    if (saturated_support_front->next != NULL &&
        saturated_support_front->next->layer_num == pair_layer &&
        !saturated_support_front->next->to_bottom) {
      same_layer_deep_depth_cm =
        fmin(same_layer_deep_depth_cm,
             saturated_support_front->next->depth_cm - support_tol_cm);
    }

    const double same_layer_depth_cm =
      saturated_support_front->depth_cm -
    ...
    if (same_layer_depth_cm < saturated_support_front->depth_cm - MBAL_ITERATIVE_TOLERANCE &&
        lgarto_promote_surface_front_intercepted_by_rising_mobile_groundwater(
          ...
          cum_layer_thickness_cm, soil_type, frozen_factor, head,
          soil_properties)) {
      const double promoted_depth_cm =
        lgarto_canonical_mobile_groundwater_depth_cm(
          same_layer_depth_cm, num_layers, cum_layer_thickness_cm);
      if (updated_groundwater_depth_cm != NULL) {
        *updated_groundwater_depth_cm = promoted_depth_cm;
      }
      const double storage_after_promote_cm =
        lgarto_mobile_groundwater_CR_storage_cm(num_layers,
                                                cum_layer_thickness_cm,
                                                soil_type, *head,
                                                soil_properties);
      return storage_after_promote_cm - storage_before_cm;
    }
    const int same_layer_target_layer =
      lgarto_layer_for_depth_cm(same_layer_depth_cm, num_layers,
                                cum_layer_thickness_cm);
    const bool crosses_redundant_near_contact =
      same_layer_depth_cm <
        saturated_support_front->depth_cm - MBAL_ITERATIVE_TOLERANCE &&
      previous_to_pair != NULL &&
      previous_to_pair->layer_num == pair_layer &&
      same_layer_depth_cm <
        previous_to_pair->depth_cm - MBAL_ITERATIVE_TOLERANCE &&
      std::isfinite(previous_to_pair->psi_cm) &&
      std::isfinite(vadose_side_front->psi_cm) &&
      fabs(previous_to_pair->psi_cm - vadose_side_front->psi_cm) <=
        lgar_psi_assertion_tolerance_cm(previous_to_pair->psi_cm,
                                       vadose_side_front->psi_cm);
    const bool same_layer_packet_crossing_rise =
      same_layer_target_layer == pair_layer &&
      (same_layer_depth_cm <
         saturated_support_front->depth_cm - support_tol_cm ||
       crosses_redundant_near_contact) &&
      previous_to_pair != NULL &&
      previous_to_pair->layer_num == pair_layer &&
      (same_layer_depth_cm <
         previous_to_pair->depth_cm - support_tol_cm ||
       crosses_redundant_near_contact);
    if (same_layer_packet_crossing_rise) {
      const double canonical_same_layer_depth_cm =
        lgarto_canonical_mobile_groundwater_depth_cm(
          same_layer_depth_cm, num_layers, cum_layer_thickness_cm);
      double packet_crossing_vadose_psi_cm = vadose_side_front->psi_cm;
      for (struct wetting_front *scan = *head;
           scan != NULL && scan != vadose_side_front; scan = scan->next) {
        if (scan->is_WF_GW && !scan->to_bottom &&
            scan->layer_num == pair_layer &&
            scan->depth_cm >
              canonical_same_layer_depth_cm + MBAL_ITERATIVE_TOLERANCE &&
            std::isfinite(scan->psi_cm)) {
          packet_crossing_vadose_psi_cm = scan->psi_cm;
          break;
        }
      }
      struct wetting_front *trial_head = listCopy(*head, NULL);
      if (trial_head != NULL) {
        const bool rebuilt_trial =
          lgarto_rebuild_mobile_groundwater_storage_pair_and_submerge_at_depth(
            canonical_same_layer_depth_cm, num_layers, cum_layer_thickness_cm,
            soil_type, frozen_factor, &trial_head, soil_properties,
            packet_crossing_vadose_psi_cm);
        const double trial_storage_cm =
          rebuilt_trial
            ? lgarto_mobile_groundwater_CR_storage_cm(num_layers,
                                                      cum_layer_thickness_cm,
            ...
            : NAN;
        listDelete(trial_head);
        const double trial_storage_tol_cm =
          fmax(1.0e-8, 1.0e-8 * fmax(1.0, fabs(target_storage_cm)));
        if (rebuilt_trial &&
            std::isfinite(trial_storage_cm) &&
            fabs(trial_storage_cm - target_storage_cm) <=
              trial_storage_tol_cm &&
            lgarto_rebuild_mobile_groundwater_storage_pair_and_submerge_at_depth(
              canonical_same_layer_depth_cm, num_layers,
              cum_layer_thickness_cm, soil_type, frozen_factor, head,
              soil_properties, packet_crossing_vadose_psi_cm)) {
          if (updated_groundwater_depth_cm != NULL) {
            *updated_groundwater_depth_cm = canonical_same_layer_depth_cm;
          }
          const double storage_after_submerge_cm =
            lgarto_mobile_groundwater_CR_storage_cm(num_layers,
                                                    cum_layer_thickness_cm,
                                                    soil_type, *head,
                                                    soil_properties);
          return storage_after_submerge_cm - storage_before_cm;
        }
      }
    }
    if (same_layer_depth_cm >= same_layer_shallow_depth_cm - support_tol_cm &&
        same_layer_depth_cm <= same_layer_deep_depth_cm + support_tol_cm) {
      const double bounded_same_layer_depth_cm =
        lgarto_canonical_mobile_groundwater_depth_cm(
          fmin(same_layer_deep_depth_cm,
               fmax(same_layer_shallow_depth_cm, same_layer_depth_cm)),
          num_layers, cum_layer_thickness_cm);
      if (lgarto_move_mobile_groundwater_storage_pair_to_depth(
            bounded_same_layer_depth_cm, num_layers,
            cum_layer_thickness_cm, *head, start_depth_cm)) {
        (void)
          lgarto_maintain_mobile_groundwater_support(
            bounded_same_layer_depth_cm, num_layers, cum_layer_thickness_cm,
            soil_type, frozen_factor, head, soil_properties);
        if (updated_groundwater_depth_cm != NULL) {
          *updated_groundwater_depth_cm = bounded_same_layer_depth_cm;
        }
        const double storage_after_direct_cm =
          lgarto_mobile_groundwater_CR_storage_cm(num_layers,
                                                  cum_layer_thickness_cm,
                                                  soil_type, *head,
                                                  soil_properties);
        return storage_after_direct_cm - storage_before_cm;
      }
    }
  }

  const double storage_at_shallow_cm =
    lgarto_trial_CR_storage_after_mobile_groundwater_storage_pair_depth(
      pair_shallow_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
      frozen_factor, *head, soil_properties);
  const double storage_at_deep_cm =
    lgarto_trial_CR_storage_after_mobile_groundwater_storage_pair_depth(
      pair_deep_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
      frozen_factor, *head, soil_properties);
  if (!std::isfinite(storage_at_shallow_cm) ||
      !std::isfinite(storage_at_deep_cm) ||
      storage_at_shallow_cm < storage_at_deep_cm) {
    return 0.0;
  }

  double selected_depth_cm = start_depth_cm;
  if (target_storage_cm >= storage_at_shallow_cm) {
    selected_depth_cm = pair_shallow_depth_cm;
  }
  else if (target_storage_cm <= storage_at_deep_cm) {
    selected_depth_cm = pair_deep_depth_cm;
  }
  else {
    double lo_depth_cm = pair_shallow_depth_cm;
    ...
    for (int iter = 0; iter < 80; iter++) {
      const double mid_depth_cm = 0.5 * (lo_depth_cm + hi_depth_cm);
      const double mid_storage_cm =
        lgarto_trial_CR_storage_after_mobile_groundwater_storage_pair_depth(
          mid_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
          frozen_factor, *head, soil_properties);
      if (!std::isfinite(mid_storage_cm)) {
        break;
      }

      const double mid_error_cm = target_storage_cm - mid_storage_cm;
      if (std::fabs(mid_error_cm) < best_error_cm) {
        best_error_cm = std::fabs(mid_error_cm);
        best_depth_cm = mid_depth_cm;
      }
      ...
      if (mid_storage_cm < target_storage_cm) {
        hi_depth_cm = mid_depth_cm;
      }
      else {
        lo_depth_cm = mid_depth_cm;
  ...
  selected_depth_cm =
    lgarto_canonical_mobile_groundwater_depth_cm(selected_depth_cm,
                                                 num_layers,
                                                 cum_layer_thickness_cm);

  bool rebuild_support_pair = false;
  const double selected_trial_storage_cm =
    lgarto_trial_CR_storage_after_mobile_groundwater_storage_pair_depth(
      selected_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
      frozen_factor, *head, soil_properties);
  double selected_storage_error_cm =
    std::isfinite(selected_trial_storage_cm)
      ? std::fabs(selected_trial_storage_cm - target_storage_cm)
      : HUGE_VAL;
  if (lgarto_depth_from_bottom_saturated_CR_storage_cm(
        target_storage_cm, num_layers, cum_layer_thickness_cm, soil_type,
        soil_properties, &rebuild_depth_cm) &&
    ...
    if (trial_head != NULL) {
      (void)
        lgarto_rebuild_mobile_groundwater_storage_pair_at_depth(
          rebuild_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
          frozen_factor, &trial_head, soil_properties);
      const double rebuild_trial_storage_cm =
        lgarto_mobile_groundwater_CR_storage_cm(num_layers,
                                                ...
                                                soil_type, trial_head,
                                                soil_properties);
      const double rebuild_storage_error_cm =
        std::isfinite(rebuild_trial_storage_cm)
          ? std::fabs(rebuild_trial_storage_cm - target_storage_cm)
          : HUGE_VAL;
      if (rebuild_storage_error_cm + MBAL_ITERATIVE_TOLERANCE <
          selected_storage_error_cm) {
        rebuild_support_pair = true;
        selected_storage_error_cm = rebuild_storage_error_cm;
      }
      listDelete(trial_head);
    }
  }

  if (rebuild_support_pair) {
    selected_depth_cm = rebuild_depth_cm;
    (void)
      lgarto_rebuild_mobile_groundwater_storage_pair_at_depth(
        selected_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
        frozen_factor, head, soil_properties);
  ...
  const double storage_after_cm =
    lgarto_mobile_groundwater_CR_storage_cm(num_layers, cum_layer_thickness_cm,
                                            soil_type, *head, soil_properties,
                                            selected_depth_cm);
  const double actual_CR_storage_change_cm = storage_after_cm - storage_before_cm;
  if (!std::isfinite(actual_CR_storage_change_cm)) {
    fprintf(stderr,
            "Error: mobile groundwater CR-storage solve produced non-finite storage change.\n"
            "  requested_CR_storage_change_cm=%.17g selected_depth_cm=%.17g\n"
            "  storage_before_cm=%.17g storage_after_cm=%.17g\n",
            requested_CR_storage_change_cm,
            selected_depth_cm,
            storage_before_cm,
            storage_after_cm);
    fflush(stderr);
  ...
  return actual_CR_storage_change_cm;
}

extern double lgarto_sync_mobile_groundwater_support_to_CR_storage(double target_CR_storage_cm,
								   double current_groundwater_depth_cm,
								   int num_layers,
								   double *cum_layer_thickness_cm,
								   int *soil_type,
								   ...
								   double *explicit_mass_change_cm)
{
  ...
  if (explicit_mass_change_cm != NULL) {
    *explicit_mass_change_cm = 0.0;
  }

  if (head == NULL || *head == NULL || num_layers <= 0 ||
      cum_layer_thickness_cm == NULL || soil_type == NULL ||
      soil_properties == NULL || !std::isfinite(target_CR_storage_cm) ||
      !std::isfinite(current_groundwater_depth_cm)) {
    return 0.0;
  }
  ...
  const double mass_before_cm =
    lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
  const double requested_CR_storage_change_cm =
    fmax(0.0, target_CR_storage_cm) - storage_before_cm;

  double solved_groundwater_depth_cm = current_groundwater_depth_cm;
  const double support_sync_CR_storage_change_cm =
    lgarto_move_mobile_groundwater_support_for_CR_storage_change(
      requested_CR_storage_change_cm,
      current_groundwater_depth_cm,
      num_layers,
      cum_layer_thickness_cm,
  ...
  // CR sync runs after the normal correction loops; canonicalize any connected
  // surface/GW boundary contact before the next substep treats it as surface.
  lgarto_convert_surface_fronts_drier_than_TO_below(head,
                                                    "mobile groundwater CR sync");
  const double storage_after_cm =
    lgarto_mobile_groundwater_CR_storage_cm(num_layers, cum_layer_thickness_cm,
                                            soil_type, *head, soil_properties,
                                            solved_groundwater_depth_cm);
  const double actual_CR_storage_change_cm =
    storage_after_cm - storage_before_cm;
  const double mass_after_cm =
    lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
  if (explicit_mass_change_cm != NULL) {
    *explicit_mass_change_cm = mass_after_cm - mass_before_cm;
  }
  if (updated_groundwater_depth_cm != NULL) {
    *updated_groundwater_depth_cm = solved_groundwater_depth_cm;
  }

  if (!std::isfinite(support_sync_CR_storage_change_cm) ||
      !std::isfinite(actual_CR_storage_change_cm) ||
...
extern double lgarto_rewet_receded_groundwater_zone(double previous_groundwater_depth_cm,
						    ...
						    struct wetting_front **head,
						    struct soil_properties_ *soil_properties,
						    double max_rewet_storage_cm)
{
  if (head == NULL || *head == NULL || num_layers <= 0 ||
      cum_layer_thickness_cm == NULL || soil_type == NULL ||
      soil_properties == NULL ||
      !std::isfinite(previous_groundwater_depth_cm) ||
      !std::isfinite(groundwater_depth_cm) ||
      !std::isfinite(max_rewet_storage_cm) ||
      max_rewet_storage_cm <= 0.0) {
    return 0.0;
  }
  ...
  const double depth_tol_cm =
    fmax(MOBILE_GROUNDWATER_SUBMERGENCE_TOL_CM,
         1.0e-10 * fmax(1.0, fmax(fixed_column_depth_cm,
                                   fmax(previous_groundwater_depth_cm,
                                        groundwater_depth_cm))));
  if (groundwater_depth_cm <= previous_groundwater_depth_cm + depth_tol_cm) {
    return 0.0;
  }

  const double rewet_top_cm =
    fmax(0.0, fmin(previous_groundwater_depth_cm, fixed_column_depth_cm));
  const double rewet_bottom_cm =
    fmax(0.0, fmin(groundwater_depth_cm, fixed_column_depth_cm));
  if (rewet_bottom_cm <= rewet_top_cm + depth_tol_cm) {
    return 0.0;
  }

  struct lgarto_recession_rewet_candidate
  {
    struct wetting_front *front;
    double original_theta;
    double delta_theta;
    double interval_length_cm;
    double overlap_length_cm;
    double target_psi_cm;
  };

  std::vector<lgarto_recession_rewet_candidate> candidates;
  std::vector<double> previous_depth_by_layer(num_layers + 1, 0.0);
  for (int layer = 1; layer <= num_layers; layer++) {
    previous_depth_by_layer[layer] = cum_layer_thickness_cm[layer - 1];
  }

  double requested_storage_cm = 0.0;
  for (struct wetting_front *current = *head; current != NULL;
       current = current->next) {
    if (current->to_bottom == TRUE ||
        current->is_WF_GW == FALSE ||
      ...
      if (current != NULL && current->layer_num >= 1 &&
          current->layer_num <= num_layers &&
          std::isfinite(current->depth_cm)) {
        previous_depth_by_layer[current->layer_num] = current->depth_cm;
      }
      continue;
    }

    const int layer_num = current->layer_num;
    const double interval_top_cm = previous_depth_by_layer[layer_num];
    const double interval_bottom_cm = current->depth_cm;
    previous_depth_by_layer[layer_num] = interval_bottom_cm;
    if (!std::isfinite(interval_top_cm) ||
        interval_bottom_cm <= interval_top_cm + depth_tol_cm) {
      continue;
    }

    const double overlap_top_cm = fmax(interval_top_cm, rewet_top_cm);
    const double overlap_bottom_cm = fmin(interval_bottom_cm, rewet_bottom_cm);
    const double overlap_length_cm = overlap_bottom_cm - overlap_top_cm;
    if (overlap_length_cm <= depth_tol_cm) {
      continue;
    }

    const double overlap_midpoint_cm = 0.5 * (overlap_top_cm + overlap_bottom_cm);
    double target_psi_cm = fmax(0.0, groundwater_depth_cm - overlap_midpoint_cm);
    if (current->is_WF_GW == TRUE &&
        current->next != NULL &&
        current->next->is_WF_GW == TRUE &&
        current->next->layer_num == current->layer_num &&
        std::isfinite(current->next->psi_cm)) {
      target_psi_cm = fmax(target_psi_cm, current->next->psi_cm);
    }
    const int soil_num = soil_type[layer_num];
    const double theta_e = soil_properties[soil_num].theta_e;
    const double theta_r = soil_properties[soil_num].theta_r;
    ...
    const double vg_m = soil_properties[soil_num].vg_m;
    const double vg_n = soil_properties[soil_num].vg_n;
    if (!std::isfinite(theta_e) || !std::isfinite(theta_r) ||
        theta_e <= theta_r) {
      continue;
    }

    const double target_theta =
      calc_theta_from_h(target_psi_cm, vg_a, vg_m, vg_n, theta_e, theta_r);
    if (!std::isfinite(target_theta) ||
        target_theta <= current->theta + 1.0e-12) {
      continue;
    }

    const double interval_length_cm = interval_bottom_cm - interval_top_cm;
    double delta_theta =
      (target_theta - current->theta) * (overlap_length_cm / interval_length_cm);
    if (current->theta + delta_theta > theta_e) {
      delta_theta = theta_e - current->theta;
    }
    if (delta_theta <= 0.0) {
      continue;
    }

    candidates.push_back({current, current->theta, delta_theta,
                          interval_length_cm, overlap_length_cm,
                          target_psi_cm});
    requested_storage_cm += interval_length_cm * delta_theta;
  }

  if (candidates.empty() || requested_storage_cm <= 0.0) {
    return 0.0;
  }

  const double scale =
    fmax(0.0, fmin(1.0, max_rewet_storage_cm / requested_storage_cm));
  if (scale <= 0.0) {
    return 0.0;
  }
  ...
  for (size_t i = 0; i < candidates.size(); i++) {
    candidates[i].front->theta =
      candidates[i].original_theta + scale * candidates[i].delta_theta;
    lgar_refresh_front_state_from_theta(candidates[i].front, soil_type,
                                        frozen_factor, soil_properties);
  }

  const double mass_after_cm = lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
  const double rewet_storage_cm = mass_after_cm - mass_before_cm;
  if (!std::isfinite(rewet_storage_cm)) {
    fprintf(stderr,
            "Error: mobile groundwater recession rewet produced non-finite storage.\n"
            "  previous_groundwater_depth_cm=%.17g groundwater_depth_cm=%.17g\n"
            "  Wetting front list follows:\n",
            previous_groundwater_depth_cm,
            groundwater_depth_cm);
    fflush(stderr);
    listPrint(*head);
    fflush(stdout);
  ...
  if (verbosity.compare("high") == 0 && rewet_storage_cm > 0.0) {
    printf("Mobile groundwater recession rewet %.17lf cm over %.17lf-%.17lf cm "
           "after GW receded from %.17lf to %.17lf cm, capped by %.17lf cm CR storage.\n",
           rewet_storage_cm,
           rewet_top_cm,
           rewet_bottom_cm,
           previous_groundwater_depth_cm,
           groundwater_depth_cm,
           max_rewet_storage_cm);
  }

  return fmax(0.0, rewet_storage_cm);
}
							      ...
							      struct soil_properties_ *soil_properties)
{
  ...
  while (changed && iterations < MAX_NUM_WETTING_FRONTS) {
    ...
    for (int wf = listLength(*head); wf >= 1; wf--) {
      struct wetting_front *current = listFindFront(wf, *head, NULL);
      if (current == NULL ||
          !std::isfinite(current->depth_cm) ||
          current->depth_cm <= ZERO_DEPTH_TO_DELETE_DEPTH_TOL_CM) {
      ...
      if (current->to_bottom == TRUE) {
        ...
        const int converted_front_num = current->front_num;
        const double converted_depth_cm = current->depth_cm;
        const double original_psi_cm = current->psi_cm;
        const double operation_mass_before_cm =
          lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
        ...
        if (recipient != NULL &&
            ...
            recipient->layer_num != current->layer_num &&
            std::isfinite(recipient->psi_cm)) {
          lgarto_set_front_hydraulic_state_from_psi(current, recipient->psi_cm,
                                                    soil_type, frozen_factor,
                                                    soil_properties);
        }
        current->is_WF_GW = TRUE;
        ...
        if (verbosity.compare("high") == 0) {
          const double operation_flux_cm =
            ...
            lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
          printf("Mobile groundwater converted submerged to_bottom front %d "
                 "at %.17lf cm below GW depth %.17lf cm from surface to GW "
                 "scaffold; psi %.17lf -> %.17lf cm; routed %.17lf cm to "
                 "mobile groundwater exchange.\n",
                 converted_front_num,
                 converted_depth_cm,
                 groundwater_depth_cm,
        ...
        const double layer_base_depth_cm =
          cum_layer_thickness_cm[current->layer_num - 1];
        ...
        if (groundwater_depth_cm <= layer_base_depth_cm + submerged_depth_tol_cm) {
          struct lgarto_submerge_saved_to_bottom_state
          {
            struct wetting_front *front;
            bool is_WF_GW;
            double theta;
          ...
          std::vector<lgarto_submerge_saved_to_bottom_state> to_bottom_states;
          for (struct wetting_front *snap = *head; snap != NULL; snap = snap->next) {
            if (snap->to_bottom &&
                snap->depth_cm < groundwater_depth_cm - submerged_depth_tol_cm) {
              to_bottom_states.push_back({
                snap,
                snap->is_WF_GW,
                snap->theta,
          ...
          if (verbosity.compare("high") == 0) {
            const double operation_flux_cm =
              operation_mass_before_cm -
              lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
            printf("Mobile groundwater deleted submerged surface front %d at "
                   "%.17lf cm below GW depth %.17lf cm; routed %.17lf cm "
                   "to mobile groundwater exchange.\n",
                   deleted_front_num,
                   deleted_depth_cm,
                   groundwater_depth_cm,
                   operation_flux_cm);
        ...
        const int clipped_front_num = current->front_num;
        const double original_depth_cm = current->depth_cm;
        const double operation_mass_before_cm =
          lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
        current->depth_cm = groundwater_depth_cm;
        current->is_WF_GW = TRUE;
        current->dzdt_cm_per_h = 0.0;
        for (struct wetting_front *scan = current->next;
             scan != NULL && scan->layer_num == current->layer_num;
             scan = scan->next) {
          if (scan->is_WF_GW || scan->to_bottom ||
              scan->depth_cm <= groundwater_depth_cm + submerged_depth_tol_cm) {
            continue;
          }
          // Deeper surface fronts overtaken by the same water-table rise are saturated.
          scan->is_WF_GW = TRUE;
          scan->dzdt_cm_per_h = 0.0;
          lgarto_set_front_hydraulic_state_from_psi(scan, 0.0, soil_type,
                                                    frozen_factor,
                                                    soil_properties);
        }
        const double operation_flux_cm =
        ...
        if (verbosity.compare("high") == 0) {
          printf("Mobile groundwater converted surface front %d from %.17lf cm "
                 "to TO/GW at GW depth %.17lf cm.\n",
                 clipped_front_num,
                 original_depth_cm,
                 groundwater_depth_cm);
        }
        ...
        break;
	      ...
	      if (groundwater_depth_cm <= layer_base_depth_cm + submerged_depth_tol_cm) {
	        /* The GW table has risen above this layer; the layer's saturated
	           to_bottom scaffold carries the storage, so remove stale interior
	           TO/GW supports instead of forming a false boundary pair. */
	        const int deleted_front_num = current->front_num;
	        const int deleted_layer_num = current->layer_num;
	        const double deleted_depth_cm = current->depth_cm;
	        const double operation_mass_before_cm =
	          lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
	        (void) listDeleteFront(deleted_front_num, head, soil_type, soil_properties);
	        ...
	        if (verbosity.compare("high") == 0) {
	          const double operation_flux_cm =
	            operation_mass_before_cm -
	            lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
	          printf("Mobile groundwater deleted submerged TO/GW front %d at "
	                 "%.17lf cm because GW depth %.17lf cm is above layer %d; "
	                 "routed %.17lf cm to mobile groundwater exchange.\n",
	                 deleted_front_num,
	                 deleted_depth_cm,
	                 groundwater_depth_cm,
	                 deleted_layer_num,
  ...
  bool restore_missing_support = false;
  if (!repaired && !lgarto_find_mobile_groundwater_storage_pair(
        *head, submerged_depth_tol_cm, NULL, NULL, groundwater_depth_cm)) {
    for (struct wetting_front *front = *head; front != NULL; front = front->next) {
      // A saturated orphan at the scalar depth is a damaged support pair;
      // an unsaturated front merely approaching that depth is still moving.
      if (front->is_WF_GW && !front->to_bottom &&
          std::fabs(front->depth_cm - groundwater_depth_cm) <= submerged_depth_tol_cm &&
          front->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM) {
        restore_missing_support = true;
        break;
      }
    }
  }
  if (repaired) {
    // Submergence can expose connected layer-bottom scaffolds above the GW chain.
    (void) lgarto_convert_surface_fronts_drier_than_TO_below(
      head, "mobile groundwater submergence repair");
  }
  if (repaired || restore_missing_support) {
    // Restore the pair after submergence edits or an orphaned saturated support.
    (void) lgarto_maintain_mobile_groundwater_support(
      groundwater_depth_cm, num_layers, cum_layer_thickness_cm, soil_type,
      frozen_factor, head, soil_properties);
  }
				...
				bool mobile_groundwater_level)
{
  ...
  if (current_free_drainage == NULL) {
    fprintf(stderr,
            "Error: infiltration/free-drainage selector returned a missing front.\n"
            "  requested_front=%d number_of_wetting_fronts=%d mobile_groundwater_level=%d\n",
            wf_that_supplies_free_drainage_demand,
            number_of_wetting_fronts,
            mobile_groundwater_level ? 1 : 0);
    fflush(stderr);
    listPrint(head);
    fflush(stdout);
    abort();
  ...
  if (number_of_wetting_fronts == num_layers || saturated_terminal_TO_column) {
    ...
    if (!terminal_all_TO_free_drainage && mobile_groundwater_level &&
        ...
        current_free_drainage_next->layer_num != layer_num_fp &&
        std::isfinite(current_free_drainage_next->psi_cm)) {
      theta_below = calc_theta_from_h(current_free_drainage_next->psi_cm,
                                      vg_a, vg_m, vg_n, theta_e, theta_r);
    }
  ...
  // A negative storage-limited infiltration capacity means the profile has no
  // room to accept matrix infiltration in this substep. Route available surface
  // water to runoff rather than creating negative infiltration bookkeeping.
  if (!std::isfinite(f_p) || f_p < 0.0) {
    f_p = 0.0;
  }
                                                ...
                                                struct soil_properties_ *soil_properties)
{
  ...
  const double Se = calc_Se_from_theta(front->theta, theta_e, theta_r);
  if (!lgar_TO_psi_matches_theta(front, soil_type, soil_properties)) {
...
static void lgar_refresh_all_front_states_from_theta(struct wetting_front *head,
                                                     ...
                                                     struct soil_properties_ *soil_properties)
{
  const std::vector<lgar_preserved_boundary_psi_state> preserved_boundary_psi_states =
    lgar_collect_lossless_boundary_psi_states(head, soil_type, soil_properties);
  for (struct wetting_front *current = head; current != NULL; current = current->next) {
    lgar_refresh_front_state_from_theta(current, soil_type, frozen_factor, soil_properties);
    // Preserve exact shared boundary psi when the capped theta inverse is non-unique.
    (void) lgar_restore_lossless_boundary_psi(current, preserved_boundary_psi_states);
  }
  ...
  for (auto boundary_it = boundary_upper_fronts.rbegin();
       boundary_it != boundary_upper_fronts.rend();
       ++boundary_it) {
    struct wetting_front *current = *boundary_it;
    struct wetting_front *next = current->next;
    if (!lgar_boundary_psi_snap_candidate(current, next, soil_type, soil_properties)) {
      continue;
    }
...
static bool lgar_created_surface_front_can_fold_into_connected_gw_chain(struct wetting_front *repair_target,
                                                                        ...
                                                                        struct soil_properties_ *soil_properties)
{
  if (repair_target == NULL || repair_target->next == NULL || head == NULL ||
      cum_layer_thickness_cm == NULL) {
    return false;
  }

  struct wetting_front *chain_head = repair_target->next;
  if (!chain_head->is_WF_GW) {
    return false;
  }
  ...
  if (downstream_gw_structure_is_saturated || hydraulically_redundant_with_chain_head) {
    return true;
  }

  const double layer_top_cm = cum_layer_thickness_cm[repair_target->layer_num - 1];
  const double differential_storage_cm =
    std::fabs(repair_target->theta - chain_head->theta) *
    fmax(0.0, repair_target->depth_cm - layer_top_cm);
  if (repair_target->layer_num != 1 || chain_head->psi_cm <= 1.0e-8 ||
      differential_storage_cm > SURFACE_CREATION_FOLD_MASS_TOLERANCE_CM) {
    return false;
  }

  struct wetting_front *trial_head = listCopy(head, NULL);
  struct wetting_front *trial_chain =
    listDeleteFront(repair_target->front_num, &trial_head, soil_type, soil_properties);
  // Keep the tiny rainfall mass on the connected TO chain instead of retaining a numerical-resolution front.
  const double trial_residual_cm = lgar_restore_surface_creation_mass_on_connected_gw_chain(
    trial_chain, target_mass, &trial_head, cum_layer_thickness_cm,
    soil_type, soil_properties);
  const bool can_fold = std::fabs(trial_residual_cm) <= MBAL_ITERATIVE_TOLERANCE;
  listDelete(trial_head);
  return can_fold;
}
                                                                                            ...
                                                                                            struct soil_properties_ *soil_properties)
{
  ...
  const bool created_surface_not_drier_than_next =
    !repair_target->is_WF_GW &&
    !repair_target->to_bottom &&
    repair_target->theta >= repair_target->next->theta - 1.0e-10;
  const bool created_surface_not_drier =
    created_surface_not_drier_than_next && repair_target->next->to_bottom;
  ...
  const bool can_fold_into_gw_chain =
    created_surface_not_drier && repair_target->next->is_WF_GW && same_layer_and_depth &&
    lgar_created_surface_front_can_fold_into_connected_gw_chain(repair_target, target_mass, *head,
                                                                cum_layer_thickness_cm, soil_type,
                                                                soil_properties);
  const bool can_fold_saturated_surface_into_gw_packet =
    created_surface_not_drier_than_next && repair_target->next->is_WF_GW &&
    repair_target->layer_num == repair_target->next->layer_num &&
  ...
  if (can_fold_into_surface_chain || can_fold_into_gw_chain ||
      can_fold_saturated_surface_into_gw_packet) {
    if (verbosity.compare("high") == 0) {
      printf("surface creation folding redundant surface front %d into connected chain below "
             "because it is storage-neutral with front %d (depth %.15f cm, theta %.15f; "
             "GW=%d, to_bottom=%d, next theta=%.15f, psi=%.15f).\n",
             repair_target->front_num, repair_target->next->front_num, repair_target->depth_cm,
             repair_target->theta,
  ...
  struct soil_properties_ *soil_properties)
{
  ...
  for (struct wetting_front *candidate = *head;
    ...
    lgar_theta_mass_balance_correction_with_min_psi(
      false, candidate->front_num, target_mass, head,
      cum_layer_thickness_cm, soil_type, soil_properties, min_psi_cm);
...
{
  double residual_cm = target_mass - lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
  const int max_passes = listLength(*head);
  for (int pass = 0; pass < max_passes &&
                         residual_cm > MBAL_ITERATIVE_TOLERANCE; pass++) {
    const double prior_abs_residual_cm = fabs(residual_cm);
    /* Retain the best chain update from each pass so downstream wetting can
       release the pressure-head bound on an upstream chain. */
    residual_cm = lgarto_restore_mass_in_available_TO_storage(
      target_mass, num_layers, cum_layer_thickness_cm, soil_type, head,
      soil_properties);
    if (fabs(residual_cm) + 1.0e-12 >= prior_abs_residual_cm) {
      break;
    }
  }
  return residual_cm;
                                                     ...
                                                     struct soil_properties_ *soil_properties)
{
  ...
  if (correction_type_surf == 6) {
    ...
    lgarto_convert_surface_fronts_drier_than_TO_below(
      head, "surface creation mixed surface/surface/TO correction");
  }
...
{
  double lower_boundary_flux_correction_cm = 0.0;
  // During surface creation, internal TO residuals can be intermediate; let
  // the pending creation repair/canonicalization absorb them before routing.
  auto defer_internal_TO_residual = [&]() {
    return lgarto_has_out_of_layer_movable_TO_front(num_layers, cum_layer_thickness_cm, *head) ||
	  ...
	  if (correction_type == 2) {
	    const double mass_balance_flux_correction_cm =
	      lgarto_TO_WFs_merge_via_depth(*target_mass, column_depth, cum_layer_thickness_cm, head,
	                                    soil_type, soil_properties, mobile_groundwater_level,
	                                    surface_lower_boundary_depth_cm);
    if (defer_internal_TO_residual()) {
      if (verbosity.compare("high") == 0 &&
          std::fabs(mass_balance_flux_correction_cm) > MBAL_ITERATIVE_TOLERANCE) {
        printf("surface creation deferred TO depth-merge residual %.12e cm "
               "until creation repair/canonicalization finishes.\n",
               mass_balance_flux_correction_cm);
      }
    }
    else {
  ...
  if (correction_type == 4) {
    ...
    if (defer_internal_TO_residual()) {
      if (verbosity.compare("high") == 0 &&
          std::fabs(mass_balance_flux_correction_cm) > MBAL_ITERATIVE_TOLERANCE) {
        printf("surface creation deferred TO layer-crossing residual %.12e cm "
               "until creation repair/canonicalization finishes.\n",
               mass_balance_flux_correction_cm);
      }
    }
                                                        ...
                                                        bool mobile_groundwater_level)
{
  ...
  if (created_front_joins_TO_packet) {
    ...
    if (lgar_reset_overtaken_groundwater_fronts_to_surface(*head, new_surface_front)) {
      // Zero-depth TO supports are metadata; reorder them before correction tests for crossings.
      listSendToTop(*head);
    }
  }
  ...
  for (int phase = 0; phase < 8; phase++) {
      ...
      if (correction_type != 0) {
        ...
        const double lower_boundary_flux_correction_cm =
          lgar_apply_creation_general_correction(correction_type, num_layers, &target_mass, column_depth,
                                                 surface_lower_boundary_depth_cm, cum_layer_thickness_cm,
                                                 soil_type, frozen_factor, head,
                                                 soil_properties, mobile_groundwater_level);
        if (creation_excess_gw_flux_cm != NULL) {
          *creation_excess_gw_flux_cm += lower_boundary_flux_correction_cm;
        }
        continue;
    ...
    repair_target = lgar_fold_redundant_created_gw_front_into_connected_chain(repair_target, head,
                                                                              cum_layer_thickness_cm,
                                                                              soil_type, soil_properties);
    repair_target = lgar_fold_redundant_created_surface_front_into_connected_chain(
      repair_target, head, target_mass, cum_layer_thickness_cm, soil_type, soil_properties);
    if (repair_target == NULL) {
      break;
    }
      ...
      if (post_repair_correction_type != 0) {
        const double lower_boundary_flux_correction_cm =
          lgar_apply_creation_general_correction(post_repair_correction_type, num_layers, &target_mass,
                                                 ...
                                                 cum_layer_thickness_cm, soil_type, frozen_factor,
                                                 head, soil_properties, mobile_groundwater_level);
        if (creation_excess_gw_flux_cm != NULL) {
          *creation_excess_gw_flux_cm += lower_boundary_flux_correction_cm;
        }
        continue;
  ...
  lgar_refresh_all_front_states_from_theta(*head, cum_layer_thickness_cm,
                                           soil_type, frozen_factor, soil_properties);
}
...
static double lgar_dual_permeability_normalize_theta_roundoff(
  double theta, const struct soil_properties_& soil)
{
  const double tolerance = 64.0 * DBL_EPSILON;
  if (!std::isfinite(theta) || theta < soil.theta_r - tolerance ||
      theta > soil.theta_e + tolerance) {
    throw runtime_error(
      "Accumulated dual-permeability exchange exceeded hydraulic bounds.");
  }
  return fmax(soil.theta_r, fmin(soil.theta_e, theta));
}
...
static bool lgar_delete_negligible_same_layer_surface_front(
  double tolerance_cm, double *cum_layer_thickness_cm,
  int *layer_soil_type, struct soil_properties_ *soil_properties,
  struct wetting_front **head)
{
  for (struct wetting_front *front = *head;
       front != NULL && front->next != NULL; front = front->next) {
    struct wetting_front *next = front->next;
    if (front->is_WF_GW || next->is_WF_GW || front->to_bottom ||
        front->layer_num != next->layer_num || front->depth_cm > next->depth_cm ||
        fabs(front->theta - next->theta) > THRESHOLD_NO_MOISTURE_DIFF ||
        fabs(front->psi_cm - next->psi_cm) >
          lgar_psi_assertion_tolerance_cm(front->psi_cm, next->psi_cm)) {
      continue;
    }

    const double layer_top_cm = cum_layer_thickness_cm[front->layer_num - 1];
    const double storage_contrast_cm =
      (front->depth_cm - layer_top_cm) * (front->theta - next->theta);
    if (fabs(storage_contrast_cm) <= tolerance_cm) {
      // Retain a to_bottom scaffold while deleting the hydraulically empty front above it.
      (void) listDeleteFront(front->front_num, head, layer_soil_type, soil_properties);
      return true;
    }
  }
  return false;
  ...
  const char *context)
{
  ...
  for (int pass = 0; pass < max_passes; pass++) {
    ...
    const int front_count_before_TO_cleanup = listLength(*head);
    lgar_delete_zero_depth_groundwater_supports_causing_same_layer_TO_psi_reversal(
      head, layer_soil_type, soil_properties, cum_layer_thickness_cm);
    if (listLength(*head) != front_count_before_TO_cleanup) {
      continue;
    }
    if (lgar_delete_negligible_same_layer_surface_front(
          tolerance_cm, cum_layer_thickness_cm, layer_soil_type,
          soil_properties, head)) {
  ...
  struct wetting_front **head_fracture)
{
  ...
  for (std::size_t index = 0; index + 1 < unique_depths.size(); index++) {
    ...
    // Endpoints are already deduplicated; strict midpoint ownership prevents
    // tolerance-sized neighboring intervals from entering microscopic blocks.
    ...
    const double exchange_start_depth_cm =
      fmax(start_depth_cm,
           fmax(matrix_block.start_depth_cm, fracture_block.start_depth_cm));
    const double exchange_end_depth_cm =
      fmin(end_depth_cm,
           fmin(matrix_block.end_depth_cm, fracture_block.end_depth_cm));
    if (!lgar_dual_permeability_depths_close(
          exchange_start_depth_cm, start_depth_cm) ||
        !lgar_dual_permeability_depths_close(
          exchange_end_depth_cm, end_depth_cm)) {
      throw runtime_error(
        "Dual-permeability exchange region materially exceeds its owning block.");
    }
    // Deduplicated endpoints can extend slightly beyond one domain's block.
    const double exchange_thickness_cm =
      exchange_end_depth_cm - exchange_start_depth_cm;
    if (exchange_thickness_cm <= 0.0) continue;
    ...
    const double region_transfer_cm =
      region_exchange.transfer_water_content * exchange_thickness_cm;
  ...
  for (lgar_dual_permeability_block& block : matrix_blocks) {
    const int soil = layer_soil_type[block.front->layer_num];
    const double updated_theta = block.front->theta -
      block.transfer_cm / (matrix_fraction * block.thickness_cm);
    // Capacity-limited arithmetic can land one ULP outside a hydraulic bound.
    block.front->theta = lgar_dual_permeability_normalize_theta_roundoff(
      updated_theta, soil_properties_matrix[soil]);
  }
  for (lgar_dual_permeability_block& block : fracture_blocks) {
    const int soil = layer_soil_type[block.front->layer_num];
    const double updated_theta = block.front->theta + block.transfer_cm /
      (ratio_fracture_vol_to_total_vol * block.thickness_cm);
    block.front->theta = lgar_dual_permeability_normalize_theta_roundoff(
      updated_theta, soil_properties_fracture[soil]);
  }
  for (lgar_dual_permeability_block& block : matrix_blocks) {
    const int soil = layer_soil_type[block.front->layer_num];
    block.front->psi_cm = lgar_dual_permeability_psi_from_theta(
                                             ...
                                             struct soil_properties_ *soil_properties)
{
  ...
  const double domain_bottom_cm = cum_layer_thickness_cm[num_layers];
  double storage_cm = 0.0;
  struct wetting_front *previous = NULL;

  for (struct wetting_front *current = head; current != NULL; current = current->next) {
    const int layer = current->layer_num;
    if (!current->is_WF_GW || layer < 1 || layer > num_layers) {
      previous = current;
      continue;
    }
    ...
    const double theta_e = soil_properties[soil].theta_e;
    const double theta_r = soil_properties[soil].theta_r;
    const bool saturated_front =
        current->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM ||
        current->theta >= theta_e - 1.0e-10;
    if (!saturated_front || theta_e <= theta_r) {
      previous = current;
      continue;
    }
    ...
    interval_top_cm = fmin(interval_top_cm, cum_layer_thickness_cm[layer]);
    double interval_bottom_cm = fmin(current->depth_cm, cum_layer_thickness_cm[layer]);
    interval_bottom_cm = fmin(interval_bottom_cm, domain_bottom_cm);
...
// ############################################################################################
static double lgarto_layer_resistance_at_psi(double thickness_cm,
                                             int layer_num,
                                             double psi_cm,
                                             int *soil_type,
                                             double *frozen_factor,
                                             struct soil_properties_ *soil_properties,
                                             double avoid_div_by_zero_factor)
{
  if (thickness_cm <= 0.0) {
    return 0.0;
  }

  int soil_num_loc = soil_type[layer_num];
  double theta_prev_loc = calc_theta_from_h(psi_cm,
                                            soil_properties[soil_num_loc].vg_alpha_per_cm,
                                            soil_properties[soil_num_loc].vg_m,
                                            soil_properties[soil_num_loc].vg_n,
                                            soil_properties[soil_num_loc].theta_e,
                                            soil_properties[soil_num_loc].theta_r);

  double Se_prev_loc = calc_Se_from_theta(theta_prev_loc,
                                          soil_properties[soil_num_loc].theta_e,
                                          soil_properties[soil_num_loc].theta_r);

  double K_cm_per_h_prev_loc = calc_K_from_Se(
      Se_prev_loc,
      soil_properties[soil_num_loc].Ksat_cm_per_h * frozen_factor[layer_num],
      soil_properties[soil_num_loc].vg_m);

  if (!std::isfinite(K_cm_per_h_prev_loc) || K_cm_per_h_prev_loc <= 0.0) {
    K_cm_per_h_prev_loc = avoid_div_by_zero_factor;
  }

  return thickness_cm / K_cm_per_h_prev_loc;
}

static double lgarto_integrated_resistance_to_depth(double path_top_cm,
                                                    double path_bottom_cm,
                                                    double psi_cm,
                                                    int num_layers,
                                                    int *soil_type,
                                                    double *cum_layer_thickness_cm,
                                                    ...
                                                    double avoid_div_by_zero_factor)
{
  if (!std::isfinite(path_top_cm) || !std::isfinite(path_bottom_cm) ||
      path_bottom_cm <= path_top_cm) {
    return 0.0;
  }

  double denominator = 0.0;
  const double top_cm = fmax(0.0, path_top_cm);
  const double bottom_cm = path_bottom_cm;

  for (int k = 1; k <= num_layers; k++) {
    const double layer_top_cm = cum_layer_thickness_cm[k - 1];
    const double layer_bottom_cm = cum_layer_thickness_cm[k];
    const double segment_top_cm = fmax(top_cm, layer_top_cm);
    const double segment_bottom_cm = fmin(bottom_cm, layer_bottom_cm);

    if (segment_bottom_cm > segment_top_cm) {
      denominator += lgarto_layer_resistance_at_psi(
          segment_bottom_cm - segment_top_cm, k, psi_cm, soil_type,
          frozen_factor, soil_properties, avoid_div_by_zero_factor);
    }
  }

  const double fixed_column_depth_cm = cum_layer_thickness_cm[num_layers];
  if (bottom_cm > fixed_column_depth_cm) {
    const double segment_top_cm = fmax(top_cm, fixed_column_depth_cm);
    if (bottom_cm > segment_top_cm) {
      denominator += lgarto_layer_resistance_at_psi(
          bottom_cm - segment_top_cm, num_layers, psi_cm, soil_type,
          frozen_factor, soil_properties, avoid_div_by_zero_factor);
    }
  }
				   ...
				   double groundwater_depth_cm, bool TO_only)
{
  ...
  do {  // loop through the wetting fronts
    ...
    if (current->is_WF_GW) {
      ...
      const double fixed_column_depth_cm = cum_layer_thickness_cm[num_layers];
      const double mobile_support_pair_tol_cm =
        fmax(MOBILE_GROUNDWATER_SUBMERGENCE_TOL_CM,
             1.0e-10 * fmax(1.0, fixed_column_depth_cm));
      const bool is_mobile_groundwater_support_vadose_side =
        !current->to_bottom &&
        next != NULL &&
        next->is_WF_GW &&
        !next->to_bottom &&
        current->layer_num == next->layer_num &&
        fabs(current->depth_cm - next->depth_cm) <= mobile_support_pair_tol_cm &&
        next->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM;

      if (is_mobile_groundwater_support_vadose_side) {
        dzdt = 0.0;
      }
        ...
        if (next_to_use == NULL) {
          ...
          denominator = lgarto_integrated_resistance_to_depth(
              current->depth_cm, D, next_to_use->psi_cm, num_layers, soil_type,
              cum_layer_thickness_cm, frozen_factor, soil_properties,
              avoid_div_by_zero_factor);
          ...
          const double left_path_top_cm = (previous != NULL) ? current->depth_cm : 0.0;
          denominator_left = lgarto_integrated_resistance_to_depth(
              left_path_top_cm, D, temp_psi, num_layers, soil_type,
              cum_layer_thickness_cm, frozen_factor, soil_properties,
              avoid_div_by_zero_factor);
                      ...
                      double *solved_psi_cm)
{
  ...
  if (delta_mass <= MBAL_ITERATIVE_TOLERANCE) {
    ...
    if (solved_psi_cm != NULL) *solved_psi_cm = psi_cm_loc;
    return theta;
  }
...
static struct wetting_front *lgarto_insert_missing_to_bottom_scaffold(int layer_num,
								      double *cum_layer_thickness_cm,
								      struct wetting_front **head,
								      struct wetting_front *reference_front)
{
  if (head == NULL || *head == NULL || cum_layer_thickness_cm == NULL || layer_num < 1) {
    return NULL;
  }

  for (struct wetting_front *front = *head; front != NULL; front = front->next) {
    if (front->to_bottom && front->layer_num == layer_num) {
      return front;
    }
  }
  ...
  struct wetting_front *previous = NULL;
  for (struct wetting_front *front = *head; front != NULL; front = front->next) {
    if (front->depth_cm > boundary_depth_cm + DEPTH_AVOIDS_SAME_WF_DEPTH) {
      break;
    }
    previous = front;
  }
  ...
  struct wetting_front *inserted =
    listInsertFront(boundary_depth_cm,
                    reference_front != NULL ? reference_front->theta : previous->theta,
                    previous->front_num + 1,
                    layer_num,
                    TRUE,
                    head);
  if (inserted == NULL) {
  ...
  inserted->depth_cm = boundary_depth_cm;
  inserted->to_bottom = TRUE;
  inserted->is_WF_GW = reference_front != NULL ? reference_front->is_WF_GW : previous->is_WF_GW;
  inserted->theta = reference_front != NULL ? reference_front->theta : previous->theta;
  inserted->psi_cm = reference_front != NULL ? reference_front->psi_cm : previous->psi_cm;
  inserted->K_cm_per_h = reference_front != NULL ? reference_front->K_cm_per_h : previous->K_cm_per_h;
  inserted->dzdt_cm_per_h = 0.0;
  return inserted;
}
					    ...
					    int *soil_type, struct soil_properties_ *soil_properties)
{
  ...
  for (int wf = 1; wf != listLength(*head); wf++) {
    ...
    if ((current->theta > next->theta) && current->is_WF_GW && next->is_WF_GW &&
      ...
      if (current->to_bottom) {
        const int scaffold_layer_num = current->layer_num;
        current = listDeleteFront(current->front_num, head, soil_type, soil_properties);
        if (current != NULL) {
          (void) lgar_restore_theta_merge_mass_via_depth(target_mass, column_depth,
                                                         cum_layer_thickness_cm, head,
                                                         current);
          (void) lgarto_insert_missing_to_bottom_scaffold(scaffold_layer_num,
                                                          cum_layer_thickness_cm,
                                                          head,
                                                          current);
        }
        ...
        if (next->to_bottom == FALSE && previous->to_bottom == FALSE) {
          ...
          if (current != NULL) {
            if (upstream_to_bottom_chain) {
              // The surviving lower front defines the exposed layer boundary after the bridge is deleted.
              lgar_restore_upstream_to_bottom_chain_psi(current, current->psi_cm, soil_type,
                                                       soil_properties, *head);
            }
...
extern int lgarto_correction_type(int num_layers, double* cum_layer_thickness_cm, struct wetting_front** head,
  ...
  for (int wf = 1; wf != listLength(*head); wf++) {
    ...
    if (next != NULL) {
      ...
      if (((current->depth_cm > next->depth_cm) && (current->is_WF_GW == TRUE) &&
          ...
          lgarto_same_layer_TO_GW_near_contact_merge_ready(current, next) ||
	          lgarto_reversed_same_layer_TO_GW_duplicate_merge_ready(
	            current, next, *head, cum_layer_thickness_cm, mobile_groundwater_level,
	            effective_lower_boundary_depth_cm)) {
        correction_type = 2;
      }
    ...
    if (next != NULL) {
      const double boundary_psi_tol_cm =
        lgar_boundary_roundtrip_psi_tolerance_cm(current->psi_cm,
                                                 next->psi_cm);
      // A crossed surface front can become psi-continuous with a to_bottom GW
      // bridge before correction; still hand it to the boundary cleanup path.
      const bool surface_boundary_crossing_preserved =
        current->theta > next->theta ||
        (next->to_bottom == TRUE && next->is_WF_GW == TRUE &&
         std::fabs(current->psi_cm - next->psi_cm) <= boundary_psi_tol_cm);
      if ((current->is_WF_GW == FALSE) && (next->is_WF_GW == FALSE) &&
      ...
      if ((current->is_WF_GW == FALSE) &&
          ...
          surface_boundary_crossing_preserved && (current->layer_num != num_layers)) {
        correction_type = 6;
      }
    }
    ...
    if (lgarto_to_bottom_surface_front_connected_to_GW_below(current)) {
      correction_type = 8;
    }
...
static bool lgar_find_protected_mobile_groundwater_pair(
  struct wetting_front *head,
  double *cum_layer_thickness_cm,
  double expected_pair_depth_cm,
  struct wetting_front **vadose_side_front,
  struct wetting_front **saturated_support_front)
{
  ...
  if (head == NULL || cum_layer_thickness_cm == NULL) {
    return false;
  }

  int deepest_layer = 0;
  for (struct wetting_front *scan = head; scan != NULL; scan = scan->next) {
    deepest_layer = std::max(deepest_layer, scan->layer_num);
  }
  if (deepest_layer < 1) {
    return false;
  }
  ...
  const double depth_tol_cm =
    fmax(MOBILE_GROUNDWATER_SUBMERGENCE_TOL_CM,
         1.0e-10 * fmax(1.0, cum_layer_thickness_cm[deepest_layer]));
  return lgarto_find_mobile_groundwater_storage_pair(
    head, depth_tol_cm, vadose_side_front, saturated_support_front,
    expected_pair_depth_cm);
}
  ...
  double expected_pair_depth_cm)
{
  struct wetting_front *vadose_side_front = NULL;
  struct wetting_front *saturated_support_front = NULL;
  return front != NULL && lgar_find_protected_mobile_groundwater_pair(
           head, cum_layer_thickness_cm, expected_pair_depth_cm,
           &vadose_side_front, &saturated_support_front) &&
         (front == vadose_side_front || front == saturated_support_front);
}
                                                                   ...
                                                                   double *cum_layer_thickness_cm)
{
  ...
  while (*head != NULL && (*head)->next != NULL &&
    ...
    if (lgar_is_mobile_groundwater_storage_pair_member(
          *head, *head, cum_layer_thickness_cm)) {
      break;
    }
    if (verbosity.compare("high") == 0) {
                                                           ...
                                                           double *cum_layer_thickness_cm)
{
  ...
  while (leading_zero_depth_groundwater_fronts > max_leading_zero_depth_groundwater_fronts) {
    struct wetting_front *front_to_delete = *head;
    while (front_to_delete != NULL &&
           lgar_is_mobile_groundwater_storage_pair_member(
             front_to_delete, *head, cum_layer_thickness_cm)) {
      front_to_delete = front_to_delete->next;
    }
    if (front_to_delete == NULL || !front_to_delete->is_WF_GW ||
        front_to_delete->layer_num != 1 ||
        std::fabs(front_to_delete->depth_cm) > CREATION_COLOCATED_TOLERANCE_CM) {
      break;
    }
    if (verbosity.compare("high") == 0) {
    ...
    listDeleteFront(front_to_delete->front_num, head, soil_type, soil_properties);
    leading_zero_depth_groundwater_fronts =
      lgar_count_leading_zero_depth_groundwater_fronts(*head);
  }
}
...
static struct wetting_front *lgar_unlink_front_without_canonicalization(
  struct wetting_front *front,
  struct wetting_front **head)
{
...
{
  if (head == NULL || *head == NULL || cum_layer_thickness_cm == NULL ||
      soil_type == NULL || frozen_factor == NULL || soil_properties == NULL) {
    return false;
  }

  const double depth_tol_cm = ZERO_DEPTH_TO_DELETE_DEPTH_TOL_CM;
  int deepest_layer = 0;
  int root_zone_TO_fronts = 0;
  for (struct wetting_front *scan = *head; scan != NULL; scan = scan->next) {
    deepest_layer = std::max(deepest_layer, scan->layer_num);
    if (root_zone_depth_cm > 0.0 && scan->is_WF_GW && !scan->to_bottom &&
        scan->depth_cm > 1.0e-8 &&
        scan->depth_cm < root_zone_depth_cm - 1.0e-8 &&
        scan->next != NULL && scan->next->is_WF_GW) {
      root_zone_TO_fronts++;
    }
  }
  const double support_tol_cm =
  ...
  struct wetting_front *active_vadose = NULL;
  struct wetting_front *active_support = NULL;
  (void) lgarto_find_mobile_groundwater_storage_pair(
    *head, support_tol_cm, &active_vadose, &active_support);

  // Scan the full ordinary chain and remove only its least consequential
  // eligible interval; one invocation can therefore delete at most one front.
  struct lgarto_dense_TO_merge_candidate {
    struct wetting_front *merge_front = NULL;
    struct wetting_front *retained_front = NULL;
    int layer_num = 0;
  ...
  for (struct wetting_front *upper = *head;
       upper != NULL && upper->next != NULL && upper->next->next != NULL &&
         upper->next->next->next != NULL;
       upper = upper->next) {
    struct wetting_front *fronts[4] = {
      upper, upper->next, upper->next->next, upper->next->next->next
    };
    struct wetting_front *merge_front = fronts[1];
    struct wetting_front *retained_front = fronts[2];
    struct wetting_front *lower_front = fronts[3];
    ...
    bool ordinary_same_layer_chain = true;
    for (int i = 0; i < 4; i++) {
      if (!fronts[i]->is_WF_GW || fronts[i]->to_bottom ||
          fronts[i]->layer_num != layer_num ||
          fronts[i]->depth_cm <= depth_tol_cm ||
          !std::isfinite(fronts[i]->theta) ||
          !std::isfinite(fronts[i]->psi_cm) ||
          fronts[i] == active_vadose || fronts[i] == active_support ||
          (i < 3 &&
           (fronts[i + 1]->depth_cm <= fronts[i]->depth_cm + depth_tol_cm ||
            fronts[i]->theta > fronts[i + 1]->theta + state_tol ||
            fronts[i]->psi_cm + state_tol < fronts[i + 1]->psi_cm))) {
        ordinary_same_layer_chain = false;
        break;
      }
    }
    if (!ordinary_same_layer_chain) {
      continue;
    }
    ...
    const int soil_num = soil_type[layer_num];
    const struct soil_properties_ &soil = soil_properties[soil_num];
    const double merged_Se =
      calc_Se_from_theta(merged_theta, soil.theta_e, soil.theta_r);
    const double merged_psi_cm =
      calc_h_from_Se(merged_Se, soil.vg_alpha_per_cm, soil.vg_m, soil.vg_n);
    if (!std::isfinite(merged_theta) || !std::isfinite(merged_Se) ||
        !std::isfinite(merged_psi_cm) ||
        merged_theta + state_tol < merge_front->theta ||
        merged_theta > retained_front->theta + state_tol ||
        merged_psi_cm > merge_front->psi_cm +
          lgar_psi_assertion_tolerance_cm(merged_psi_cm, merge_front->psi_cm) ||
        merged_psi_cm +
          lgar_psi_assertion_tolerance_cm(merged_psi_cm, retained_front->psi_cm) <
            retained_front->psi_cm ||
        merged_psi_cm +
          lgar_psi_assertion_tolerance_cm(merged_psi_cm, lower_front->psi_cm) <
            lower_front->psi_cm) {
      continue;
    }

    const double upper_gap_threshold_cm =
      fmax(TO_PSI_GAP_REFINEMENT_MIN_GAP_CM,
    ...
    best.merge_front = merge_front;
    best.retained_front = retained_front;
    best.layer_num = layer_num;
  ...
  const int deleted_front_num = best.merge_front->front_num;
  const double mass_before_cm = lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
  const int soil_num = soil_type[best.layer_num];
  const struct soil_properties_ &soil = soil_properties[soil_num];
  best.retained_front->theta = best.merged_theta;
  best.retained_front->psi_cm = best.merged_psi_cm;
  best.retained_front->K_cm_per_h =
    calc_K_from_Se(best.merged_Se,
                   frozen_factor[best.layer_num] * soil.Ksat_cm_per_h,
                   soil.vg_m);
  best.retained_front->dzdt_cm_per_h = 0.0;
  (void) lgar_unlink_front_without_canonicalization(best.merge_front, head);
  const double mass_change_cm =
    lgar_calc_mass_bal(cum_layer_thickness_cm, *head) - mass_before_cm;

  if (!std::isfinite(mass_change_cm) ||
      std::fabs(mass_change_cm) > TO_PSI_GAP_REFINEMENT_MASS_TOLERANCE_CM) {
    fprintf(stderr,
            "Error: dense finite TO/GW front merge changed profile storage by %.17g cm.\n"
            "  deleted_front_num=%d tolerance=%.17g cm\n",
            mass_change_cm, deleted_front_num,
            TO_PSI_GAP_REFINEMENT_MASS_TOLERANCE_CM);
    fflush(stderr);
    listPrint(*head);
    fflush(stdout);
    abort();
  ...
  const char *reason)
{
  ...
  if (lgar_is_mobile_groundwater_storage_pair_member(
        front, *head, cum_layer_thickness_cm)) {
    return front;
  }
  (void)soil_type;
  (void)soil_properties;
  ...
  struct wetting_front *after_delete =
    lgar_unlink_front_without_canonicalization(front, head);
  const double mass_after_cm = lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
  const double mass_change_cm = mass_after_cm - mass_before_cm;
...
static bool lgar_finite_same_layer_TO_duplicate_psi_match(double psi_a_cm,
                                                          double psi_b_cm)
{
  if (!std::isfinite(psi_a_cm) || !std::isfinite(psi_b_cm)) {
    return false;
  }

  const double psi_scale_cm =
    fmax(1.0, fmax(std::fabs(psi_a_cm), std::fabs(psi_b_cm)));
  const double psi_tol_cm = fmax(1.0e-8, 1.0e-12 * psi_scale_cm);
  return std::fabs(psi_a_cm - psi_b_cm) <= psi_tol_cm;
}

static bool lgar_front_is_colocated_psi0_support_member(
  const struct wetting_front *front,
  int layer_num,
  double depth_cm,
  double depth_tol_cm)
{
  return front != NULL &&
         front->is_WF_GW &&
         front->to_bottom == FALSE &&
         front->layer_num == layer_num &&
         std::isfinite(front->depth_cm) &&
         std::fabs(front->depth_cm - depth_cm) <= depth_tol_cm &&
         std::isfinite(front->psi_cm) &&
         front->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM;
}

static bool lgar_delete_redundant_colocated_psi0_support_if_mass_neutral(
  int front_num,
  struct wetting_front **head,
  int *soil_type,
  struct soil_properties_ *soil_properties,
  ...
  const char *reason)
{
  ...
  struct wetting_front *candidate = *head;
  while (candidate != NULL && candidate->front_num != front_num) {
    candidate = candidate->next;
  }
  if (lgar_is_mobile_groundwater_storage_pair_member(
        candidate, *head, cum_layer_thickness_cm)) {
    return false;
  }
  ...
  (void) listDeleteFront(front_num, &trial_head, soil_type, soil_properties);
  const double trial_mass_after_cm =
    lgar_calc_mass_bal(cum_layer_thickness_cm, trial_head);
  const double trial_mass_change_cm = trial_mass_after_cm - mass_before_cm;
  listDelete(trial_head);

  if (!std::isfinite(trial_mass_change_cm) ||
      std::fabs(trial_mass_change_cm) > FINITE_SAME_LAYER_TO_DELETE_MASS_TOL_CM) {
    if (verbosity.compare("high") == 0) {
      printf("Skipping redundant colocated psi=0 TO/GW support deletion for front %d "
             "(%s); trial mass change %.17g cm exceeds %.17g cm.\n",
             front_num,
             reason != NULL ? reason : "no reason",
             trial_mass_change_cm,
             FINITE_SAME_LAYER_TO_DELETE_MASS_TOL_CM);
    }
    return false;
  }

  (void) listDeleteFront(front_num, head, soil_type, soil_properties);
  const double mass_after_cm = lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
  const double mass_change_cm = mass_after_cm - mass_before_cm;
  if (verbosity.compare("high") == 0) {
    printf("Deleted redundant colocated psi=0 TO/GW support front %d (%s; "
           "mass change %.17g cm).\n",
           front_num,
           reason != NULL ? reason : "no reason",
           mass_change_cm);
  ...
  if (!std::isfinite(mass_change_cm) ||
    ...
    fprintf(stderr,
            "Error: colocated psi=0 TO/GW support cleanup changed profile storage by %.17g cm.\n"
            "  deleted_front_num=%d tolerance=%.17g cm reason=%s\n"
            "  Wetting front list after deletion follows:\n",
            mass_change_cm,
            front_num,
            FINITE_SAME_LAYER_TO_DELETE_MASS_TOL_CM,
  ...
  double *cum_layer_thickness_cm)
{
  ...
  double fixed_column_depth_cm = 0.0;
  for (struct wetting_front *scan = *head; scan != NULL; scan = scan->next) {
    if (scan->layer_num > 0) {
      fixed_column_depth_cm =
        fmax(fixed_column_depth_cm, cum_layer_thickness_cm[scan->layer_num]);
    }
  }
  const double depth_tol_cm =
    fmax(MOBILE_GROUNDWATER_SUBMERGENCE_TOL_CM,
  ...
  while (deleted_front) {
    ...
    for (struct wetting_front *current = *head; current != NULL;
      ...
      if (current->is_WF_GW &&
          ...
          current->layer_num > 0 &&
          std::isfinite(current->depth_cm) &&
          std::fabs(current->depth_cm -
                    cum_layer_thickness_cm[current->layer_num]) <=
            depth_tol_cm &&
          std::isfinite(current->psi_cm) &&
          current->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM &&
          lgar_front_is_colocated_psi0_support_member(
            current->next, current->layer_num, current->depth_cm,
            depth_tol_cm) &&
          lgar_front_is_colocated_psi0_support_member(
            current->next->next, current->layer_num, current->depth_cm,
            depth_tol_cm)) {
        const int delete_front_num = current->next->next->front_num;
        deleted_front =
          lgar_delete_redundant_colocated_psi0_support_if_mass_neutral(
            delete_front_num, head, soil_type, soil_properties,
            cum_layer_thickness_cm,
            "extra saturated support after bottom to_bottom scaffold");
        break;
      }

      if (!lgar_front_is_colocated_psi0_support_member(
            current, current->layer_num, current->depth_cm, depth_tol_cm)) {
        continue;
      }

      const bool preceded_by_vadose_side_pair_member =
        previous != NULL &&
        previous->is_WF_GW &&
        previous->to_bottom == FALSE &&
        previous->layer_num == current->layer_num &&
        std::isfinite(previous->depth_cm) &&
        std::fabs(previous->depth_cm - current->depth_cm) <= depth_tol_cm &&
        std::isfinite(previous->psi_cm) &&
        previous->psi_cm > BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM;

      if (preceded_by_vadose_side_pair_member &&
          lgar_front_is_colocated_psi0_support_member(
            current->next, current->layer_num, current->depth_cm,
            depth_tol_cm)) {
        const int delete_front_num = current->next->front_num;
        deleted_front =
          lgar_delete_redundant_colocated_psi0_support_if_mass_neutral(
            delete_front_num, head, soil_type, soil_properties,
            cum_layer_thickness_cm,
            "extra saturated support below active vadose-side pair member");
        break;
      }
      ...
      if (lgar_front_is_colocated_psi0_support_member(
          ...
          current->next->next->is_WF_GW &&
          current->next->next->to_bottom &&
          current->next->next->layer_num == current->layer_num &&
          std::isfinite(current->next->next->depth_cm) &&
          std::fabs(current->next->next->depth_cm - current->depth_cm) <=
            depth_tol_cm &&
          std::isfinite(current->next->next->psi_cm) &&
          current->next->next->psi_cm <=
            BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM &&
          current->layer_num > 0 &&
          std::fabs(current->depth_cm -
                    cum_layer_thickness_cm[current->layer_num]) <=
        ...
        deleted_front =
          lgar_delete_redundant_colocated_psi0_support_if_mass_neutral(
            delete_front_num, head, soil_type, soil_properties,
            cum_layer_thickness_cm,
            "extra saturated support colocated with bottom to_bottom scaffold");
        break;
      }
      ...
      if (lgar_front_is_colocated_psi0_support_member(
        ...
        deleted_front =
          lgar_delete_redundant_colocated_psi0_support_if_mass_neutral(
            delete_front_num, head, soil_type, soil_properties,
            cum_layer_thickness_cm,
            "extra saturated support in colocated psi=0 stack");
        break;
      }
    }
  }
...
extern double lgarto_cleanup_redundant_colocated_psi0_support_stacks(double *cum_layer_thickness_cm,
                                                                     int *soil_type,
                                                                     struct wetting_front **head,
                                                                     struct soil_properties_ *soil_properties)
{
  ...
  const double mass_after_cm =
    lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
  const double mass_change_cm = mass_after_cm - mass_before_cm;
  return std::isfinite(mass_change_cm) ? mass_change_cm : 0.0;
}
  ...
  double *cum_layer_thickness_cm)
{
  ...
  while (deleted_front) {
    deleted_front = false;
    struct wetting_front *protected_vadose_front = NULL;
    struct wetting_front *protected_support_front = NULL;
    // Pair identity is unchanged while this pass only examines candidates.
    (void) lgar_find_protected_mobile_groundwater_pair(
      *head, cum_layer_thickness_cm, NAN,
      &protected_vadose_front, &protected_support_front);
    for (struct wetting_front *current = *head; current != NULL && current->next != NULL;
         current = current->next) {
      struct wetting_front *next = current->next;
      if (current->is_WF_GW == FALSE || next->is_WF_GW == FALSE ||
      ...
      if (current == protected_vadose_front || current == protected_support_front ||
          next == protected_vadose_front || next == protected_support_front) {
        continue;
      }
      ...
      const bool same_theta =
        std::fabs(current->theta - next->theta) <= 1.0e-12;
      const bool same_psi =
        lgar_finite_same_layer_TO_duplicate_psi_match(current->psi_cm,
                                                      next->psi_cm);
      if (!same_theta || !same_psi) {
        continue;
      }
      if (current->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM &&
          next->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM) {
        continue;
      }
      ...
      const double mass_after_cm = lgar_calc_mass_bal(cum_layer_thickness_cm, *head);
      const double mass_change_cm = mass_after_cm - mass_before_cm;
      const double delete_mass_tol_cm = FINITE_SAME_LAYER_TO_DELETE_MASS_TOL_CM;
      ...
      if (!std::isfinite(mass_change_cm) ||
          std::fabs(mass_change_cm) > delete_mass_tol_cm) {
        fprintf(stderr,
...
extern void lgar_clean_redundant_fronts(struct wetting_front** head, int *soil_type,
  ...
  lgar_trim_excess_leading_zero_depth_groundwater_fronts(
    head, soil_type, soil_properties, cum_layer_thickness_cm);
  if (apply_zero_depth_groundwater_cap) {
    lgar_cap_leading_zero_depth_groundwater_fronts(
      head, soil_type, soil_properties, cum_layer_thickness_cm);
  }
  lgar_dry_zero_depth_groundwater_fronts_to_surface_profile(
    head, soil_type, soil_properties, cum_layer_thickness_cm);
  ...
  for (int wf = 1; wf != (listLength(*head)); wf++) {
    ...
    // Any two-member saturated support is structurally ambiguous until the
    // scalar GW depth identifies the active pair; do not collapse it here.
    const bool saturated_support_pair =
      current->is_WF_GW && next->is_WF_GW &&
      !current->to_bottom && !next->to_bottom &&
      current->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM &&
      next->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM;
    const bool mobile_groundwater_support_pair =
      saturated_support_pair || (cum_layer_thickness_cm != NULL
        ? (current == protected_vadose_front || current == protected_support_front ||
           next == protected_vadose_front || next == protected_support_front)
        : current->is_WF_GW && next->is_WF_GW &&
          !current->to_bottom && !next->to_bottom &&
          next->psi_cm <= BOUNDARY_NEAR_SATURATION_PSI_SNAP_MAX_CM);
    ...
    if (same_layer && same_theta && colocated_in_depth &&
        ...
        !mobile_groundwater_support_pair) {
  ...
  struct soil_properties_ *soil_properties, double min_psi_cm)
{
  ...
  // A valid incoming dry state may lie above the generic solver cap.
  const double psi_wet_bound_cm =
    fmax(0.0, fmin(psi_dry_bound_cm, std::isfinite(min_psi_cm) ? min_psi_cm : 0.0));
  ...
  if (std::isfinite(original_psi_cm) &&
      ...
      original_psi_cm <= psi_dry_bound_cm) {
...
static void lgar_theta_mass_balance_correction_with_max_psi(bool use_dry_over_wet, int front_num, double prior_mass, struct wetting_front** head, double *cum_layer_thickness_cm, int *soil_type, struct soil_properties_ *soil_properties, double max_psi_cm){
  ...
  const bool original_psi_bounded = std::isfinite(original_psi_cm) &&
    original_psi_cm >= 0.0 && original_psi_cm <= psi_dry_bound_cm;
  double mass_at_bounded_original = mass_at_original;
  if (original_psi_bounded) {
    mass_at_bounded_original =
      lgar_apply_theta_mass_balance_correction_psi(use_dry_over_wet, original_psi_cm,
                                                   current, head, cum_layer_thickness_cm,
                                                   soil_type, soil_properties);
    if (fabs(mass_at_bounded_original - prior_mass) < best_abs_error_cm) {
  ...
  auto brackets_target = [&](double mass_lo_cm, double mass_hi_cm) {
    return prior_mass >= fmin(mass_lo_cm, mass_hi_cm) - MBAL_ITERATIVE_TOLERANCE &&
      prior_mass <= fmax(mass_lo_cm, mass_hi_cm) + MBAL_ITERATIVE_TOLERANCE &&
      fabs(mass_hi_cm - mass_lo_cm) > MBAL_ITERATIVE_TOLERANCE;
  };
  double bracket_lo_psi_cm = 0.0;
  double bracket_hi_psi_cm = psi_dry_bound_cm;
  double bracket_lo_mass_cm = mass_at_saturation;
  bool target_bracketed = false;

  if (original_psi_bounded) {
    const bool wet_bracketed = brackets_target(mass_at_saturation, mass_at_bounded_original);
    const bool dry_bracketed = brackets_target(mass_at_bounded_original, mass_at_dry_limit);
    // Prefer the side of the original state that supplies the required wetting or drying.
    if ((mass_at_bounded_original < prior_mass && wet_bracketed) || !dry_bracketed) {
      bracket_hi_psi_cm = original_psi_cm;
      target_bracketed = wet_bracketed;
    }
    else {
      bracket_lo_psi_cm = original_psi_cm;
      bracket_lo_mass_cm = mass_at_bounded_original;
      target_bracketed = dry_bracketed;
    }
  }
  if (!target_bracketed) {
    bracket_lo_psi_cm = 0.0;
    bracket_hi_psi_cm = psi_dry_bound_cm;
    bracket_lo_mass_cm = mass_at_saturation;
    target_bracketed = brackets_target(mass_at_saturation, mass_at_dry_limit);
  }

  if (target_bracketed) {
    double bracket_lo_error_cm = bracket_lo_mass_cm - prior_mass;
    for (int iter = 0; iter < MAX_ITER_MBAL_LOOP; iter++) {
      const double probe_psi_cm = 0.5 * (bracket_lo_psi_cm + bracket_hi_psi_cm);
      const double probe_mass_cm =
        lgar_apply_theta_mass_balance_correction_psi(use_dry_over_wet, probe_psi_cm,
                                                     ...
                                                     cum_layer_thickness_cm,
                                                     soil_type, soil_properties);
      const double probe_error_cm = probe_mass_cm - prior_mass;
      const double probe_abs_error_cm = fabs(probe_error_cm);
      if (probe_abs_error_cm < best_abs_error_cm) {
        best_psi_cm = probe_psi_cm;
        best_abs_error_cm = probe_abs_error_cm;
      }
      ...
      // Preserve a sign-changing bracket even when temporary depth inversions
      // make storage nonmonotonic over the full pressure-head range.
      if ((probe_error_cm > 0.0) == (bracket_lo_error_cm > 0.0)) {
        bracket_lo_psi_cm = probe_psi_cm;
        bracket_lo_error_cm = probe_error_cm;
      }
      else {
        bracket_hi_psi_cm = probe_psi_cm;
      }
